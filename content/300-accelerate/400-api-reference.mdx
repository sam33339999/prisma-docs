---
title: 'API 參考'
metaTitle: 'Accelerate: API 參考'
metaDescription: 'API 參考 文件 for Accelerate.'
tocDepth: 3
toc: true
---

The Accelerate API reference documentation is based on 以下 schema:
...
With the Accelerate extension for Prisma Client, 您可以 use the `cacheStrategy` parameter for model queries and use the [`ttl`](/postgres/database/caching#time-to-live-ttl) and [`swr`](/postgres/database/caching#stale-while-revalidate-swr) parameters to define a cache strategy for Accelerate. The Accelerate extension 需要 that you install Prisma Client version `4.10.0`.

### Options

The `cacheStrategy` parameter takes an option with 以下 keys:
...
以下 is a list of all read query operations that support `cacheStrategy`:
...
您可以 invalidate the cache using the [`$accelerate.invalidate` API](/accelerate/).
...
:::note
您可以 invalidate up to 5 tags per call.
:::

## `$accelerate.invalidateAll`

您可以 invalidate the entire cache using the `$accelerate.invalidateAll` API.
...
Starting from Accelerate version `2.0.0`, 您可以 provide a custom implementation of the fetch function when extending the Prisma Client with Accelerate. This allows you greater flexibility and control over how HTTP requests are handled within your application.

To pass a custom fetch implementation, 您可以 use 以下 pattern:
...
您可以 find the full error code reference for Prisma Accelerate [here](/orm/reference/error-reference#prisma-accelerate).
