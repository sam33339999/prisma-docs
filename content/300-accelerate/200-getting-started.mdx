---
title: '開始使用 with Prisma Accelerate'
metaTitle: '開始使用 with Prisma Accelerate'
metaDescription: '學習 如何 get up and running with Prisma Accelerate.'
sidebar_label: '開始使用'
tocDepth: 3
toc: true
---
...
開始使用 with Accelerate, you will need 以下:

- A [Prisma Data Platform account](https://console.prisma.io)
- A project that uses [Prisma Client](/orm/prisma-client) `4.16.1` or higher. If your project is using interactive transactions, 您需要 use `5.1.1` or higher. (We always recommend using the latest version of Prisma.)
...
If you require IP allowlisting or firewall configurations with trusted IP addresses, enable Static IP for enhanced security. Learn more on [how 啟用 static IP for Accelerate in the Platform Console](/accelerate/static-ip).
...
> `directUrl` is useful for you to carry out migrations and introspections. However, you don't need `directUrl` 使用 Accelerate in your application.
...
💡 Accelerate 需要 [Prisma Client](/orm/prisma-client) version `4.16.1` or higher and [`@prisma/extension-accelerate`](https://www.npmjs.com/package/@prisma/extension-accelerate) version `1.0.0` or higher.

💡 Accelerate extension [`@prisma/extension-accelerate`](https://www.npmjs.com/package/@prisma/extension-accelerate) version `2.0.0` and above 需要 Node.js version `18` or higher.
...
When using Prisma Accelerate in long-running application servers, such as a server deployed on AWS EC2, 您可以 generate the Prisma Client by executing 以下 command:

```terminal
npx prisma generate
```

When using Prisma Accelerate in a Serverless or an Edge application, we recommend you to run 以下 command to generate Prisma Client:
...
Add 以下 to extend your existing Prisma Client instance with the Accelerate extension:
...
If VS Code does not recognize the `$extends` method, 參閱 [this section](/accelerate/faq#vs-code-does-not-recognize-the-extends-method) on 如何 resolve the issue.
...
If you are using [Prisma Optimize](/optimize) in your application, make sure you apply it _before_ the Accelerate extension. 例如：
...
By enabling Accelerate and supplying the Accelerate connection string, your queries now use the connection pooler 預設情況下.
...
您應該 now see improved performance for your cached queries.
...
As of Prisma version `5.2.0` 您可以 use Prisma Studio with the Accelerate connection string.
...
例如, if a dashboard displays customer information and a customer’s contact details change, cache invalidation allows you to refresh only that data instantly, ensuring support staff always 參見 latest information without waiting for the cache to expire.
