---
title: '使用者 interface (GUI) guidelines'
metaTitle: 'Prisma docs 樣式 指南: GUI guidelines'
metaDescription: 'On this page you can find information 關於 the Prisma guidelines for documenting UI elements and steps to walk users through interfaces.'
tocDepth: 3
search: false
---
...
If 您需要 describe 結果s of a step, separate them with a line break so that they appear indented on a new line under the step.
...
To make UI steps easier to read, in most cases 您可以 omit the UI term for each UI element.
...
1. Click **Delete** for the item you want 刪除.
...
## Do not document 需求 for input fields (text boxes)
...
Document the user interface as it appears and do not alter UI text in the documentation to meet style guide or other 需求.

例如, if a button has the text CANCEL, match the all-caps in the documentation.
