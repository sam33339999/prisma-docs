---
title: '拼寫, punctuation, and formatting'
metaTitle: 'Prisma docs 樣式 指南: 拼寫, punctuation, and formatting'
metaDescription: 'This section of the 樣式 指南 provides guidelines on 如何 present the information you 添加.'
tocDepth: 4
...
- Contractions that include the words "is" or "are". 例如： it's and you're.
- However, use contractions sparingly.

However, avoid other contractions. 例如：
...
For the names of GUI elements (buttons, drop-down menus, and so on), use **bold**. Also use the same capitalization as in the GUI. 例如：
...
Exception: they are acceptable in congratulatory or welcome messages, 例如： "Congratulations - you've completed the tutorial!"

### Capitalize and spell out proper nouns

Although you might be tempted 使用 abbreviations like "Postgres" (instead of the official proper noun "PostgreSQL") or not worry about casing when writing "Javascript" (instead of "JavaScript"), we use the official forms of proper nouns. A few common examples are:
...
Sometimes there are some terms where it's not clear whether 使用 a hyphen or not. To strive for consistency, we list those terms here.
...
- "您必須 regenerate Prisma Client when you introduce a new data source"

## Files and file types

When you 參閱 a file or file type, use lower case and code format, and include the dot. Use "an" as the preposition if the filename extension, when pronounced, starts with a vowel, otherwise use "a":
...
注意： when you 參閱 a specific file, use the capitalization that is used in the file name:

- the `schema.prisma` file

### Special case: JSON

- When you 參閱 JSON in general, use all caps and no code format
- When you 參閱 the Prisma `Json` API, use `Json`
- When you 參閱 a JSON file, use the formatting rules above: "a `.json` file"

### Use inline code format for paths and file names

例如：

- "The generated Prisma Client is located in the `./node_modules/.prisma` folder 預設情況下."
- "The `schema.prisma` file is located in..."
- "使用 multiple `.env` files..."

## Use inline code format when referring to strings in text

例如：

"以下 query returns all records where the `email` field is equal to `Sarah`."

## Avoid excessive code formatting

Documents can quickly get visually cluttered if they have too much special formatting. Our docs are highly technical, and we often 參閱 code snippets or technical keywords that appear in the user's code. For these keywords, it's appropriate for us 使用 code formatting. However, we should not 參閱 general technologies (such as JSON) with code formatting:

例如：

Prisma automatically converts JavaScript objects (例如, `{ extendedPetsData: "none"}`) to JSON.

## Make lists clear with the Oxford Comma

Use the Oxford Comma except in titles. It is a comma used after the penultimate item in a list of three or more items, before "and" or "or". It makes things clearer. 範例： "... an Italian painter, sculptor, and architect".
...
This [`createMany()`](/orm/reference/prisma-client-reference/#createmany) query does 以下:

- Creates several `User` records
- Creates several nested `Post` records
- Creates several nested `Category` records

### Show 結果 of a query wherever possible

Use the [`<CodeWithResult>`](/about/docs-components/mdx-examples#code-with-result) component to show a query and 結果s of that query.

### Use `highlight` comments to highlight code

If 您需要 highlight lines in your code samples, use the [`highlight` magic comments](/about/docs-components/mdx-examples#code-block-with-highlighted-code). 例如：
...
Use 以下 as reference when creating and editing docs: [formatting inline code and code blocks](https://github.com/adam-p/markdown-here/wiki/Markdown-Cheatsheet#code).
...
As an example, consider 以下 code block where `__DATABASE_CONNECTION_URL__` is a placeholder for the PostgreSQL connection URL:
...
Whenever you use a placeholder, explain 如何 obtain a value for the placeholder, or link to another resource that explains this. Explicitly call out that this is a placeholder that must be replaced with a "real value". Include an example of what that real value might look like:
...
When 您需要 provide a series of CLI commands as instructions to the reader, use a single block. Don't use a list unless you want to provide context for each step:
