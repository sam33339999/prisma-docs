---
title: '編寫 樣式'
metaTitle: 'Prisma docs 樣式 指南: 編寫 樣式'
metaDescription: 'This section of the 樣式 指南 provides guidelines on tone of voice, audience, and other tonal matters.'
tocDepth: 4
...
- English has many synonyms - where possible, choose the simplest available word for the job. 範例： "in" instead of "within", "use" instead of "utilize".
...
US English is the most globally recognized form of English. Use US English punctuation, grammar, and spelling. 例如：
...
Try to be as specific as possible when you 參閱 a specific noun. Do not 參閱 the noun as "it" unless you have just defined the proper noun in a previous sentence or clause. Even then, your writing might be clearer for international audiences if you specify the noun again. In particular, avoid starting sentences with "It".

## Write in the second person

The second person ("you") gives a conversational tone and speaks directly to the reader. Avoid the first person ("I", "we", "let's", and "us"). 範例：

> "您必須 commit the entire `prisma/migrations` folder to source control."

Exception: Use "we" when you 參閱 Prisma the organization. 例如, here Prisma (the organization) recommends a course of action:

> "We recommend that you share a single instance of `PrismaClient` across your application".

## Use inclusive language

When you 參閱 one or more people in the third person, use inclusive, gender-neutral language. Use "they/them/their" instead of "he/him/his" or "she/her/her". Avoid gender-specific words like "guys".
...
The Prisma docs include a lot of technical detail, and jargon is unavoidable. However, we strive 使用 as little jargon as possible.

When you use jargon, follow these guidelines:

- If 您可以 explain the jargon in a few words, then you might p參閱 explain it there and then. Use your judgement to decide whether that is best for this doc.
...
- For longer explanations, link to a definition elsewhere.
  ...
  - If the jargon is _not_ specific to Prisma, check if 您可以 find the definition quickly with a web search. If so, then do not explain the term or link to a definition. We can reasonably expect users to know the jargon or to find an explanation for themselves.
...
Use active voice instead of passive voice. It's a more concise and straightforward way to communicate. 例如：
...
Sometimes 您可以 omit the acting component or module altogether:

- “參閱 the generated log file in `/directoryX`.”
...
- Use the `createMany` method 建立 multiple records in a single transaction
- 您可以 use nested writes 建立 a user and that user's posts at the same time
...
範例： "Ensure that you rebuild your schema.".
...
- Use present simple even when you want to write about something that will happen as 結果 of a user action: say that 結果 happens, not that it will happen.
...
When a paragraph or sentence offers an optional path, the beginning of the first sentence should indicate that it’s optional. 例如, "if you’d like to learn more about xyz, see our reference guide" is clearer than "Go to the reference guide if you’d like to learn more about xyz."
...
For optional steps in a procedure, a succinct way to convey this is to precede the step with "Optional:". 例如：

5. Optional: In the **Tags** field, specify one or more tags for your file. Separate multiple tags with commas.

## 程式碼範例

Start all query examples with a constant. This gives you a noun to 參閱 later in the document, and in most cases this clarifies the example.
