---
title: 'Prisma docs 圖片 guidelines'
metaTitle: 'Prisma docs 樣式 指南: 圖片 guidelines'
...
您可以 take screenshots with your favorite tool. We find it very easy 使用 SnagIt when 您需要 edit screenshots or add annotations to them. We suggest its use as a best practice.
...
- In SnagIt, 您可以 add a shadow with **Effects** > **Shadow** and configure its parameters as listed below.
...
If 您可以, avoid annotations. If necessary, apply annotations that are minimal and similar in style to the screenshot below.
...
For details, 參見 sections below. Again, we suggest the use of SnagIt as a best practice.
...
- Do not use randomized characters in the filename. 例如, `d0fjlsf81.png`.
- At the beginning of filenames, use keywords that identify the UI context.
- If applicable, follow with keywords that specify the action that you demonstrate in the screenshot. 例如： `data-browser-select-model.png`.
...
- Use [Figma](https://www.figma.com) 建立 diagrams.
...
If 您需要 shrink an image by width, use the `<img>` component.
