...
title: '關於'
metaTitle: '關於'
metaDescription: '關於 the Prisma docs.'
...
- [Our style guide](/about/style-guide) is a good place to start to 學習關於 how we write our documentation and the tone we use.
...
`User` and `Post` are the canonical models that are being used throughout the Prisma docs. The `User` and `Post` models have been selected for 以下 reasons:
...
Meet the user where they are. If a piece of information needs to be described in multiple places, do so, but be sure 使用 [a markdown fragment](https://docusaurus.io/docs/markdown-features/react#importing-markdown) so that information can be updated in one place in the future.
