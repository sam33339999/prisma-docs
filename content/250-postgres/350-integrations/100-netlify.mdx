---
title: 'Netlify'
metaTitle: 'Prisma Postgres via Netlify 擴充功能'
metaDescription: '學習 如何 建立 Prisma Postgres 資料庫 via the official Netlify 擴充功能 and deploy your applications with it.'
tocDepth: 3
toc: true
---
...
安裝 the extension, click **Install** at the top of the [Prisma Postgres extension page](https://www.netlify.com/integrations/prisma).

### Add the Prisma 平台 integration token to your Netlify team

Perform 以下 steps _once_ to connect your Netlify team with a Prisma Platform workspace:
...
4. Follow the prompts 建立 a new Netlify integration token and copy the token value.
5. Paste the token into the **Integration Token** field above. The workspace ID will be automatically filled in.
6. Click **Save** to complete the setup.

Once this setup is complete, your Netlify team is connected to your Prisma workspace. 您可以 now configure individual Netlify sites 使用 Prisma Postgres.

### Add Prisma Postgres to a Netlify site

Perform 以下 steps _for every Netlify site_ in which you want 使用 Prisma Postgres:
...
To ensure the generated Prisma Client library is available on your deployed Netlify site, 您應該 add a `postinstall` script to the `scripts` section of your `package.json` file:
...
## 範例： Deploy a Next.js template with Prisma Postgres
...
在本節中, you are going 添加 Prisma Postgres to the starter project _on your local machine_:
...
1. Install the Prisma CLI as a development dependency:
    ...
    ```
1. Initialize Prisma ORM 建立 Prisma schema, config and `.env` file:
    ```terminal
    npx prisma init
    ```

#### 3.2. Run 遷移 and create sample data

1. Open the newly created `schema.prisma` file and add 以下 model to it:
...
#### 3.3. Update application code to 查詢 and show `User` records
...
With this code in place, 您可以 now go ahead and run the app locally:

```terminal
npm run dev
```

您應該 see a list of the `User` records that you created in the previous step.

#### 3.4. Add the `postinstall` script to generate Prisma Client

As mentioned [above](#generate-prisma-client-in-a-postinstall-script-in-packagejson), 您需要 add a `postinstall` script to your `package.json` to ensure your Prisma Client library gets properly generated:
...
在本節中, 您需要 add the Netlify extension to your Netlify site. Follow the instructions in the [Usage](#usage) section above to do that.

After having completed these steps, find the **Trigger deploy** button and select **Clear cache and deploy site** in the dropdown.

### 5. Validate the 部署

Open the deployed site by clicking the **Open production deploy** button. 您應該 now 參見 same UI as you did at the end of step 3 when you were running the app locally.