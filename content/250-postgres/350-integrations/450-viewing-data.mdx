---
title: 'Viewing 資料'
metaTitle: 'Viewing 資料'
metaDescription: 'Viewing and editing 資料 in Prisma Postgres via Prisma Studio or other 資料庫 GUIs.'
tocDepth: 3
toc: true
---

您可以 view and edit your data in Prisma Postgres using either [Prisma Studio](/orm/tools/prisma-studio) or 3rd party database editors.
...
您可以 also run Prisma Studio locally by running:

```terminal
npx prisma studio
```

This should start a live server in `http://localhost:5555` where 您可以 visit and interact with your database.

## Connecting to Prisma Postgres instance with 3rd party database editors

您可以 connect to your Prisma Postgres instance using third party database editors like pgAdmin, TablePlus, Postico etc using [`@prisma/ppg-tunnel` package](https://www.npmjs.com/package/@prisma/ppg-tunnel). 參見 example below to connect using TablePlus.
...
Run 以下 command to connect to your Prisma Postgres instance via `@prisma/ppg-tunnel` package:
...
Copy the **`port`** from 輸出 above, you will need it in the next step.
...
Based on the database editor you are using, 您可以 connect to your Prisma Postgres instance using the details you obtained from 輸出 of the `@prisma/ppg-tunnel` package. 添加 the connection string in TablePlus:

1. Open TablePlus and click on the **+** icon 添加 a new connection.
2. Select **PostgreSQL** as the database type.
3. Enter 以下 details:
    - **Name**: Any name you want to give to your connection.
    - **Host**: `127.0.0.1` in this case. 
    - **Port**: The **`port`** number you obtained from 輸出 of the `@prisma/ppg-tunnel` package.
    - **User**: 這將 be ignored due to the tunnel, 您可以 provide any value here.
    - **Password**:  這將 be ignored due to the tunnel, 您可以 provide any value here.
...
Based on the database editor you are using, 您可以 connect to your Prisma Postgres instance using the details you obtained from 輸出 of the `@prisma/ppg-tunnel` package. 添加 the connection string in DataGrip:
...
3. Enter 以下 details:
    ...
    - **Host**: `127.0.0.1` in this case. 
    - **Port**: The **`port`** number you obtained from 輸出 of the `@prisma/ppg-tunnel` package.
    - **User**: 這將 be ignored due to the tunnel, 您可以 keep this field empty.
    - **Password**:  這將 be ignored due to the tunnel, 您可以 keep this field empty.
    - **URL**: In the URL append 以下 query parameter `?sslmode=disable` to the end of the URL.
...
Based on the database editor you are using, 您可以 connect to your Prisma Postgres instance using the details you obtained from 輸出 of the `@prisma/ppg-tunnel` package. 添加 the connection string in DBeaver:

1. Open DBeaver and click on the **New Database Connection** button or **File** > **New** 添加 a new connection.
2. Select **PostgreSQL** as the database type.
3. Select **URL** option in Connect by section.
4. Enter the URL in this format:
    - **URL**: Your URL should be in this format: `jdbc:postgresql://localhost:52604/postgres?sslmode=disable`. Here make sure that you enter the port number you obtained from 輸出 of the `@prisma/ppg-tunnel` package. In this case, the port number is `52604`. There is no need to enter Username or Password as authentication is manged by Tunnel. 
...
Based on the database editor you are using, 您可以 connect to your Prisma Postgres instance using the details you obtained from 輸出 of the `@prisma/ppg-tunnel` package. 添加 the connection string in Postico:

1. Open Postico and click on the **New Server** button 添加 a new connection.
2. Enter 以下 details:
    - **Name**: Any name you want to give to your connection.
    - **Host**: `127.0.0.1` in this case. 
    - **Port**: The **`port`** number you obtained from 輸出 of the `@prisma/ppg-tunnel` package.
