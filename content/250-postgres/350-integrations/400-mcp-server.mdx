...
metaDescription: '管理 Prisma Postgres 資料庫 使用 LLMs with the Prisma 模型-Context-Protocol (MCP) Server'
tocDepth: 3
toc: true
---
...
您可以 start the remote MCP server as follows:
...
Once you're connected to the remote MCP server, 您可以 also always prompt your AI agent to "List the Prisma tools" to get a full overview of the latest supported tools.
...
您可以 start the local MCP server as follows:
...
在本節中, we're covering the config formats of the most popular AI tools.
...
這將 prompt you to open VS Code. Once opened, you'll be guided 安裝 the Prisma MCP server directly into your VS Code configuration. 

If your browser blocks the link, [您可以 set it up manually](https://code.visualstudio.com/docs/copilot/chat/mcp-servers#_add-an-mcp-server-to-your-workspace) by creating a `.vscode/mcp.json` file in your workspace and adding:
...
To learn more about Cursor's MCP integration, 查看 the [Cursor MCP docs](https://docs.cursor.com/context/model-context-protocol#configuration-locations).

#### Add via one-click installation

您可以 add the Prisma MCP server to Cursor using the [one-click installation](https://docs.cursor.com/context/model-context-protocol#one-click-installation) by clicking on 以下 link:
...
這將 prompt you to open the Cursor app in your browser. Once opened, you'll be guided 安裝 the Prisma MCP server directly into your Cursor configuration.

#### Add via Cursor Settings UI

When opening the **Cursor Settings**, 您可以 add the Prisma MCP Server as follows:
...
Adding it via the  **Cursor Settings** settings will modify the global `~/.cursor/mcp.json` config file. In this case, the Prisma MCP server will be 可用於 _all_ your Cursor projects:
...
To learn more about Windsurf's MCP integration, 查看 the [Windsurf MCP docs](https://docs.codeium.com/windsurf/mcp).

#### Add via Windsurf MCP Plugin Store (Recommended)

Use the Prisma MCP plugin from the [Windsurf MCP Plugin Store](https://docs.windsurf.com/windsurf/cascade/mcp#adding-a-new-mcp-plugin). Follow [the steps here](/orm/more/ai-tools/windsurf#add-prisma-mcp-server-via-windsurf-plugins) 添加 the Prisma MCP plugin in Windsurf. This is the simplest and recommended way 添加 the Prisma MCP server to Windsurf.

#### Add via Windsurf Settings UI

When opening the **Windsurf Settings** (via **Windsurf - Settings** > **Advanced Settings or Command Palette** > **Open Windsurf Settings Page**), 您可以 add the Prisma MCP Server as follows:
...
Adding it via the  **Windsurf Settings** will modify the global `~/.codeium/windsurf/mcp_config.json` config file. Alternatively, 您可以 also manually add it to that file:
...
您可以 add the Prisma MCP to Warp as a globally available tool. First, [visit your MCP settings](https://docs.warp.dev/knowledge-and-collaboration/mcp#how-to-access-mcp-server-settings) and click **+ Add**. From here, 您可以 configure the Prisma MCP server as JSON. Use the `command` and `args` properties to start the Prisma MCP server as a setup command. 您可以 optionally configure Prisma to activate on startup using the `start_on_launch` flag:
...
Claude Code is a terminal-based AI tool where 您可以 add MCP server using the `claud mcp add` command for the local MCP server:
...
Follow the instructions in the [Claude Desktop MCP docs](https://modelcontextprotocol.io/quickstart/user#2-add-the-filesystem-mcp-server) 建立 the required configuration file:
...
Alternatively, 您可以 add the _remote_ MCP server as an [integration](https://www.anthropic.com/news/integrations):

1. In a new chat, find the **Connect apps** field in 主要 Claude UI below the input prompt field
...
您可以 add the remote Prisma MCP server to ChatGPT to manage your Prisma Postgres databases using natural language. Learn more about setting it up in our [ChatGPT integration guide](/orm/more/ai-tools/chatgpt).
