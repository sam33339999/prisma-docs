---
title: '管理 API'
metaTitle: 'Prisma Postgres: 管理 API 參考'
metaDescription: '管理 API 參考 文件 for Prisma Postgres.'
---
...
:::tip OpenAPI
An interactive [**OpenAPI 3.1 specification** is available here](https://api.prisma.io/v1/swagger-editor), where 您可以 explore endpoints, request/response bodies, and detailed examples.
:::
...
Append an endpoint path to the base URL to construct the full URL for a request. 例如：
...
使用 OAuth 2.0, your application must:
...
3. **Exchange the code for an access token**: Use the code from step 2 in 以下 request
...
您可以 explore and interact with all endpoints in a live Swagger UI playground.
