---
title: 'Full 資料表 scans caused by LIKE operations'
metaTitle: 'Optimize Recommendations: Full 資料表 scans caused by LIKE operations'
metaDescription: "學習 關於 the recommendation provided by Optimize for full 資料表 scans caused by Like operations."
tocDepth: 3
toc: true
---
...
以下 query targeting the `User` model provides `contains` and `endsWith` as options, which translate to `LIKE` and `ILIKE` SQL operators.
