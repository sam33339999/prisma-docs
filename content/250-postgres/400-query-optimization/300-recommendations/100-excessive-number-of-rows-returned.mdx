---
title: 'Excessive 數字 of rows returned'
metaTitle: 'Optimize Recommendations: Excessive 數字 of rows returned'
metaDescription: "學習 關於 the recommendation provided by Optimize for excessive 數字 of rows returned."
tocDepth: 3
toc: true
---
...
以下 query targeting a `User` model does not provide a [`take` option](/orm/reference/prisma-client-reference#findmany):
...
- **Impact on the user's device:** Displaying all the data at once can strain the user's device resources. 例如, loading thousands of rows in a web application can slow down or freeze the browser, consuming significant memory and CPU resources.
