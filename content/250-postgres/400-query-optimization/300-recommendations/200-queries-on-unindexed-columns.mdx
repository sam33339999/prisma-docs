---
title: '未索引欄位的查詢'
metaTitle: 'Optimize 建議: 未索引欄位的查詢'
metaDescription: "學習 關於 the recommendation provided by Optimize for 未索引欄位的查詢."
tocDepth: 3
toc: true
---
...
以下 queries targeting the `User` model use a [`where` property](/orm/prisma-client/queries/filtering-and-sorting) to filter on columns that do not have indexes:
...
- **Space vs. time:** Indexing 需要 additional storage space to save index data, but it significantly speeds up data retrieval.
...
- **Columns with large data:** Indexing columns with large data can lead to higher storage 需求 and might not provide significant performance improvements.
