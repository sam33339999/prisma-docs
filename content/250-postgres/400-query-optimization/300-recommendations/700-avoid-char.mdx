...
metaDescription: "學習 關於 the recommendation provided by Optimize for 使用 `@db.Char(n)` 原生 型別."
---
...
In 以下範例, the `@db.Char(n)` native type has been used within the `Item` model on the `name` field:
