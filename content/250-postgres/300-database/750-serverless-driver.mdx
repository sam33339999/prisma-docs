---
title: "Serverless 驅動程式"
metaTitle: "Serverless 驅動程式"
metaDescription: "A lightweight PostgreSQL 驅動程式 for Prisma Postgres optimized for serverless and 邊緣 environments with HTTP/WebSocket 支援, result streaming, and minimal memory footprint."
tocDepth: 3
toc: true
...
- 相容於 Cloudflare Workers, Vercel Edge Functions, Deno Deploy, AWS Lambda, Bun, and browsers
...
The serverless driver 需要 a Prisma Postgres Direct TCP connection URL:
...
Parameters over 1KB are automatically streamed without buffering in memory. For large binary parameters, 您必須 use `boundedByteStreamParameter()` which creates a `BoundedByteStreamParameter` object that carries the total byte size, required by the PostgreSQL protocol:
...
The `boundedByteStreamParameter()` function is provided by the `@prisma/ppg` library and 需要 the total byte size to be known in advance due to PostgreSQL protocol 需求.
...
參見 [npm package documentation](https://www.npmjs.com/package/@prisma/ppg) for more details.
...
參見 [npm package](https://www.npmjs.com/package/@prisma/ppg) for complete API documentation.
...
- 需要 a Prisma Postgres instance and does not work with [local development](/postgres/database/local-development) databases
