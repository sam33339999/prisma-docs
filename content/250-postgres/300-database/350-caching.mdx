---
title: '快取 查詢 in Prisma Postgres'
sidebar_label: '快取'
metaTitle: '快取 查詢 in Prisma Postgres'
metaDescription: '學習 關於 快取 查詢 in Prisma Postgres'
tocDepth: 3
toc: true
---

Prisma Postgres supports built-in query caching to reduce database load and improve query performance. 您可以 configure cache behavior using the `cacheStrategy` option 可用於 all read queries.
...
## Setting up 快取 in Prisma Postgres

啟用 query caching in your Prisma Postgres project, you need 配置 Accelerate caching and set up the client extension. Follow these steps:

### 1. Enable 快取 in the 平台 控制台
...
### 3. Configure Prisma Client with 快取

Update your Prisma Client setup 使用 the Accelerate extension:
...
Once configured, 您可以 add caching to any read query using the `cacheStrategy` option:
...
Your caching setup is now complete! Continue reading to 學習關於 different cache strategies and 如何 optimize them for your use case.

## Cache strategies

For all read queries in Prisma Client, 您可以 define the `cacheStrategy` parameter that configures cache behavior. The cache strategy allows you to define two main characteristics of the cache:
...
Time-to-Live (TTL) determines how long cached data is considered fresh. By specifying the `ttl` in seconds, 您可以 control the duration for which data in the cache remains valid. When a read query is executed, if the cached response is within the `ttl` limit, Prisma Client retrieves the data from the cache without querying the database. If the cached data is not available or has expired, Prisma Client queries the database and stores 結果s in the cache for future requests.
...
### Invalidate the TTL and keep your cached 查詢 results up-to-date

If your application 需要 real-time or near-real-time data, cache invalidation ensures that users 參見 most current data, even when using a large `ttl` (Time-To-Live). By invalidating your cache, 您可以 bypass extended caching periods to show live data whenever it's needed.

例如, if a dashboard displays customer information and a customer’s contact details change, TTL (Time-To-Live) settings ensure the cache automatically expires after a set duration. This allows the system to refresh only the updated data at the next access, ensuring support staff always 參見 latest information without manually refreshing the cache.
...
To invalidate a cached query result, 您可以 add tags and then use the `$accelerate.invalidate` API.
...
To invalidate the query below, 您需要 provide the cache tag in the `$accelerate.invalidate` API:
...
### Invalidate the SWR and keep your cached 查詢 results up-to-date

If your application 需要 real-time or near-real-time data, cache invalidation ensures that users 參見 most current data, even when using a large `swr` (Stale-While-Revalidate). By invalidating your cache, 您可以 bypass extended caching periods to show live data whenever it's needed.

例如, consider a dashboard that displays stock levels for products in a warehouse. With SWR (Stale-While-Revalidate) settings, the dashboard can immediately display the last known stock data, even if it’s slightly outdated, while new data is fetched in the background. This ensures that staff can continue working with recent information without waiting, with the stock levels updating as soon as revalidation completes.

However, in cases where stock data needs to be updated immediately—例如, if a product is low in stock and the count needs real-time accuracy—cache invalidation allows the system to proactively clear specific data from the cache. This forces a refresh of the latest stock data instantly, so staff always have the most up-to-date information without waiting for SWR to complete the revalidation.
...
Caching helps you improve query response times and reduce database load. However, it also means you might serve stale data to the client. Whether or not serving stale data is acceptable and to what extent depends on your use case. `ttl` and `swr` are parameters 您可以 use the tweak the cache behavior.
...
**When to invalidate:** If a user makes significant updates to their profile, such as changing their profile picture or bio, the cache should be [invalidated](/postgres/database/caching#on-demand-cache-invalidation) immediately to ensure that followers 參見 latest updates without waiting for the SWR refresh.

### Cache strategy using TTL + SWR

For very fast response times and reduced database load, use both TTL and SWR. 您可以 use this strategy to fine-tune your application’s tolerance for stale data.
...
**When to invalidate:** If a critical update or breaking news article is published, the cache should be [invalidated](/postgres/database/caching#on-demand-cache-invalidation) immediately to ensure readers 參見 latest information without delay. This approach is especially useful for applications where certain news items may need to override the normal cache cycle for timeliness.

## On-demand cache invalidation

If your application 需要 real-time or near-real-time data, cache invalidation ensures that users 參見 most current data, even when using a large `ttl` (Time-To-Live) or `swr` (Stale-While-Revalidate) [cache strategy](/postgres/database/caching#cache-strategies). By invalidating your cache, 您可以 bypass extended caching periods to show live data whenever it's needed.

您可以 invalidate the cache using the [`$accelerate.invalidate` API](/accelerate/api-reference#accelerateinvalidate):
...
您需要 provide the cache tag in the `$accelerate.invalidate` API:
...
例如, if a query is executed on a critical path without specifying a cache strategy, 結果 may be incorrect, with no clear explanation. This issue often arises when implicit caching is unintentionally left enabled.

To avoid such problems, 您必須 explicitly opt-in to caching. This ensures you are aware that caching is not enabled 預設情況下, preventing potential errors.
