---
title: "Postgres 擴充"
metaTitle: "Postgres 擴充"
metaDescription: "學習 關於 使用 Postgres 擴充 with Prisma Postgres"
tocDepth: 3
toc: true
...
### 2. Create and use the extension in your 遷移 file

In the empty migration file, 您可以 write any custom SQL to be executed in the database:
...
### 3. Execute the 遷移 against the 資料庫

Run 以下 command to execute the migration and apply its changes in your database:
...
### 4. Pull the document table into your Prisma 綱要
...
Because the `VECTOR` type is not yet natively 支援 Prisma ORM, it's represented as an [`Unsupported`](/orm/prisma-schema/data-model/models#unsupported-types) type.
...
您可以 also use [TypedSQL](/orm/prisma-client/using-raw-sql/typedsql) for type-safe SQL queries against your database.

## Temporary 限制

### Limited availability of 擴充
...
### No Prisma Studio 支援 for special data types from 擴充
...
## Other 擴充 are coming soon

Support for 以下 extensions is going to come soon:
