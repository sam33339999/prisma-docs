...
metaDescription: '學習 如何 embed Prisma Studio in your own applications to provide users with an amazing 資料 editing experience.'
tocDepth: 3
toc: true
---
...
:::tip
If you want to see what embedded Studio looks like, **[查看 the demo](https://github.com/prisma/studio-core-demo) on GitHub**!
:::

## Use cases

您可以 embed Prisma Studio in your own app in various scenarios:
...
- Backend:
  ...
  - A Prisma Postgres instance (您可以 create one with `npx prisma init --db`)
...
In your React app, 您可以 use the `Studio` component to render the tables in your database via Prisma Studio. It receives an _executor_ which is responsible for packaging the current SQL query in an HTTP request (also allowing for custom headers/payloads) and sending it to the `/studio` endpoint in your backend.

> 查看 the [demo](https://github.com/prisma/studio-core-demo/blob/main/frontend/index.tsx) on GitHub for a full reference implementation.
...
您可以 customize the look and feel of Prisma Studio so that it matches your application’s design. This is done by passing a custom theme to the `Studio` component. A theme is simply a set of CSS variables that define colors, spacing, and other style properties for both light and dark modes.
...
With this setup, Studio inherits your custom colors, borders, and typography rules, making it feel like a natural part of your app rather than a separate tool. 您可以 define as many or as few variables as you need depending on the level of customization you want.
...
> 查看 the [demo](https://github.com/prisma/studio-core-demo/blob/main/server/index.ts) on GitHub for a full reference implementation.
...
When you want to authenticate the users of your app against Prisma Studio, 您可以 do that by adding custom logic around your embedded Prisma Studio version.

On the frontend, 您可以 ensure to pass the `Authorization` header and other data (e.g. a user ID) when creating the executor:
...
In your server-side implementation, 您可以 then retrieve these values from the incoming request and extract the Prisma Postgres API key that's needed for this user's query:
...
🔐 移除 our branding or to inquire about upcoming partner-only features, ping us here: [partnerships@prisma.io](mailto:partnerships@prisma.io)
