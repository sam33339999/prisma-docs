---
title: "備份"
metaTitle: "備份 in Prisma Postgres"
metaDescription: "學習 關於 備份 in Prisma Postgres"
tocDepth: 3
toc: true
---
...
Please 請注意 any database changes or events that occurred after the most recent snapshot may not be restored.
...
If you would like 建立 a backup file of your database, 您可以 use `pg_dump` and use a [direct connection](/postgres/database/direct-connections). This is useful for migrating data between databases or creating a local copy of your database.
...
建立 backups, ensure you have the PostgreSQL command-line tools installed. Run 以下 commands based on your operating system:
...
您可以 now dump the database by running 以下 command and using your own connection string:

```terminal
pg_dump --dbname="postgres://USER:PASSWORD@db.prisma.io:5432/?sslmode=require" > ./mydatabase.bak
```

這將 create your backup file named `mydatabase.bak` in the current directory.