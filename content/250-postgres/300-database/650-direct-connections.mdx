...
title: "直接連線"
metaTitle: "直接連線 (TCP)"
metaDescription: "學習 關於 connecting directly to your Prisma Postgres 資料庫 via direct TCP."
tocDepth: 3
toc: true
---
...
If you connect to it via another tool, 您可以 do so with a [direct connection string](#connection-string) following the conventional PostgreSQL format.

## 如何 connect to Prisma Postgres via direct TCP

In order to get a direct connection string, 您需要:
...
SSL mode is required when connecting to Prisma Postgres via direct TCP, so 您需要 append `sslmode=require` to your TCP connection string.
...
Prisma Postgres closes idle connections after an extended period of time. If that happens in your application, 您可以 re-open a new connection. (Most database clients re-connect automatically.)
...
User permissions are limited to read, write and schema changes. It is not possible 建立 separate databases, manage users and roles, or perform other administrative actions.
...
The tunnel expects you to have 以下 `DATABASE_URL` environment variable set to the connection URL of your Prisma Postgres instance. If you are running the tunnel command from your project where an `.env` file has `DATABASE_URL` already set, 您可以 skip this step as the tunnel will automatically pick it up.
...
To start the proxy server, run 以下 command:
...
這將 start the tunnel on a randomly assigned TCP port. The proxy automatically handles authentication, so any database credentials are accepted. The tunnel also encrypts traffic, meaning clients should be set to not require SSL.

您可以 now connect to your Prisma Postgres editor using your favorite PostgreSQL client, e.g. `psql` or a GUI like [TablePlus](/postgres/integrations/viewing-data#2a-connect-to-prisma-postgres-using-tableplus) or [DataGrip](/postgres/integrations/viewing-data#2b-connect-to-prisma-postgres-using-datagrip). To do so, you only need to provide the **`host`** and **`port`** from 輸出 above. The TCP tunnel will handle authentication via the API key in your Prisma Postgres connection URL, so 您可以 omit the values for **`username`** and **`password`.**

### 自訂 host and port

預設情況下, the tunnel listens on `127.0.0.1` and assigns a random port. Since it provides access to your Prisma Postgres database, it should only be exposed within a trusted network. 您可以 specify a custom host and port using the `--host` and `--port` flags:
...
When using the TCP tunnel, keep 以下 in mind:
