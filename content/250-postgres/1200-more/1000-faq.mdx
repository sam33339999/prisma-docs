---
title: 'Prisma Postgres 常見問題'
sidebar_label: '常見問題' 
metaTitle: '常見問題 | Prisma Postgres'
metaDescription: '學習 關於 the 常見問題 regarding Prisma Postgres.'
tocDepth: 3
toc: true
---
...
Yes, 您可以 use Prisma Postgres with any database library or tool via a [direct connection](/postgres/database/direct-connections).
...
1. Verify Your GitHub Email Address
    - Check 主要 email address associated with your GitHub account (e.g., from your GitHub profile or notification settings).
2. Create a New Email/Password Account
    - Go to the email/password sign-up page.
    - Use the _same email address_ linked to your GitHub account 建立 the new account.
...
This might be an issue with the TypeScript server not yet recognizing the regenerated Prisma Client. To resolve this, 您需要 restart TypeScript.

1. In VS Code, open the Command Palette. 您可以 do so when you press F1 or select **View** > **Command Palette**.
...
## What regions is Prisma Postgres 可用於?

Prisma Postgres is currently 可用於 以下 regions:
...
### Does 查詢 execution time affect pricing in Prisma Postgres?
...
### 如何 pricing differ between using Prisma ORM and direct TCP connections?
...
請注意 a single Prisma ORM query may translate into multiple SQL queries which may make using Prisma ORM more economical than direct SQL.

### Do read and write 查詢 cost the same?

Yes, read and write queries are counted equally as _operations_ and are billed the same way.

### Does a `SELECT 1` 查詢 count as a billable operation?
...
您可以 estimate your operation usage in Prisma ORM by integrating an application performance monitoring tool like Prometheus. 

### What strategies can I use to optimize cost per operation?

Prisma Postgres bills by operation. The more 您可以 perform using a single operation, the lower your bill. Some tips to reduce the number of operations:
...
- **Use nested-relation helpers** such as [`connectOrCreate`](/orm/reference/prisma-client-reference#connectorcreate) or [`set`](/orm/reference/prisma-client-reference#set) 建立 or link related records in a single operation.
...
If a later query needs 結果 of an earlier one (例如, you need the user ID you just created), stick with an interactive transaction for correctness. Otherwise, batching and array transactions let you collapse multiple queries into a single billed operation, keeping both your operation count, and your cost down.
...
We will use 以下 equations to estimate the monthly bill:
...
您可以 use your own MAU count, activity level, and storage used to project costs on any plan using the equations above.

:::

We will associate each workload with a paid plan and its corresponding pricing details, 例如, the **Starter plan** for the small workload, the **Pro plan** for the medium workload, and the **Business plan** for the large workload. Then we will apply the equations to the example workloads to generate a rough estimate of a monthly bill. 例如：
...
We also have a Free plan, but we are leaving it out in 以下 calculations because it's intended for evaluation only and is not meant for production workloads. 您可以 also learn more about the pricing details for each plan on the [pricing page](https://www.prisma.io/pricing?utm_source=docs).

:::

**Example of a small workload on the Starter plan**:

A hobby or early-stage side-project with ~`500` MAUs. Each user performs ~`10` actions per day, and the entire database uses ~`0.5` GB of storage. Based on the assumptions, you would calculate the monthly bill using 以下 equations:
...
A growing SaaS product serving ~`5000` MAUs. Power users average ~`40` actions per day, and the app stores ~`6` GB of data. Based on the assumptions, you would calculate the monthly bill using 以下 equations:
...
A production-grade, consumer-facing application handling ~`50000` MAUs. Heavy usage with ~`60` actions per user per day drives significant traffic, and the dataset reaches ~`40` GB.  Based on the assumptions, you would calculate the monthly bill using 以下 equations:
...
### How do I 升級 my plan if I am using Prisma Postgres via Vercel?
...
### 如何 Prisma Postgres's cache layer know what region to fetch the cache from?
...
您可以 invalidate the cache on-demand via the [`$accelerate.invalidate` API](/accelerate/api-reference#accelerateinvalidate) if you're on a [paid plan](https://www.prisma.io/pricing#accelerate), or 您可以 invalidate your entire cache, on a project level, a maximum of five times a day. This limit is set based on [your plan](https://www.prisma.io/pricing). 您可以 manage this via the Accelerate configuration page.

### 什麼是 Prisma Postgres's 快取 layer's consistency 模型?
...
The freshness of the data served by the cache depends on the cache strategy defined in your query. 參閱 [this section](/postgres/database/caching#selecting-a-cache-strategy) 欲了解更多資訊 on selecting the right cache strategy for your query.

### How is Prisma Postgres's 快取 layer different from other 快取 tools, such as Redis?
...
- Is globally distributed, 預設情況下, reducing the latency of your queries. Other cache tools would require additional configuration to make them available globally.

### When should I not use Prisma Postgres's 快取 功能?
...
- Your app is exclusively used within a specific region and both your application server and database are situated in that same region on the same network. 例如, database queries will likely be much faster if your application server and database are in the same region and network. However, If your application server is in different regions or networks from your database, the cache nodes will speed up your queries because the data will be cached in the closest data center to your application.
...
### 什麼是 the maximum allowed value for the `ttl` parameter when configuring `cacheStrategy`?

The [Time-to-live](/postgres/database/caching#time-to-live-ttl) (`ttl`) parameter can be set for up to a _year_. However, it's important to 請注意 items within the cache may be evicted if they are not frequently accessed.
...
Prisma Postgres's cache layer performs best when it observes a higher load from a project. Many cache operations, such as committing data to cache and refreshing stale data, happen asynchronously. When benchmarking the cache layer, we recommend doing so with loops or a load testing approach. 這將 mimic higher load scenarios better and reduce outliers from low frequency operations.
...
### What regions are Prisma Postgres's cache nodes 可用於?

Prisma Postgres's cache layer runs on Cloudflare's network and cache hits are served from Cloudflare's 300+ locations. 您可以 find the regions where Prisma Postgres's cache nodes are available here: [https://www.cloudflare.com/network/](https://www.cloudflare.com/network/).

### How long does it take to invalidate a cache 查詢 result?
...
### 什麼是 the difference between **Invalidate** and **Revalidate**?
...
### 什麼是 on-demand cache invalidation?
...
- **High-priority data**: For time-sensitive data, like breaking news or urgent notifications, where it’s essential for users to 參見 most current information right away.
...
#### Can I increase the 查詢 duration and response size limits for my Prisma Postgres instance?

Yes, 您可以 increase your Prisma Postgres limits based on your subscription plan. Here are the configurable limits:
...
:::warning
While 您可以 increase these limits based on your subscription plan, it's *still* recommended to optimize your database operations. [Learn more in our troubleshooting guide.](/postgres/database/api-reference/error-reference)
:::


## 查詢最佳化

Prisma Postgres allows query optimization via [Prisma Optimize](/optimize) and provides performance recommendations to help improve your database queries during development. 您可以 enable it with Prisma Postgres or [also use it with your own database](/optimize/getting-started), but setup and integration steps differ.

### Can you automatically implement optimizations?

Prisma Postgres's query optimization feature offers insights and recommendations on 如何 improve your database queries. It does not alter any existing queries or your Prisma schema.
...
Yes, the recommendation usage resets at the beginning of each calendar month. 例如, if you use `5` recommendations by the end of the month, your usage will reset to `0` at the start of the next month.

### Can I get charged for exceeding the recommendation limit on the starter plan?

Yes, if you’re on the starter plan, exceeding `5` recommendations in a billing cycle will result in a `$5` charge at the end of that cycle. 欲了解更多資訊, visit [our pricing page](https://www.prisma.io/pricing#optimize).

### How are viewed Prisma AI 建議 tracked for billing? Are they counted based on generated or viewed 建議?
...
### Can I enable 查詢 optimizations for Prisma Postgres in production?
...
您可以 use the `enable` property in the client extension to run it [only in development environment](https://www.npmjs.com/package/@prisma/extension-optimize). 預設情況下, the `enable` property is set to `true`.
...
### Why do I see "[optimize] HTTP 409 Conflict: There is no active recording to write 查詢 to" warning?
