...
metaDescription: 'Prisma Studio is a visual 資料庫 editor.'
...
Unlike previous versions, the new Prisma Studio is SQL-driven and does not rely on the Prisma schema file at all. Instead, it introspects your database directly to understand the schema structure. This means 您可以 use it with any supported database without needing a Prisma schema or project.

請注意 Prisma Studio is not open source but 您可以 still create issues in the [`prisma/studio`](https://github.com/prisma/studio) repo. All "old Studio" issues are being closed as there won't be any further work on the previous version.

## 前置需求

Prisma Studio is a standalone tool that only 需要 a database connection. You have two options:

### Option 1: Use with any 資料庫 (no Prisma project required)
...
This approach 適用於out any Prisma ORM setup—Studio will introspect your database schema directly.
...
If you have a Prisma project, run 以下 command in your project directory:
...
Studio displays all tables from your database in a sidebar. Select any table to view its data in a table format. 您可以 open multiple tables in separate tabs to work with related data simultaneously.

### View and edit records

您可以 edit data in two ways:
...
## 資料庫 支援 Prisma Studio

Prisma Studio currently supports 以下 databases: PostgreSQL, MySQL, and SQLite.

### SQLite 需求 for Prisma Studio
...
- **Node.js 20**: 需要 installing `better-sqlite3` as a dependency
...
If you don't have `node:sqlite` 可用於 your runtime or prefer not 安裝 `better-sqlite3` as a hard dependency (it adds ~10MB), 您可以 use `npx` to temporarily install the required packages:
...
Caching issues may cause Prisma Studio 使用 an older version of the query engine. You may see 以下 error:
