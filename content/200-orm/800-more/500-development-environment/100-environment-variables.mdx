---
title: 'Managing Prisma ORM 環境 variables and settings'
metaTitle: 'Managing Prisma ORM 環境 variables and settings'
sidebar_label: '環境 variables'
metaDescription: '學習 如何 管理 the 環境 variables and settings in your Prisma ORM project'
---

An environment variable is a key value pair of string data that is stored on your machine's local environment. 參閱 our [Environment variables reference documentation](/orm/reference/environment-variables-reference) for specific details.
...
With Prisma ORM v6.4.0, we released the `prisma.config.ts` file. This file allows you to manage your environment variables and settings in a more flexible way. [View our reference](/orm/reference/prisma-config-reference) 欲了解更多資訊.
...
The Prisma CLI looks for `.env` files, in order, in 以下 locations:
...
If a `.env` file is located in step 1., but additional, clashing `.env` variables are located in steps 2. - 4., the CLI will throw an error. 例如, if you specify a `DATABASE_URL` variable in two different `.env` files, you will get 以下 error:
...
以下 table describes where the Prisma CLI looks for the `.env` file:
...
| `prisma [command]`                              | No schema (例如, when running `prisma db pull` in an empty directory) | `./.env`<br />`./prisma/.env`                             |

Any environment variables defined in that `.env` file will automatically be loaded when running a Prisma CLI command.

:::info

Looking 使用 more than one `.env` file? See [Using multiple `.env` files](#using-multiple-env-files) for information on 如何 setup and use multiple `.env` files in your application.

:::

參閱 the `dotenv` documentation for information about [what happens if an environment variable is defined in two places](https://www.npmjs.com/package/dotenv#what-happens-to-environment-variables-that-were-already-set).
...
Additionally, 您可以 use environment variables in the expansion that are set _outside_ of the `.env` file. 例如 a database URL that is set on a PaaS like Heroku or similar:
...
這將 make the environment variable `DATABASE_URL_WITH_SCHEMA` with value `postgresql://test:test@localhost:5432/test?schema=foo` available for Prisma ORM.

### Using environment variables in your code

If you want environment variables to be evaluated at runtime, 您需要 load them manually in your application code (例如, by using [`dotenv`](https://github.com/motdotla/dotenv)):
...
If you are using a custom file name for your environment variables, 您可以 configure `dotenv` 使用 that filename:
...
If you need variable expansion across environment files, 您可以 additionally use [`dotenv-expand`](https://github.com/motdotla/dotenv-expand):
...
If you are using multiple `.env` files, 您可以 refernce an environment file in your project's code depending on the environment you are running in.
...
以下範例 will use setting the `DATABASE_URL` environment variable which is often used for the database connection URL.
...
以下範例 illustrate 如何 set the environment variable (for the current user) using both Command Prompt (`cmd.exe`) and PowerShell, depending on your preference.
...
Then using a package like [`dotenv-cli`](https://www.npmjs.com/package/dotenv-cli), 您可以 load the correct connection URL for the environment you are working in.
...
In order for Prisma ORM and Jest to know which `.env` file 使用, alter your `package.json` scripts to include and call the `dotenv` package and specify which file 使用 depending on what commands you are running and in which environment you want them to run.
...
您可以 use the [`dotenv-cli`](https://www.npmjs.com/package/dotenv-cli) package to specify which environment file Prisma ORM should use when running a migration.
...
When running tests, we advise you to [mock Prisma Client](/orm/prisma-client/testing/unit-testing#mocking-prisma-client). In doing so, 您需要 tell Jest which environment it should use when running its tests.

預設情況下, Prisma Client will use the environment specified in the default `.env` file located at the project's root.
