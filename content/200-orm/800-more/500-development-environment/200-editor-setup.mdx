---
title: 'Editor 設定'
metaTitle: 'Editor and IDE 設定'
metaDescription: '學習 如何 configure your editor and IDEs for an optimal developer experience with Prisma ORM.'
tocDepth: 3
---

<TopBlock>

本頁描述 how 您可以 configure your editor for an optimal developer experience when using Prisma ORM.
...
您可以 install the official [Prisma VS Code extension](https://marketplace.visualstudio.com/items?itemName=Prisma.prisma). It adds extra capabilities to VS Code when developing applications with Prisma ORM:
...
- Code Completion
  ...
  - 您可以 trigger this manually with the `Ctrl+Space` shortcut.
- Documentation help
...
- Formatting
  - Format code either manually or on save (if configured).
    - To automatically format on save, add 以下 to your `settings.json` file:    
      ```json
      "editor.formatOnSave": true
      ```
    - 啟用 formatting in combination with `prettier`, add 以下 to your `settings.json` file:or use the [Prettier plugin for Prisma](https://github.com/umidbekk/prettier-plugin-prisma)        
      ```json
      "[prisma]": {
...
> **Note**: Community projects are not maintained or officially 支援 Prisma and some features may by out of sync. Use at your own discretion.
...
您可以 get IDE-style autocompletion for Prisma CLI using [`inshellisense`](https://github.com/microsoft/inshellisense/tree/main). It supports: bash, zsh, fish, pwsh, powershell (Windows Powershell).
...
`inshellisense` is built on top of [Fig](https://fig.io/) which 您可以 also use directly. It works in bash, zsh, and fish.
