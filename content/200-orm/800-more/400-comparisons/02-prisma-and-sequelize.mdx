...
metaDescription: '學習 how Prisma ORM compares to Sequelize.'
...
Use `model: Post as "Post"` if you used an alias to define the relationship between `User` and `Post` - 例如： `User.hasMany(Post, { as: "Post", foreignKey: "authorId" });`
...
Sequelize [doesn't offer a dedicated API for relation filters](https://github.com/sequelize/sequelize/issues/10943). 您可以 get similar functionality by sending a raw SQL query to the database.
