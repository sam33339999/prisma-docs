---
title: 'ORM releases and 成熟度 levels'
metaTitle: 'ORM releases and 成熟度 levels'
metaDescription: '學習 關於 the release 處理, versioning, and 成熟度 of Prisma ORM components and 如何 deal with breaking changes that might happen throughout releases.'
tocDepth: 3
---

本頁說明 the release process of Prisma ORM, how it's versioned and 如何 deal with breaking changes that might happen throughout releases.

## Releases

Prisma ORM releases typically happen every two weeks. 請注意 this is _not_ a hard rule – releases might be postponed for internal reasons.
...
Previews are typically available behind a feature flag or require some form of opt-in (例如, by providing a `--preview-feature` flag in the CLI or [adding them to a `previewFeatures` property in the `generator` block](/orm/reference/preview-features/cli-preview-features) for Prisma Client in your Prisma schema).

We don't recommend using Preview features or products in production.

另請參閱: [All currently available Preview features](/orm/reference/preview-features).
...
#### 如何 SemVer versioning work?

Semantic Versioning (SemVer) uses 以下 rules for version upgrade (quoted from the [SemVer](https://semver.org/) spec):
...
#### 如何 Prisma ORM versioning follow SemVer?
...
- Breaking changes can still be rolled out in `MINOR` but only for opt-in Preview and Early Access features that are not active 預設情況下 (e.g. via a Preview feature flag or a specific opt-in option or new CLI command).
...
> **注意：** Up until version `2.28.0`, Prisma ORM did not follow SemVer versioning strictly. This means that releases in the `2.MINOR.PATCH` range, `MINOR` versions may have included breaking changes. To learn more about Prisma ORM's adoption of SemVer, 查看 the [blog post](https://www.prisma.io/blog/prisma-adopts-semver-strictly).
