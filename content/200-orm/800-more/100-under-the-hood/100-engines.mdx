...
title: '引擎'
metaTitle: '引擎'
metaDescription: "Prisma's 查詢 引擎 manages the communication with the 資料庫 when 使用 Prisma ORM. 學習 how it works on this page."
---

:::warning[Prisma ORM v7 is Rust binary-free 預設情況下]

As of **Prisma ORM v7**, Prisma ORM runs without Rust engines 預設情況下. This means:
...
本節描述 the **legacy Rust-based engine architecture**. In Prisma ORM v7, the default architecture uses a TypeScript-based query compiler without Rust engines.
...
You might want 使用 a [custom library or binary](https://github.com/prisma/prisma-engines) file if:
...
Use 以下 environment variables to specify custom locations for your Rust engine binaries:
...
您可以 define environment variables globally on your machine or in the `.env` file.
...
Run 以下 command to set the environment variable globally (in this example, `PRISMA_QUERY_ENGINE_BINARY`):
...
Run 以下 command to output the paths to all binaries:

```terminal
npx prisma -v
```

輸出 shows that the query engine path comes from the `PRISMA_QUERY_ENGINE_BINARY` environment variable:
...
This can be useful if you have a custom OS that 需要 custom-built engines.
...
請注意 the query engine is implemented in Rust. The source code is located in the [`prisma-engines`](https://github.com/prisma/prisma-engines/) repository.

## The 查詢 engine at runtime
...
When using Rust-based engines, Prisma Client loads the query engine as a [Node-API library](https://nodejs.org/api/n-api.html) 預設情況下. 您可以 alternatively [configure Prisma 使用 the query engine compiled as an executable binary](#configuring-the-query-engine), which is run as a sidecar process alongside your application.
...
以下 diagram depicts a "typical flow":
...
1. The query engine returns 結果 as plain old JavaScript objects to Prisma Client
...
## Responsibilities of the 查詢 engine
...
The Rust-based query engine has 以下 responsibilities in an application that uses Prisma Client:
...
您可以 access the logs of the Rust-based query engine by setting the [`DEBUG`](/orm/prisma-client/debugging-and-troubleshooting/debugging) environment variable to `engine`:

```terminal
export DEBUG="engine"
```

您可以 also get more visibility into the SQL queries that are generated by the query engine by setting the [`query` log level](/orm/reference/prisma-client-reference#log-levels) in Prisma Client:
...
## Configuring the 查詢 engine

:::warning

The configuration options 在本節中 apply **only to Rust-based engines** used in Prisma ORM v6.19 and earlier. Prisma ORM v7 uses the TypeScript-based query compiler 預設情況下 and does not use these Rust engine configurations.

:::

### Defining the 查詢 engine type for Prisma Client

When using Rust-based engines, 您可以 choose between a Node-API library or an executable binary. Configure the query engine type by providing the `engineType` property to the Prisma Client `generator`:
...
The environment variable `PRISMA_CLIENT_ENGINE_TYPE` is not supported in Prisma ORM v7 because Prisma Client now defaults to the TypeScript-based query compiler without Rust engine binaries. See [Prisma ORM without Rust engines](/orm/prisma-client/setup-and-configuration/no-rust-engine) 欲了解更多資訊.

:::

:::info[Historical context]

- Until Prisma 3.x the default and only engine type available was `binary`, so there was no way 配置 the engine type to be used by Prisma Client and Prisma CLI.
- From versions [2.20.0](https://github.com/prisma/prisma/releases/2.20.0) to 3.x the `library` engine type was available and used 預設情況下 by [activating the preview feature flag](/orm/reference/preview-features/client-preview-features#enabling-a-prisma-client-preview-feature) "`nApi`" or using the `PRISMA_FORCE_NAPI=true` environment variable.
...
### Defining the 查詢 engine type for Prisma CLI

When using Rust-based engines, Prisma CLI also uses its own query engine. 您可以 configure it 使用 the binary version of the query engine by defining the environment variable [`PRISMA_CLI_QUERY_ENGINE_TYPE=binary`](/orm/reference/environment-variables-reference#prisma_cli_query_engine_type).

:::warning

The environment variable `PRISMA_CLI_QUERY_ENGINE_TYPE` is not supported in Prisma ORM v7 because Prisma CLI now defaults to the TypeScript-based query compiler without Rust engine binaries. See [Prisma ORM without Rust engines](/orm/prisma-client/setup-and-configuration/no-rust-engine) 欲了解更多資訊.
