---
title: 'Vercel 建置 dependency caching workaround'
metaTitle: '學習 to configure your 建置 處理 on Vercel to avoid caching-related problems'
metaDescription: '學習 to configure your 建置 處理 on Vercel to avoid caching-related problems'
---
...
If you deploy an application using Prisma ORM to [Vercel](https://vercel.com/), you may run into 以下 error message on deployment:
...
Another way 配置 `prisma generate` to be run on every deployment is 添加 the command to the build settings via Vercel's UI.
