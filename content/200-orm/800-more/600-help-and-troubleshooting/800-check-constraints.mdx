---
title: '資料 驗證 with CHECK constraints (PostgreSQL)'
metaTitle: '資料 驗證 with CHECK constraints (PostgreSQL)'
metaDescription: '學習 如何 configure CHECK constraints for 資料 驗證 with Prisma ORM and PostgreSQL by following the step-by-step instructions in this practical 指南.'
---

## Overview

本頁說明 how 配置 [check constraints](https://www.postgresql.org/docs/9.4/ddl-constraints.html#DDL-CONSTRAINTS-CHECK-CONSTRAINTS) in a PostgreSQL database. A check constraint is a condition that must be satisfied before a value can be saved to a table - 例如, the discounted price of a product must always be less than the original price.
...
Start by creating a project directory for the files that you'll create throughout this guide. Open terminal or command line and run 以下 commands:
...
Then execute 以下 command in your terminal 建立 a new database called `CheckDemo`:
...
您可以 validate that the database was created by running the `\dt` command which lists all tables (_relations_) in your database (right now there are none):
...
在本節中, you'll **create a new table with a single check constraint on a single column** in the `CheckDemo` database.

Create a new file named `single-column-check-constraint.sql` and add 以下 code to it:
...
Now run the SQL statement against your database 建立 a new table called `product`:
...
Run 以下 command to 參見 a list of check constraints that apply to the `product` table:

```
\d+ product
```

You will see 以下 output, which includes a list of all check constraints:
...
請注意 PostgreSQL will auto-generate a constraint name if you do not provide one. 例如, the constraint created by `price NUMERIC CHECK (price > 0.01 AND price <> 1240.00)` would be `price_check`.
...
Create a new file named `multi-column-check-constraint.sql` and add 以下 code to it:
...
Now run the SQL statement against your database 建立 a new table called `anotherproduct`:
...
Create a new file named `multiple-check-constraints.sql` and add 以下 code to it:
...
Now run the SQL statement against your database 建立 a new table called `secondtolastproduct`:
...
Congratulations, you just created a table called `lastproduct` in the database. The table has three columns named `reducedprice`, `price` and `tags`, and 以下 check constraints:
...
在本節中, you'll **add a check constraint to a table that already exists in your database**. To do so, you first need 建立 a new table and then alter the table 添加 the constraint.

Create a new file named `add-single-check-constraint-later.sql` and add 以下 code:
...
2. Alter the table 添加 a check constraint named `price_not_zero_constraint`

Now run the SQL statements against your database 建立 a new table called `lastproduct`:
...
在本節中 you'll introspect your database to generate the Prisma models for these tables.
...
In order to introspect your database, 您需要 tell Prisma ORM 如何 connect to it. You do so by configuring a `datasource` in your Prisma schema.

Create a new file named `schema.prisma` and add 以下 code to it:
...
In the above code snippet, 您需要 replace the uppercase placeholders with your own connection details. 例如, if your database is running locally it could look like this:

```
DATABASE_URL=postgresql://janedoe:mypassword@localhost:5432/CheckDemo
```

With both the `schema.prisma` and `.env` files in place, 您可以 run Prisma ORM's introspection with 以下 command:
...
Run 以下 command 安裝 and generate Prisma Client in your project:

```
npx prisma generate
```

Now 您可以 use Prisma Client to send database queries in Node.js.

## 8. Validate the check constraints in a Node.js script

Create a new file named `index.js` and add 以下 code to it:
...
To validate the multi-column check constraint, replace the code in `index.js` with 以下:
