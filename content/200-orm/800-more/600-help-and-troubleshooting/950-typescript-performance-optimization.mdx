---
title: 'Optimizing TypeScript 效能 with large Prisma schemas'
metaTitle: 'Optimizing TypeScript 效能 with large Prisma schemas'
metaDescription: '學習 如何 dramatically improve TypeScript compilation 效能 when 使用 large Prisma schemas 使用 型別 optimization strategies'
...
本指南向您展示如何 achieve these dramatic performance gains using TypeScript's `typeof` operator instead of direct type references.
...
(Of course, if you're familiar with the TypeScript type system, 您可以 use other methods.)
...
3. **Memory Efficiency**: Referencing an existing instance's inferred type 需要 significantly less memory than expanding complex conditional types and generics
...
The complete benchmark code and test cases used to verify this analysis are 可用於 the ts-bench repository:  
