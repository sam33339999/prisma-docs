---
title: '使用 dates in PostgreSQL'
metaTitle: 'PostgreSQL 日期 Types - 格式, Functions, and More'
metaDescription: '讀取 on to 學習 關於 如何 使用 日期 types in PostgreSQL, including information on formats, functions, and more.'
---

## Introduction

The ability to store date values inside of your database allows you 添加 a time element to your queries and analysis of your data. It is important to know 如何 work with date types in your respective database so that 您可以 be accurate in your reporting whether it's order information, peoples' ages, or any other use case.

在本指南中, we are going to discuss storing `DATE` types in PostgreSQL and the various ways that 您可以 work with them.
...
In PostgreSQL, it is possible to set a default date value of the current date. This can be done when creating your table by using the `DEFAULT` and `CURRENT_DATE` keywords. The `last_checkout` column from our library checkouts table accepts the current date 預設情況下:
...
Then when querying the `checkouts` table, we get 以下:
...
By knowing the ins and outs of the `DATE` type in PostgreSQL, you are then able 使用 functions working with the information that you store. We'll walk through some common functions building off of the table introduced in the prior section.

### Get the current date

In PostgreSQL, 您可以 get the current date and time by using the built-in `NOW()` function. 以下 statement will return both the day and time:
...
If the time is not of interest, 您可以 also specify to only return the date with double colons `::` to cast a `DATETIME` value to the `DATE` value:
...
All three of these options will return you the date in the `yyyy-mm-dd` format. Within PostgreSQL, 您可以 adjust the format of this output if desired.
...
2. the template that defines 輸出 format
...
您可以 also display the date in a format like `Sep 27, 2021`:
...
Depending on the 需求 of a system, you may need a date formatted in a specific way. This is a scenario where being able to specify 輸出 in PostgreSQL is useful.
...
We can continue with the same example to calculate the age at the current date in years, months, and days using the `AGE()` function. 以下 statement uses the `AGE()` function to calculate the age of a publication from our library `checkouts` tables:
...
It is important to 請注意 if you pass a single date into the `AGE()` function, then it will automatically use the current date to subtract and calculate. You are also able to pass two dates into the function to calculate age such as:
...
以下 statement pulls out the year, month, and day from the published date of <u>Ulysses</u>:
...
This is a useful function to be aware of when you may only need a portion of a date value for a calculation with your data 例如.

## Conclusion

在本指南中, we covered the basics of what 您可以 do with the `DATE` data type in PostgreSQL. It is important to know how date data works inside of your database. Having a grasp on the ways 您可以 access it and operate on it allows you to make age calculations, execute extractions in your queries, and also configure your output if necessary for matching another system's 需求.
...
例如, 您可以 write 以下 and return the hour, `20`:
...
To extract a part of a date in PostgreSQL, 您可以 use the `EXTRACT()` function. This function can parse out the year, month, or day from a date type as well as specify quarter.
...
您可以 truncate a timestamp in PostgreSQL by using the `DATE_TRUNC()` function. This function truncates a `TIMESTAMP` or `INTERVAL` based on a specified date part such as year, month, day, etc.
