---
title: 'An 介紹 to PostgreSQL 資料 types'
metaTitle: 'PostgreSQL 資料 Types - Numeric, Text, and More'
metaDescription: "PostgreSQL's 資料 型別 system allows you to define your data structures and store data in various formats.  These are some of the most common data types."
---

## Introduction

One of 主要 features of relational databases in general is the ability to define [schemas](/orm/more/help-and-troubleshooting/dataguide/database-glossary#schema) or [table structures](/orm/more/help-and-troubleshooting/dataguide/database-glossary#table) that exactly specify the format of the data they will contain. This is done by prescribing the columns that these structures contain along with their [_data type_](/orm/more/help-and-troubleshooting/dataguide/database-glossary#data-type) and any constraints.

Data types specify a general pattern for the data they accept and store. Values must adhere to the 需求 that they outline in order to be accepted by PostgreSQL. While it is possible to define custom 需求, data types provide the basic building blocks that allow PostgreSQL to validate input and work with the data using appropriate operations.

PostgreSQL includes [a wide range of data types](https://www.postgresql.org/docs/current/datatype.html) that are used to label and validate that values conform to appropriate types. 在本指南中, we will discuss the most common data types 可用於 PostgreSQL, the different input and output formats they use, and how 配置 various fields to meet your applications' needs.

### 什麼是 the data types in PostgreSQL?
...
### 開始使用 with PostgreSQL data types
...
The basic list of integer types includes 以下:

| Integer type | Length  | Applicable range                            | Comment                                                                                                                                           |
| ------------ | ------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `integer`    | 4 bytes | -2147483648 to 2147483647                   | This is the most common integer type 使用 as it represents a good trade off between storage and expressiveness for many practical applications. |
| `smallint`   | 2 bytes | -32768 to 32767                             | It is rare 使用 this except in places with tight storage constraints.                                                                           |
...
In addition to the standard integer types mentioned above, PostgreSQL includes a set of special integer types called _serial_ types. These types are primarily used 建立 unique identifiers, or [_primary keys_](/orm/more/help-and-troubleshooting/dataguide/database-glossary#primary-key), for records.

預設情況下, serial types will automatically use the next integer in an internally tracked sequence when new records are added. So if the last integer used in a serial column was 8559, 預設情況下, the next record will automatically use 8560. To guarantee that each value is unique, 您可以 further add a `UNIQUE` constraint on the column.
...
_Precision_ is the maximum amount of total digits that a number can have. In contrast, _scale_ is the number of digits to the right of the decimal point. By manipulating these numbers, 您可以 control how large the fractional and non-fractional components of a number are allowed to be.
...
例如, we can specify a column with a total precision of 5 and a scale of 2:

```
NUMERIC(5, 2)
```

This column would have 以下 behavior:
...
例如, to limit a floating point column to 8 digits of precision, 您可以 type:
...
Both floating point numbers provided by types like `float` and `double precision` and arbitrary precision numbers provided by the `numeric` type can be used to store decimal values. How do you know which one 使用?

The general rule is that if you need exactness in your calculations, the `numeric` type is always the better choice. The `numeric` type will store values exactly as they are provided, meaning that 結果s are entirely predictable when retrieving or computing over values. The `numeric` type is called arbitrary precision because you specify the amount of precision the type 需要 and it will store that exact amount of digits in the field.
...
So when would you use variable precision types? Variable precision types like `float` and `double precision` are well suited for scenarios where exact values are not necessary (例如, if they'll be rounded anyways) and when speed is highly valuable. Variable precision will generally offer performance benefits over the `numeric` type.
...
The alternative to fixed length character fields are variable length fields. For this, PostgreSQL provides the _`varchar`_ type. The `varchar` type stores characters with no fixed size. 預設情況下, with no integer given, `varchar` columns will accept strings of any length:

```
VARCHAR
```

By defining a `varchar` with a positive integer, 您可以 set a maximum string length:
...
- **true**: Represented by the SQL keyword `TRUE`. As input values, 以下 strings also evaluate to true: true, yes, on, and 1. 輸出 function represents true values with the string "t".
- **false**: Represented by the SQL keyword `FALSE`. As input values, 以下 strings also evaluate to false: false, no, off, and 0. 輸出 function represents false values with the string "f".
...
Care must be taken when working with the boolean `NULL`. While PostgreSQL can correctly interpret `TRUE` and `FALSE` as booleans, it cannot make that assumption for `NULL` due to its multiple uses. 您可以 explicitly cast `NULL` values to the `boolean` type in these situations to avoid this ambiguity:
...
To deal with ambiguous input, like 07/12/2019 (which could be interpreted as either July 12, 2019 or December 07, 2019 depending on format), 您可以 set the expected ordering using the [`DateStyle` parameter](https://www.postgresql.org/docs/current/runtime-config-client.html#GUC-DATESTYLE). This can be set to `DMY`, `MDY`, or `YMD` to define the expected ordering. 預設情況下, PostgreSQL will set it to `MDY` or use the `lc_time` locale to determine the appropriate ordering.
...
例如, to store time values with 3 decimal places of fractional seconds, you could define the time column like this:
...
As 您可以 see, 輸出 format doesn't have much of an affect on time representations as it does on dates. 主要 difference can be seen in the timestamp output that we'll see next.
...
When specifying both the fields to store and precision, the fields come first. Therefore, 建立 a column with 5 digits of precision that only stores the days, hours, minutes, and seconds of a given interval, you could type:

```
INTERVAL DAY TO SECOND (5)
```

請注意 您可以 only specify precision if your declared field ranges include the seconds value, since this is the only case where this argument matters.

There are a number of different ways to format input when adding values to an `interval` column. The most straightforward way is to specify the amount and the unit of each column or field you wish to provide, separated by a space. 例如：
...
The first uses 以下 unit abbreviations, represented by the bold component of 以下 fields:
...
Formats based on ISO 8601 begin with a **P** and then contain the interval string. So to represent an interval of 4 years, 2 months, 7 days, 3 hours, 27 minutes, and 8 seconds, 以下 string would work:
...
PostgreSQL can output values from `interval` columns in several formats. 輸出 style is determined by the `intervalstyle` setting, which can be one of 以下:
...
- **iso_8601**: This style produces output with ISO 8601's "format with designators" (the first ISO 8601 style described above). Replacing the pound signs with actual values, 輸出 would look like this: `P#Y#M#DT#H#M#S`

## Other useful types

Along with the types we covered with some depth above, there are additional types that are useful in specific scenarios. We'll cover these briefly to give you an idea of 如何使用 them and when they may be useful.
...
PostgreSQL enumerated types are user-defined types that have a set number of valid values. This functions similar to a drop down menu in that a choice can be made from a specific set of options. 例如, an `enum` type called `season` could be created with the values `winter`, `spring`, `summer`, and `autumn`.

使用 an `enum` type as a column, 您必須 first define it to declare its name and range of values. 您可以 create the `season` type we described above by typing:
...
Afterwards, 您可以 define a column to be of the `season` type as you would any other column when creating a table:
...
- **Composite types:** Composite types are types that are defined as a combination of two or more different types. 例如, you could create an `event` type that combines a geographical location and a timestamp to pinpoint a specific time and place.
- **Range types:** Range types include a valid range for a specified data type. While PostgreSQL includes [some range types](https://www.postgresql.org/docs/current/rangetypes.html#RANGETYPES-BUILTIN) 預設情況下, the `CREATE TYPE` command allows you 建立 your own.
- **Base types:** Base types are used to define a completely new type of data that isn't reliant on modifying existing types. To do this, you'll need to code up type functions in C to show PostgreSQL 如何 input, output, and process the data.
...
We've covered many of the most common data types used in PostgreSQL 在本指南中. While these provide a good starting point for data types in PostgreSQL, [additional types are available](https://www.postgresql.org/docs/current/datatype.html) to store other forms of data. Using the most appropriate types for your data allows you 使用 the database system to validate and operate on your data easily.

Understanding data types is essential when designing [schemas and tables](https://www.prisma.io/dataguide/postgresql/create-and-delete-databases-and-tables#create-tables-within-databases) in PostgreSQL. It also affects 如何 interact with the database from your applications, as the type system influences how data must be formatted and how it may be expressed when outputted. Learning about the options available within PostgreSQL and the side effects your choices might have is the best way to plan ahead when designing your data structures.

If you are using [Prisma Client](/orm/prisma-client) to work with your PostgreSQL databases, 您可以 find a mapping between some of the common PostgreSQL and Prisma types in [Prisma's PostgreSQL data connectors docs](/orm/overview/databases/postgresql#type-mapping-between-postgresql-and-prisma-schema).
