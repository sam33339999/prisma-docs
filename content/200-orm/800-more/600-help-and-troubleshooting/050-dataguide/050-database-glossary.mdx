---
title: 'Glossary of common 資料庫 terminology'
metaTitle: "資料庫 glossary | Prisma's 資料 指南"
metaDescription: '資料庫 terminology can be difficult to understand.  This glossary was designed to help you 學習 重要 terminology by providing definitions and context in one place.'
---

## Introduction

When dealing with databases, there is a lot of terminology that 您必須 learn in order to understand the technology, how best 使用 it, and how it relates to other parts of your environment. This glossary aims to collect common terminology used in the database community and provide definitions and context to help you grow your knowledge.

This glossary is a work in progress and a living document. We intend 更新 it 添加 new topics and refine the existing entries as time goes on. We have a backlog of terms we hope 添加 in the near future, but if you have anything you'd like us to talk about, please open a [GitHub issue](https://github.com/prisma/docs/issues/new?title=Glossary%20suggestion%3A%20) 添加 your suggestions.
...
2NF, or second normal form, describes a type of database normalization that:
  ...
  in whole, not just a portion of the candidate key. 例如, a `book` table
...
3NF, or third normal form, describes a type of database normalization that:
  1) satisfies the 需求 of 2NF,
  2) each non-key attribute is not
  transitively dependent on a key attribute. 例如, if a `user` table has
...
In computing, a bottleneck occurs when the performance or capacity of a system
  ...
  is most under contention. To solve bottlenecks, 您必須 first identify the
...
CAP theorem is a statement about distributed databases that states that any
  system can only provide at most two out of 以下 three qualities:
...
#### Cascade
In relational databases, cascade is an option for 如何 handle deletes or
...
A check constraint is perhaps the most flexible table or column constraint
  ...
  can be used to model many different types of 需求 to ensure that the
...
Columns are a component of table-oriented databases that label and potentially
  ...
  databases, the properties of a series of columns are one of 主要 ways
  of defining the properties of the table in general. Each row added to the
  table must provide values that conform to the 需求 associated with the
...
In the context of databases, committing data is the process whereby you
  ...
  system, but transactions, 例如, are one mechanism through which 您可以
...
A database cursor is a way for clients to iterate over records and query
  ...
  records by accessing 結果s as a queue. Care must be taken when using
...
Dark launching is a deployment and release strategy that helps organizations
  ...
  problems. When properly vetted, the application can be altered 使用 the new
...
Data independence is a term used to describe the separation of database
  ...
  changes (physical independence), 例如.
...
A data type is a category or attribute that expresses a constraint on valid
  values. 例如, an integer type specifies that only whole numbers are
  appropriate and expected for a variable or field. Data types allow you to
  specify expectations and 需求 about your data when defining a
...
A database management system, often called a DBMS or even just a "database",
  ...
  used to interact with the server 添加 or query data, change data structures,
...
Ephemeral storage, also sometimes called volatile or non-durable storage, is
  ...
  certain conditions. 例如, in applications, data being stored in memory
...
In the context of caches, eviction is a process where a piece of data is
  ...
  policies designed 移除 the data that is the oldest or least used.
...
The expand and contract pattern is a strategy for introducing new changes to a
  ...
  application logic 使用 both structures simultaneously. Eventually, after
...
Extract, transform, and load, often abbreviated as ETL, is a process of
  ...
  需求 and format of the new system. Finally, the reconstructed data is
...
A foreign key is a designated column or group of columns in a relational
  ...
  key in one table refers to a candidate key, typically 主要 key, in
...
A graph database is a NoSQL database that uses a graph structure to store and
  ...
  relationship. 例如, a node might describe a person with a property of
...
In relational databases, a join is an operation that connects two tables based
  ...
  column values 建立 an extended virtual row composed of the columns from
...
In the context of databases, a key is any attribute, column, or group of
  ...
  there is a main key (called 主要 key) designated as 主要 way to
  access rows.
...
A left join is a join operation for relational databases where all of the rows
  ...
  from each table. 結果s for a left join will contain the rows from both
...
Lexemes are language-level units of meaning that are relevant in natural
  ...
  other word lists to understand 如何 process them further.
...
Monolithic architecture is a term used to 參閱 a traditional application.
...
MySQL is one of the most popular relational database systems available today.
  ...
  history as powerful and easy 使用 relational system. It offers a wide array
...
NewSQL is a descriptor for a category of more recent relational database
  ...
  fairly loose categorization, it is generally used to 參閱 databases that
...
NoSQL databases, also sometimes called non-relational or not only SQL
  ...
  databases have long been available, the category generally is used to 參閱
...
#### Normalization
Database normalization is a process of structuring a database 移除 data
...
An ORM, or Object Relational Mapper, is a database tool designed to translate
  ...
  different models conflict on 如何 represent data.
...
Object relational impedance mismatch is a term used for the general tension
  ...
  term used to 參閱 many different types of problems that can occur within
...
An outer join is a type of relational database operation that joins two tables
  ...
  from each table. 結果s for an outer join will contain the rows from both
  tables where the column values matched and will additionally contain all of
...
A phantom read is a type of isolation anomaly that can occur within a
  ...
  due to changes made outside of the transaction. 例如 if you try to
...
In the context of search performance, precision is a measure of how relevant
  ...
  結果s that were returned. A query with a high level of precision does
  not retrieve many items that are not applicable to the query.

#### Primary key
A primary key is a type of database key that is designated as 主要 way to
  uniquely address a database row. While other keys may be able to pull
  individual rows, 主要 key is specifically marked for this purpose with
...
A right join is a join operation for relational databases where all of the
  ...
  columns from each table. 結果s for a right join will contain the rows
...
Role-based access control, also known as RBAC, is a security strategy that
  ...
  for various roles and making it simple 添加 or remove access 使用rs.
...
SQL, or Structured Query Language, is the most common database querying
  ...
  allows users 建立 queries to select, filter, define, and manipulate the
...
A serialization anomaly is a problem that can occur with concurrent
  ...
  impact 結果ing data. Serialization anomalies occur because operations in
...
A database shard is a segment of records stored by a database object that is
  ...
  reasons. 例如, a database table with 9 million records could be divided
  ...
  types of performance in very large datasets but it often 需要 making
  trade-offs that might degrade other types of performance (例如, on
...
A standard column family is a type of column family database object that
  ...
  to columns. Each row can define and use its own columns, so 結果ing
...
Stemming is a technique used in full-text search indexing where words with the
  ...
  例如, the words "cook", "cooked", and "cooks" might occupy a single
...
A storage engine is the underlying component in database management systems
  ...
  used according to the 需求 of your use case. Other systems, like
...
In relational databases, a table is a database structure that defines
  ...
  defined by a table's columns as well as additional table-level 需求
...
A table alias is a name given at query time for an existing or calculated table
  ...
  需要 a label to refer back to it in other parts of the query or for
  display.
...
Two-phase locking, sometimes abbreviated as 2PL, is a strategy for concurrency
  control to ensure that transactions are serializable. The two phases 參閱
...
An upsert is a database operation that either updates an existing entry or
  ...
  of a querying component that is used to search for matching records 更新
...
When talking about databases, a value is any piece of data that the database
  ...
  value beyond 什麼是 intrinsically there. The specific storage structure like
  the column or table may define 需求 about what types of values it
  stores.
...
In graph databases, vertices are entities that can hold properties and be
  ...
  define a relationship between them. 例如, an "author" vertex can be
...
In relational databases, a view is a table-like representation of a stored
  ...
  of the underlying data structure, they are derived from 結果s of their
  query. Views are useful for constructing more complex representations of data
  than exists in the underlying schema. 例如, a view might join a few
...
Volatile storage is any type of storage that is dependent on continual power
  to persist data. 例如, data stored in RAM is typically considered to be
...
Write-around caching is a caching pattern where write queries are sent to the
  ...
  需要 a way to invalidate the cache results for those items for subsequent
