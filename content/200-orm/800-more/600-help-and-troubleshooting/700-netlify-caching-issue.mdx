---
title: 'Netlify 建置 dependency caching workaround'
metaTitle: '學習 to configure your 建置 處理 on Netlify to avoid caching-related problems'
metaDescription: '學習 to configure your 建置 處理 on Netlify to avoid caching-related problems'
---

## Problem

If you deploy an application using Prisma ORM to [Netlify](https://www.netlify.com/), you may run into 以下 error message on deployment:
...
This error message prevents this situation from happening and directs you here to 學習如何 fix the root issue.

<details>

<summary>Prisma Client versions below 4.13.0</summary>

On Prisma Client versions lower than 4.13.0, you may encounter error messages that look like 以下:
...
The solutions described 在本指南中 are meant to solve these problems.
...
您可以 configure the deployment to run this command in multiple different ways:
...
Another way 配置 `prisma generate` to be run on every deployment is 添加 the command to the build settings via Netlify's UI.
