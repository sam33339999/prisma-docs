---
title: '使用 the Nuxt Prisma Module'
metaTitle: '添加 Prisma ORM Easily to Your Nuxt Apps'
metaDescription: '學習 如何 easily 添加 Prisma ORM to your Nuxt apps, use its 功能, and understand its 限制.'
...
4. 您可以 now use Prisma ORM in your project. If you accepted the prompt 添加 Prisma Studio, 您可以 access Prisma Studio through the Nuxt Devtools. 參見 [usage section](#usage) to 學習如何使用 Prisma Client in your app.

## Using a different database provider

The `@prisma/nuxt` module 適用於 any [database provider that Prisma ORM supports](/orm/reference/supported-databases). 您可以 configure the [getting started example](#getting-started) 使用 a database of your choice. The steps would be different for a [database without existing data](#using-a-database-without-existing-data) and a [database with pre-existing data](#using-a-database-with-pre-existing-data). 

### Using a database without existing data

配置 [the getting started example](#getting-started) 使用 a PostgreSQL database without any existing data:
...
4. Run the development server:
    ...
    npm run dev
    ```
    Starting the development server will prompt you to migrate the schema changes to the database, to which 您應該 agree. Then agree to the prompt 安裝 and access Prisma Studio from the Nuxt Devtools.
5. The `@prisma/nuxt` module is ready 使用 with your PostgreSQL database. 參見 [usage section](#usage) to 學習如何使用 Prisma Client in your app.

### Using a database with pre-existing data

配置 [the getting started example](#getting-started) 使用 a PostgreSQL database that already has data in it:
...
5. Starting the development server will skip the prompt to migrate the schema changes to the database, as the migrations folder already exists. Agree to the prompt 安裝 and access Prisma Studio from the Nuxt Devtools.
6. The `@prisma/nuxt` module is ready to be used with your PostgreSQL database. 參見 [usage section](#usage) to 學習如何使用 Prisma Client in your app.
...
If you're using [Nuxt server components](https://nuxt.com/docs/guide/directory-structure/components#server-components), 您可以 use the global instance of the Prisma Client in your `.server.vue` files:
```html
<script setup>
...
您可以 customize Prisma Client's capabilities by using client extensions in your `lib/prisma.ts` file. Here is an example using the [Accelerate client extension](https://www.npmjs.com/package/@prisma/extension-accelerate):
...
您可以 use the global instance of the Prisma Client in your Nuxt API route as follows:
```typescript
...
If you're using [Nuxt server components](https://nuxt.com/docs/guide/directory-structure/components#server-components), 您可以 use the global instance of the Prisma Client `.server.vue` files:
```html
<script setup>
  import prisma from '~/lib/prisma';
  const user = await prisma.user.findFirst()
...
您可以 configure the `@prisma/nuxt` module by using the `prisma` key in `nuxt.config.ts`:
...
<br />
:::note
  The `prisma` key is 可用於 `nuxt.config.ts` after successfully setting up the module by running `npm run dev`
:::

| Option              | Type      | Default | Description |
|---------------------|-----------|---------|-------------|
| **installCLI**      | `boolean` |  true   | Whether 安裝 the [Prisma CLI](/orm/tools/prisma-cli). |
| **installClient**   | `boolean` |  true   | Whether 安裝 the [Prisma Client](/orm/prisma-client) library in the project. |
| **generateClient**  | `boolean` |  true   | Whether to [generate](/orm/prisma-client/setup-and-configuration/generating-prisma-client) the `PrismaClient` instance. Executes `npx prisma generate` on every run 更新 the client based on the schema changes. |
...
| **installStudio**   | `boolean` |  true   | Whether 安裝 and start [Prisma Studio](https://www.prisma.io/studio) in the Nuxt Devtools. |
...
| **prismaRoot** | `string` |  false  | Required when using [Nuxt layers](https://nuxt.com/docs/getting-started/layers). 例如, if you have a Nuxt layer called `database`, the `prismaRoot` would be `./database` in the base nuxt config. This refers to the folder where Prisma will be initialized or checked. |
| **prismaSchemaPath** | `string` |  `undefined`  | Required when using [Nuxt layers](https://nuxt.com/docs/getting-started/layers). 例如, if you have a Nuxt layer called `database`, the `prismaSchemaPath` would be `./database/prisma/schema.prisma` in the base nuxt config. |
...
If you're encountering 以下 error when using `pnpm` and Prisma Studio isn't opening:
...
To resolve this issue, create a `.npmrc` file in your project root and add 以下 configuration to hoist Prisma dependencies:

```.npmrc file=.npmrc
hoist-pattern[]=*prisma*
```

這將 ensure that Prisma dependencies are properly resolved by `pnpm`.

### Resolving `TypeError: Failed to resolve module specifier ".prisma/client/index-browser"`

If you encounter 以下 error message in the browser console after building and previewing your application:

```
TypeError: Failed to resolve module specifier ".prisma/client/index-browser" 
```
To resolve this issue, add 以下 configuration to your nuxt.config.ts file:
...
The module is designed to work with popular package managers, including npm, Yarn, and pnpm. However, as of `v0.2`, it is not fully 相容於 Bun due to an issue causing an indefinite installation loop.
...
If you encounter 以下 message even if you have generated the client.
