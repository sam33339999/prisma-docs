---
title: 'Converting Implicit 多對多 關聯 to Explicit many-to-many relation'
metaTitle: '學習 如何 convert implicit 多對多 關聯 to explicit many-to-many relation'
metaDescription: '學習 如何 convert implicit 多對多 關聯 to explicit many-to-many relation'
---
...
Users sometimes encounter situations where they need to transition from implicit many-to-many relationships between models to explicit ones. Converting an implicit relation to explicit allows you to have more control over the relationship and store additional data specific to the relation, such as a timestamp or any other fields. This guide provides a step-by-step walkthrough on 如何 make that conversion.

## Solution

這將 guide you through the process of converting an implicit many-to-many relation to an explicit one in Prisma:
...
To convert the implicit relation to an explicit one, we need 建立 a [relation table](/orm/prisma-schema/data-model/relations/many-to-many-relations#relation-tables). The relation table will contain foreign keys referencing both tables involved in the many-to-many relation. In our example, we'll create a new model called `UserPost`. Our updated `schema.prisma` file would look like this:
...
If you are using Prisma Migrate, then 您可以 invoke this command:
...
To migrate the existing data from the implicit relation table to the new explicit relation table, you'll need to write a custom migration script. 您可以 use the Prisma Client to interact with the database, read data from the implicit relation table, and write it to the new relation table.

Considering the above `User` and `Post` models, here’s an example script 您可以 use to migrate data.
...
Once the data is migrated to the relation table, 您可以 remove the implicit relation columns ( `posts` in `User` model and `author` in `Post` model ) as shown below:
...
After making the change in schema file, 您可以 invoke this command:
