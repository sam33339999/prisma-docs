---
title: 'ChatGPT'
metaTitle: '使用 the Prisma MCP Server with ChatGPT'
metaDescription: '學習 如何 添加 the remote Prisma MCP Server to ChatGPT to 管理 your Prisma Postgres 資料庫.'
tocDepth: 3
toc: true
---
...
本指南說明如何 add the remote Prisma MCP server to ChatGPT, allowing you 建立 and manage your databases using natural language prompts.
...
If you notice that something has changed or doesn't match this guide, please [open an issue](https://github.com/prisma/docs/issues) or submit a pull request 更新 our docs.
:::
...
## 功能 of the Prisma MCP Server

By connecting the Prisma MCP server to ChatGPT, 您可以 perform a variety of database management tasks directly from the chat interface. Here are some of the key features available:
...
使用 the Prisma MCP server with ChatGPT, you need access to ChatGPT's Developer Mode. This feature is available on specific ChatGPT plans. For the most up-to-date information on supported plans, please 參閱 the [official OpenAI documentation](https://platform.openai.com/docs/guides/developer-mode).
...
4. A "New Connector" popup will open. Fill in the text fields as follows:
    - **Icon**: 您可以 download and use the [Prisma favicon](https://www.prisma.io/favicon.ico).
...
Once Developer Mode is enabled and the Prisma MCP server is added, 您可以 use it in your chats.

1. In ChatGPT, click the **+** icon to the left of the chat input box.
2. Click **More**.
3. Select the **Prisma MCP server** 添加 it to your chat as a connector.

Now 您可以 use natural language prompts to manage your database. 例如：
