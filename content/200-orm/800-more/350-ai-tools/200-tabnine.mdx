...
metaDescription: '學習 tips and 最佳實踐 for 使用 Prisma ORM with the Tabnine AI code editor.'
...
### 為什麼使用 Prisma ORM with Tabnine?

AI editors like Tabnine are powerful tools that can drastically improve developer productivity. It works especially well with 以下 workflows:
...
## Constructing and optimizing 資料庫 查詢
...
### 範例： fetching data with relationships
...
Most applications require basic CRUD operations or other boilerplate code that's usually derived from the structure of the database schema. 例如, an API layer may expose a set of HTTP endpoints that allow 建立, read, update or delete records of a certain model. Similarly, a frontend will data fetching and state management logic to interact with these API endpoints and update the UI accordingly.
...
### 範例： Generating CRUD functionality with Next.js
...
## Generating plain SQL 查詢 for use with TypedSQL
...
### 範例： Calculating the percentages of active, expired, and paused subscriptions  
...
Writing tests ensures that Prisma-based applications function correctly and behave according to 需求 and expectations. However, writing tests is a time-consuming activity that often follows predictable and repetitive patterns.
...
### 範例： Writing a unit test for a Prisma service  
...
### 範例： Refactoring a Prisma 查詢 after a 綱要 change  
...
The [Prisma VS Code extension](https://marketplace.visualstudio.com/items?itemName=Prisma.prisma) is a powerful tool for building applications with [Prisma Postgres](/postgres). If you are using Tabnine in an editor that allows you 安裝 the Prisma VS Code extension and you are using Prisma Postgres, 您應該 use it. The extension provides a dedicated UI for managing Prisma Postgres instances, both local and remote, making it easy to view, create, and delete instances, push local databases to the cloud, and visualize your schema.
