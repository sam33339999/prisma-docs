---
title: '升級 the Prisma ORM layer (MySQL)'
sidebar_label: '升級 the Prisma ORM layer'
metaTitle: '升級 the Prisma ORM layer to Prisma ORM 2 | MySQL'
metaDescription: '學習 如何 升級 the Prisma ORM layer to Prisma ORM 2 and 建立 your Prisma 綱要 with MySQL'
...
請注意 the _database name_ in the connection URL is typically composed of your _service name_ and _service stage_ (which are part of the `endpoint` in `prisma.yml`), separated by the `@` character.
...
To learn more about these inconsistencies 查看 the [Schema incompatibilities](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-mysql) page.
...
您可以 now run these SQL statements against your database either using a command line client or a GUI like TablePlus:
...
There are a few schema incompatibilities that were not yet resolved by the Upgrade CLI. At this point you still haven't fixed [scalar lists](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-mysql#scalar-lists-arrays-are-maintained-with-extra-table). 您可以 find 建議 workarounds for this and others on the [Schema incompatibilities](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-mysql) page.
