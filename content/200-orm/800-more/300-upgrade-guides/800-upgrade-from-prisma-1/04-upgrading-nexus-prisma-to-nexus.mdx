...
metaTitle: '升級 Prisma 1 with Nexus-prisma to @Nexus/綱要'
metaDescription: '學習 如何 升級 existing Prisma 1 projects with Nexus-prisma to Prisma ORM 2 and Nexus.'
---
...
This upgrade guide describes 如何 upgrade a project that's based on [Prisma 1](https://github.com/prisma/prisma1) and uses [`nexus`](https://www.npmjs.com/package/nexus) (< v0.12.0) or [`@nexus/schema`](https://github.com/graphql-nexus/nexus) together with [`nexus-prisma`](https://www.npmjs.com/package/nexus-prisma) (< v4.0.0) to implement a GraphQL server.
...
## 1. 升級 Nexus dependencies

開始使用, 您可以 remove the old Nexus and Prisma 1 dependencies:

```copy
npm uninstall nexus nexus-prisma prisma-client-lib prisma1
```

Then, 您可以 install the latest `@nexus/schema` dependency in your project:
...
The `nexus-plugin-prisma` dependency bundles all required Prisma ORM dependencies. 您應該 therefore remove the dependencies that you added installed when you upgraded the Prisma ORM layer of your app:

```copy
npm uninstall @prisma/cli @prisma/client
```

Note however that 您可以 still invoke the Prisma ORM 2 CLI with the familiar command:

```copy
npx prisma -v
```

> **Note**: If you see 輸出 of the Prisma 1 CLI when running `npx prisma -v`, be sure 刪除 your `node_modules` folder and re-run `npm install`.

## 2. Update the 配置 of Nexus and Prisma ORM

開始使用, 您可以 remove the old imports that are not needed any more with your new setup:
...
Instead, you now import 以下 into your application:
...
Next 您需要 adjust the code where you currently create your `GraphQLSchema`, most likely this is currently happening via the `makePrismaSchema` function in your code. Since this function was imported from the removed `nexus-prisma` package, you'll need to replace it with the `makeSchema` function from the `@nexus/schema` package. The way how the Prisma ORM plugin for Nexus is used also changes in the latest version.
...
If you previously typed the GraphQL `context` object that's passed through your resolver chain, 您需要 adjust the type like so:
...
Here's a quick overview of 主要 differences between the two approaches of creating GraphQL types with the latest versions of `@nexus/schema` and `nexus-plugin-prisma`.

- The `prismaObjectType` function is not available any more, all types are created with Nexus' `objectType` function.
- To expose Prisma models via Nexus, 您可以 use the `t.model` property which is added to the `t` argument that's passed into Nexus' `definition` functions. `t.model` gives you access to the properties of a Prisma model and lets you expose them.
...
#### Type definition with the latest version of `@Nexus/綱要` and the `Nexus-plugin-prisma`

With the latest version of `@nexus/schema`, 您可以 now access the `objectType` function on your main `schema` instance and expose all fields from the Prisma model like so:
...
請注意 `t.model` looks at the `name` attribute in the object that's passed as an argument to the `objectType` function and matches it against the models in your Prisma schema. In this case, it's matched against the `User` model. Therefore, `t.model` exposes functions that are named after the fields of the `User` model.
...
With the latest version of `@nexus/schema`, 您需要 expose all fields explicitly, there's no option to just expose everything from a Prisma model.
...
請注意 `t.model` looks at the `name` attribute and matches it against the models in your Prisma schema. In this case, it's matched against the `Post` model. Therefore, `t.model` exposes functions that are named after the fields of the `Post` model.
...
請注意 `t.model` looks at the `name` attribute and matches it against the models in your Prisma schema. In this case, it's matched against the `Profile` model. Therefore, `t.model` exposes functions that are named after the fields of the `Profile` model.
...
請注意 `t.model` looks at the `name` attribute and matches it against the models in your Prisma schema. In this case, it's matched against the `Category` model. Therefore, `t.model` exposes functions that are named after the fields of the `Category` model.

## 4. Migrate GraphQL operations

As a next step, 您可以 start migrating all the GraphQL _queries_ and _mutations_ from the "previous" GraphQL API to the new one.

For this guide, 以下 sample GraphQL operations will be used:
...
### 4.1. Migrate GraphQL 查詢

在本節中, you'll migrate all GraphQL _queries_ from the previous version of `nexus` and `nexus-prisma` to the latest version of `@nexus/schema` and the `nexus-plugin-prisma`.

#### 4.1.1. Migrate the `users` 查詢
...
To get the same behavior with the new Nexus, 您需要 call the `users` function on `t.crud`:
...
#### 4.1.2. Migrate the `posts(searchString: String): [Post!]!` 查詢
...
#### 4.1.3. Migrate the `user(uniqueInput: UserUniqueInput): User` 查詢
...
在本節中, you'll migrate the GraphQL mutations from the sample schema to the latest versions of `@nexus/schema` and the `nexus-plugin-prisma`.
...
To get the same behavior with the latest versions of `@nexus/schema` and the `nexus-plugin-prisma`, 您需要 call the `createOneUser` function on `t.crud` and pass an `alias` in order to rename the field in your GraphQL schema to `createUser` (otherwise it would be called `createOneUser`, after the function that's used):
...
#### 4.2.2. Migrate the `createDraft(title: String!, content: String, authorId: String!): Post!` 查詢
...
If you haven't already, 您可以 now uninstall dependencies that were related to the Prisma 1 setup:
