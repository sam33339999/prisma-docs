---
title: '升級 from MongoDB Beta'
metaTitle: '升級 from the Prisma 1 MongoDB Beta to Prisma ORM 2 or later'
metaDescription: '學習 如何 升級 your MongoDB application running Prisma 1 to Prisma ORM 2 or later.'
---

## 介紹

This guide helps you migrate from the Prisma 1 MongoDB Beta to MongoDB on Prisma ORM 2 or later. To learn more about the differences between Prisma 1 and Prisma ORM 2.x and later, 參閱 [this document](/orm/more/upgrade-guides/upgrade-from-prisma-1/how-to-upgrade#main-differences-between-prisma-1-and-prisma-orm-version-2x-and-later).
...
- Node.js: see [system 需求](/orm/reference/system-需求)
- TypeScript: see [system 需求](/orm/reference/system-需求)

## Installing Prisma ORM 3.12.0 or later

In your project directory run 以下 commands:
...
## Find the 連線 String to your MongoDB 資料庫

Next you'll want to find the connection string to your MongoDB database. 您應該 be able to find it in your `docker-compose.yml` file or on MongoDB Atlas. It's what you'd pass to MongoDB Compass. The connection string should look something like this:
...
And 您應該 see your Prisma schema in `prisma/schema.prisma` populated with your models.

:::info

If you see 以下 error: `Error in connector: SCRAM failure: Authentication failed.`, try adding `?authSource=admin` to the end of your connection string and trying again.

:::

## Touching up your Prisma 綱要

The generated Prisma Client from a freshly introspected Prisma 1 based MongoDB database may not have the best API. 您可以 adjust the model names and fields, just be sure to `@map` and `@@map` the original name to the underlying database collection and field names:
...
Take caution in doing these renames because 您需要 make sure the Prisma Schema still maps properly to the underlying database collections and field names.
...
Create a simple `test.ts` script to verify that Prisma Client can read and write to your application. 請注意 this guide is using the example in the [Prisma 1 examples repository](https://github.com/prisma/prisma1-examples/tree/master/typescript/docker-mongodb), but the code will change depending on your application.
...
您可以 then alter your `test.ts` to try writes:
...
And 您應該 see a user was created.
...
This means that your MongoDB database isn't running as a replica set. 參閱 [the link above](https://pris.ly/d/mongodb-replica-set) for steps to resolve this issue.

:::

## 升級 your Application

Now that you have a working Prisma Client, 您可以 start replacing Prisma Client 1 queries with the latest Prisma Client queries. The [Prisma Client Reference](/orm/reference/prisma-client-reference#filter-conditions-and-operators) is a helpful resource for learning 如何使用 the latest Prisma Client.
