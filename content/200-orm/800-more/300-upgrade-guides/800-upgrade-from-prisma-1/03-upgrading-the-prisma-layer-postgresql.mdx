---
title: '升級 the Prisma ORM layer (PostgreSQL)'
sidebar_label: '升級 the Prisma ORM layer'
metaTitle: '升級 the Prisma ORM layer to Prisma ORM 2 | PostgreSQL'
metaDescription: '學習 如何 升級 the Prisma ORM layer to Prisma ORM 2 and 建立 your Prisma 綱要 with PostgreSQL'
...
本頁說明 the first step of your upgrade process: Taking your Prisma 1 configuration and upgrading it to Prisma ORM 2. Concretely, you will 學習如何:

<details>
<summary>Questions answered in this page</summary>

- 什麼是 Prisma 1 → 2 schema incompatibilities?
- 如何 resolve Postgres-specific differences?
- 如何使用 the upgrade CLI safely?
...
Once done with these steps, 您可以 move on to the next guide that explains how 您可以 upgrade the application layer 使用 Prisma Client for your database queries.

> **Note**: During the upgrade process it can be helpful to get a graphical view on your database. It is therefore recommended 使用 a graphical database client to connect to your database, such as [TablePlus](https://tableplus.com/) or [Postico](https://eggerapps.at/postico/).
...
請注意 the former `prisma` command for Prisma 1 has been renamed to `prisma1`. 您可以 learn more about this [here](https://www.prisma.io/blog/prisma-2-beta-b7bcl0gd8d8e#renaming-the-prisma2-cli).

您可以 install the Prisma ORM 2 CLI in your Node.js project as follows (be sure to invoke this command in the directory where your `package.json` is located):

```terminal copy
npm install prisma --save-dev
```

> **Note**: With Prisma 1, it was usually recommended 安裝 the CLI globally. We now recommend to [install the Prisma CLI locally](/orm/tools/prisma-cli#installation) to prevent version conflicts.

您可以 now use the local installation of the `prisma` CLI by prefixing it with `npx`:

```terminal
npx prisma
```

If you're upgrading your entire project [all at once](/orm/more/upgrade-guides/upgrade-from-prisma-1/how-to-upgrade#upgrade-strategies), 您可以 now also uninstall the Prisma 1 CLI (otherwise expand below):
...
If you want to keep using the Prisma 1 CLI, it is recommend 移除 your global installation of it and add the `prisma1` CLI as a development dependency:
...
您可以 now invoke it as follows:

```terminal
npx prisma1
```

請注意 if you need a CLI version smaller than 1.34 (e.g. 1.30), 您可以 install it as follows:
...
Run 以下 command 建立 your Prisma schema (請注意 this throws an error if you already have a folder called `prisma`):

```terminal copy
npx prisma init
```

If you're seeing 以下 error, 您需要 rename your current `prisma` directory:
...
您可以 rename the current `prisma` directory to `prisma1` to make it clear that this holds the former Prisma 1 configuration:

```terminal copy
mv prisma prisma1
```

Now 您可以 run `init` and it will succeed:

```terminal copy
npx prisma init
```

It should print 以下 output:
...
- `prisma/schema.prisma`: Your Prisma schema that specifies the [data source](/orm/prisma-schema/overview/data-sources), [generator](/orm/prisma-schema/overview/generators) and [data model](/orm/prisma-schema/data-model/models) (請注意 the data model doesn't exist yet, it will be generated via introspection).
- `.env`: A [dotenv](https://github.com/motdotla/dotenv) file 配置 your database [connection URL](/orm/reference/connection-urls).
...
With Prisma 1, you specify which language variant of Prisma Client you wanted 使用 in your `prisma.yml`. With Prisma ORM 2, this information is now specified inside the Prisma schema via a `generator` block.
...
## 3. Determine your 連線 URL and connect to your 資料庫
...
First, 您需要 ensure that that `provider` field on the `datasource` block inside `schema.prisma` is configured 使用 the right database:
...
With the `provider` field set, 您可以 go ahead and configure the connection URL inside the `.env` file.
...
Based on these connection details, you need 配置 the `DATABASE_URL` environment variable inside your `.env` file as follows:

```bash file=.env
DATABASE_URL="postgresql://janedoe:randompassword@localhost:5432/prisma?schema=myproject$dev"
```

請注意 the `schema` argument is typically composed of your _service name_ and _service stage_ (which are part of the `endpoint` in `prisma.yml`), separated by the `$` character.
...
## 4. Introspect your 資料庫

For the purpose of this guide, we'll use 以下 Prisma 1 data model (select the **SQL** tab below to see what the data model maps to in SQL):
...
請注意 this data model has three [relations](/orm/prisma-schema/data-model/relations):
...
Now 您可以 run Prisma ORM's introspection against your database with 以下 command:
...
For the above Prisma 1 datamodel, this results in 以下 Prisma ORM 2 schema (請注意 the models have been reordered to match the initial order of the Prisma 1 datamodel):
...
While these inconsistencies don't actually impact the "feature set" you'll have 可用於 your Prisma Client API, they make you lose certain constraints/guarantees that were present before.

例如, Prisma ORM now won't guarantee that a `User` is connected to _at most_ one `Profile` because the relation between the tables was recognized as 1-n during introspection, so one `User` record _could_ now get connected to multiple `Profile` records.

Another issue is that 您可以 store whatever text for the `jsonData` and `role` fields, regardless of whether it's valid JSON or represents a value of the `Role` enum.

To learn more about these inconsistencies 查看 the [Schema incompatibilities](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-postgresql) page.

In 以下, we'll go through these incompatibilities and fix them one by one using the Prisma schema upgrade CLI.

## 5. Use the Prisma 綱要 升級 CLI to resolve 綱要不相容
...
During the first phase, it will generate and print a number of SQL statements that 您應該 run against your database to adjust the database schema. 您可以 either run all of the statements or a subset of them before continuing to the second phase.
...
請注意 **您可以 start over at any time during the process** and go back from the second to the first phase.

In this illustration, the green area shows the first phase, the blue area shows the second phase. 請注意 您可以 optionally run `prisma db pull` in between the phases 更新 your Prisma ORM data model:

![Fixing the schema incompatibilities](./images/fix-schema-incompatibilities.png)

使用 the Upgrade CLI, 您可以 either install it locally in your project, or invoke it once without installation using `npx` as done here:

```terminal copy
npx prisma-upgrade prisma1/prisma.yml prisma/schema.prisma
```

The CLI will greet you with 以下 message:
...
Once you confirmed, the CLI outputs the SQL statements 您應該 be running against your database:
...
> **Note**: If you're seeing the note about breaking changes, 您可以 ignore it for now. We'll discuss it later.
...
As a next step, 您可以 start sending the SQL statements to your database. 請注意 all of these changes are non-breaking and you'll be able to continue using Prisma 1 side-by-side with Prisma ORM 2.

The next sections cover the different kinds of SQL statements to be sent to your database individually.

### 5.1. Fix the 資料庫 綱要 via plain SQL (non-breaking)

在本節中, we'll walk through the printed SQL statements and run them against the database one by one.
...
The CLI currently shows 以下 output:
...
![Altering columns 使用 ENUM with SQL](./images/altering-columns-to-use-enum.png)

### 5.1.2. Add missing `DEFAULT` constraints to the 資料庫
...
您可以 now run these SQL statements against your database either using a command line client or a GUI like Postico:
...
If there are no breaking changes detected, 您可以 [skip forward to section 5.2](#52-re-introspect-your-database-to-update-your-prisma-schema)

Depending on your [upgrade strategy](/orm/more/upgrade-guides/upgrade-from-prisma-1/how-to-upgrade#upgrade-strategies), 您可以 either perform these changes now or skip to the next phase of the Upgrade CLI:

- If you are following the gradual side-by-side upgrade strategy, do not perform these changes yet since they will break your Prisma 1 setup. In that case, 您可以 continue to the next phase of the Upgrade CLI by typing <kbd>n</kbd> and hitting <kbd>RETURN</kbd>.
- If you are following the all at once upgrade strategy, 您可以 perform these changes now. In that case, continue by typing <kbd>Y</kbd> and hitting <kbd>RETURN</kbd>.

### 5.2. Fix the 資料庫 綱要 via plain SQL (breaking)

在本節中, you'll resolve the schema incompatibilities that are breaking your Prisma 1 setup. Do not perform these changes if you are still running Prisma 1 in your project!
...
To fix this, you'll need to perform 以下 migration:
...
### 5.2. Re-introspect your 資料庫 更新 your Prisma 綱要

At this point, you've resolved the schema incompatibilities with the Upgrade CLI. 您可以 now exit the Upgrade CLI for now by typing <kbd>n</kbd> and hitting <kbd>RETURN</kbd>.

在本節中, you'll update your Prisma schema with another introspection round. This time, the previous flaws of the Prisma schema will be resolved because the database schema has been adjusted:

```terminal copy
npx prisma db pull
```

This time, 結果ing Prisma schema looks as follows:
...
This schema has most issues resolved, but it still lacks 以下:

### 5.2. Add missing attributes to the Prisma 2 綱要 and other 綱要 fixes

The CLI now prints 以下:
...
At this point, you either ran all the SQL statement that were printed by the CLI or you skipped some of them. Either way, 您可以 now move on the last step and let the Upgrade CLI add the missing Prisma ORM 2 attributes. Typically these are 以下:
...
To apply these changes, 您可以 re-run the Upgrade CLI:

```terminal copy
npx prisma-upgrade prisma1/prisma.yml prisma/schema.prisma
```

If you did not resolve all schema incompatibilities, the Upgrade CLI now prints the remaining SQL statements (as well as the ones for migrating IDs). 您可以 just ignore them at this point and continue to the last step by continuously typing <kbd>Y</kbd> and hitting <kbd>RETURN</kbd> when prompted.

If you did resolve all schema incompatibilities, no SQL statements will be printed and the Upgrade CLI only outputs 以下:
...
This is not ideal and 您可以 in fact manually rename all of them to their previous versions!

Because all relation fields are _virtual_, meaning they don't _manifest_ in the database, 您可以 name them whatever you like. In this case, all relation fields are lowercased and sometimes pluralized.
...
> **Note**: For the 1-1-relation between `User` and `Profile` it was not possible to set the old name `user` for the relation field. This is because there'd be a naming conflict with the already existing [relation scalar](/orm/prisma-schema/data-model/relations#annotated-relation-fields) field that holds the foreign key. In that case, 您可以 choose a different name or alternatively rename the foreign key column directly in the database via SQL.

### 5.5. Resolving remaining 綱要不相容

There are a few schema incompatibilities that were not yet resolved by the Upgrade CLI. At this point you still haven't fixed [scalar lists](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-postgresql#scalar-lists-arrays-are-maintained-with-extra-table). 您可以 find 建議 workarounds for this and others on the [Schema incompatibilities](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-postgresql) page.

## 6. Install and generate Prisma Client

Now that you have your Prisma ORM 2 schema ready, 您可以 install Prisma Client with 以下 command:
...
Congratulations, you have now upgraded your Prisma ORM layer to Prisma ORM 2! From here on, 您可以 move on 更新 your application code using one of 以下 guides:
...
This section contains a high-level and side-by-side comparison of the Prisma Client APIs of Prisma 1 and Prisma ORM 2. For more details about the new Prisma Client API, 您可以 explore the [Prisma Client](/orm/prisma-client) docs.
...
> **Note**: 您可以 learn more about the new pagination API in the respective [release notes](https://github.com/prisma/prisma/releases/tag/2.0.0-beta.7) or the [Pagination](/orm/prisma-client/queries/pagination) page in the docs.
...
Another benefit of this approach is that 您可以 use `select` and `include` on _any_ Prisma Client query, e.g. `findUnique()`, `findMany`, `create`, `update`, `delete`, ...
...
As an example, creating a new record and only retrieving the `id` in the returned object was not possible in Prisma 1. With Prisma ORM 2 您可以 achieve this as follows:
