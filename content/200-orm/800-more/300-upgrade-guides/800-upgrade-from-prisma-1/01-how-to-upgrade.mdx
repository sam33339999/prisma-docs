---
title: '如何 upgrade'
metaTitle: '如何 upgrade from Prisma 1 to Prisma ORM version 2.x and later'
metaDescription: '學習 如何 upgrade your Prisma 1 project to Prisma ORM version 2.x and later'
---

## Overview

This page helps you make an informed decision on when and 如何 upgrade from Prisma 1 to Prisma ORM version 2._x_ and later.

## Upgrade documentation

The upgrade documentation consists of several pages, here's an overview of 如何使用 them:

- **如何 upgrade** (_you are here_): Starting point to 學習關於 the upgrade process in general.
...
- [Upgrading the Prisma layer](/orm/more/upgrade-guides/upgrade-from-prisma-1/upgrading-the-prisma-layer-postgresql): No matter what your Prisma 1 setup looks like, 您應該 **always start your upgrade process by following this guide**.

Once you're done with that guide, 您可以 choose **one of 以下 four guides to upgrade your application layer**:
...
- **Real-time API (Subscriptions)**: Prisma ORM version 2._x_ and later currently [doesn't have a way to subscribe to events happening in the database](https://github.com/prisma/prisma/issues/298) and get notified in real time. It is currently unclear if, when, and in what form a real-time API will be added to Prisma ORM versions 2._x_ and later. For the time being, 您可以 implement real-time functionality using native database triggers, or if you're using GraphQL subscriptions 您可以 consider triggering subscriptions manually inside your _mutation resolvers_.

## Schema incompatibilities

The database schema that is created when running `prisma deploy` in Prisma 1 is only partially 相容於 the one that Prisma ORM versions 2._x_ and later creates. This section gives a quick overview of the general incompatibilities and the potential workarounds. -

> **Note**: For a detailed explanation of the problems and respective workarounds, please 參閱 the [Schema incompatibilities](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-postgresql) page.
...
The [Prisma 1 Upgrade CLI](https://github.com/prisma/prisma1-upgrade) helps you apply the workarounds that are explained on the [Schema incompatibilities](/orm/more/upgrade-guides/upgrade-from-prisma-1/schema-incompatibilities-postgresql) page. It generates the SQL statements to fix the database schema and make it 相容於 Prisma ORM versions 2._x_ and later. 請注意 you are in full control over the operations that are executed against your database, the Upgrade CLI only generates and prints the statements for you. The Upgrade CLI also takes care of the workarounds in the Prisma schema.
...
請注意 the Upgrade CLI is designed in a way that **您可以 stop and re-start the process at any time**. Once you ran a SQL command that was generated by the Upgrade CLI against your database, the SQL command will not show up the next time you invoke the Upgrade CLI. That way, 您可以 gradually resolve all schema incompatibilities when it's convenient for you.
...
請注意 if you are planning to run Prisma 1 and Prisma ORM 2._x_ or later version side-by-side, 您必須 not yet resolve the [schema compatibilities](#schema-incompatibilities) that are breaking the Prisma 1 setup.
