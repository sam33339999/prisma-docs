...
metaTitle: '升級 from Prisma 1 with prisma-binding to SDL-first'
metaDescription: '學習 如何 升級 existing Prisma 1 projects with prisma-binding to Prisma ORM 2 (SDL-first).'
---
...
The code will keep the [SDL-first approach](https://www.prisma.io/blog/the-problems-of-schema-first-graphql-development-x1mn4cb0tyl3) for constructing the GraphQL schema. When migrating from `prisma-binding` to Prisma Client, 主要 difference is that the `info` object can't be used to resolve relations automatically any more, instead you'll need to implement your _type resolvers_ to ensure that relations get resolved properly.
...
Alternatively, 您可以 use the `get-schema` command of the [GraphQL CLI](https://github.com/Urigo/graphql-cli) to download your full schema:

```
npx graphql get-schema --endpoint __GRAPHQL_YOGA_ENDPOINT__ --output schema.graphql --no-all
```

> **Note**: With the above command, 您需要 replace the `__GRAPHQL_YOGA_ENDPOINT__` placeholder with the actual endpoint of your GraphQL Yoga server.

Once you obtained the `schema.graphql` file, replace your current version in `src/schema.graphql` with the new contents. 請注意 the two schemas are 100% equivalent, except that the new one doesn't use [`graphql-import`](https://github.com/ardatan/graphql-import) for importing types from a different file. Instead, it spells out all types in a single file.

Here's a comparison of these two versions of the sample GraphQL schema that we'll migrate 在本指南中 (您可以 use the tabs to switch between the two versions):
...
`PrismaClient` is your new interface to the database in Prisma ORM 2. It lets you invoke various methods which build SQL queries and send them to the database, returning 結果s as plain JavaScript objects.
...
In the code block above, the _red_ lines are the lines to be removed from your current setup, the _green_ lines are the ones that 您應該 add. Of course, it's possible that your previous setup differed from this one (e.g. it's unlikely that your Prisma ORM `endpoint` was `http://localhost:4466` if you're running your API in production), this is just a sample to indicate what it _could_ look like.

When you're now accessing `context.prisma` inside of a resolver, you now have access to Prisma Client queries.

## 2. Write your GraphQL type 解析器

`prisma-binding` was able to _magically_ resolve relations in your GraphQL schema. When not using `prisma-binding` though, 您需要 explicitly resolve your relations using so-called _type resolvers_.

> **Note** 您可以 learn more about the concept of type resolvers and why they're necessary in this article: [GraphQL Server Basics: GraphQL Schemas, TypeDefs & Resolvers Explained](https://www.prisma.io/blog/graphql-server-basics-the-schema-ac5e2950214e)
...
您可以 do so by adding a `User` field to your _resolver map_ and implement the resolvers for the `posts` and `profile` relations as follows:
...
您可以 do so by adding a `Post` field to your _resolver map_ and implement the resolvers for the `author` and `categories` relations as follows:
...
您可以 do so by adding a `Profile` field to your _resolver map_ and implement the resolvers for the `owner` relation as follows:
...
您可以 do so by adding a `Category` field to your _resolver map_ and implement the resolvers for the `posts` and `profile` relations as follows:
...
With all your type resolvers in place, 您可以 start migrating the actual GraphQL API operations.

## 3. Migrate GraphQL operations

### 3.1. Migrate GraphQL 查詢

在本節中, you'll migrate all GraphQL _queries_ from `prisma-binding` to Prisma Client.
...
請注意 this approach does **not** work yet because the _structures_ of the incoming arguments is different from the ones expected by `PrismaClient`. To ensure the structures are compatible, 您可以 use the `@prisma/binding-argument-transform` npm package which ensures compatibility:

```copy
npm install @prisma/binding-argument-transform
```

您可以 now use this package as follows:
...
Here is how 您可以 adjust the call to make it compliant with the new Prisma Client pagination API:
...
您可以 now send the respective query in the GraphQL Playground:
...
#### 3.1.3. Migrate the `user(uniqueInput: UserUniqueInput): User` 查詢
...
您可以 now send the respective query via the GraphQL Playground:
...
在本節中, you'll migrate the GraphQL mutations from the sample schema.
...
#### 3.1.3. Migrate the `createDraft(title: String!, content: String, authorId: String!): Post!` 查詢
...
您可以 now send the respective mutation via the GraphQL Playground:
...
Since the entire app has now been upgraded to Prisma ORM 2, 您可以 delete all unnecessary files and remove the no longer needed dependencies.
