---
title: '升級 to Prisma ORM 6'
metaTitle: '升級 to Prisma ORM 6'
metaDescription: '指南s on 如何 升級 to Prisma ORM 6'
tocDepth: 3
toc: true
---

Prisma ORM v6 introduces a number of **breaking changes** when you upgrade from an earlier Prisma ORM version. 本指南說明 how this upgrade might affect your application and gives instructions on 如何 handle any changes.
...
## 升級 the `prisma` and `@prisma/client` packages to v6

To upgrade to Prisma ORM v6 from an earlier version, 您需要 update both the `prisma` and `@prisma/client` packages:
...
### 綱要 change for implicit m-n relations on PostgreSQL
...
Previous versions of Prisma ORM used 建立 a _unique index_ on these two columns. In Prisma v6, this unique index is changing to a _primary key_ in order to [simplify for the default replica identity behaviour](https://github.com/prisma/prisma/issues/25196).

<details>
<summary>Expand for an example</summary>

As an example, consider 以下 Prisma schema with an implicit m-n relation between `Post` and `Tag` models:
...
In this case, Prisma ORM maintains 以下 relation table for you under the hood:
...
The [`fullTextSearch`](/orm/prisma-client/queries/full-text-search) Preview feature is promoted to General Availability only for MySQL. This means that if you're using PostgreSQL and currently make use of this Preview feature, you now need 使用 the new [`fullTextSearchPostgres`](/orm/prisma-client/queries/full-text-search#enabling-full-text-search-for-postgresql) Preview feature:
...
您可以 directly use the `Buffer` instance as a `Uint8Array`:
...
您可以 create a `Buffer` from a `Uint8Array` using `Buffer.from`:
...
### New keywords that can't be used as 模型 names: `async`, `await`, `using`

With this release, 您可以't use `async`, `await` and `using` as model names any more.


## 預覽 功能 promoted to General Availability
...
If you use the [full-text index](/orm/prisma-schema/data-model/indexes#full-text-indexes-mysql-and-mongodb) feature in your app, 您可以 now remove `fullTextIndex` from the `previewFeatures` in your Prisma schema:
...
If you use the [full-text search](/orm/prisma-client/queries/full-text-search) feature with **MySQL** in your app, 您可以 now remove `fullTextSearch` from the `previewFeatures` in your Prisma schema:
...
If you are using it with **PostgreSQL**, 您需要 update the name of the feature flag to `fullTextSearchPostgres`:
