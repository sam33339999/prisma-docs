---
title: '升級 to Prisma ORM 5'
metaTitle: '升級 to Prisma ORM 5'
metaDescription: '指南s on 如何 升級 to Prisma ORM 5'
tocDepth: 3
toc: true
---

Prisma ORM 5.0.0 introduces a number of changes, including the usage of our new JSON Protocol, [which make Prisma Client faster 預設情況下](https://www.prisma.io/blog/prisma-5-f66prwkjx72s). A full list of these changes can be found [in our release notes](https://github.com/prisma/prisma/releases/tag/5.0.0).

本指南說明 how upgrading might affect your application and gives instructions on 如何 handle breaking changes within Prisma ORM 5.

## 升級 the `prisma` and `@prisma/client` packages to version 5

To upgrade to Prisma ORM 5 from an earlier version, 您需要 update both the `prisma` and `@prisma/client` packages.
...
Prisma ORM 5 includes some minimum version changes for Node.js, TypeScript, and PostgreSQL. 使用 Prisma version 5.0.0 and up, you will need to have at least the minimum versions below:
...
Node.js v16.x is reaching [end-of-life on 11 September 2023](https://nodejs.org/en/blog/announcements/nodejs16-eol) in order to coincide with the end-of-life of OpenSSL 1.1.1. For that reason, we recommend upgrading to the current Node.js LTS, v18.x. Please 請注意 Prisma ORM 5 will be the last major version of Prisma ORM to support Node.js v16.
...
This section gives an overview of 主要 breaking changes in Prisma ORM 5.
...
Please review our [jsonProtocol changes guide](/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-5/jsonprotocol-changes) to 學習如何 update your app to account for the new protocol in Prisma ORM 5. You will need to:
...
Prisma ORM 5 drops support for a number of "array shortcuts". These shortcuts were a way 添加 a single element as a value to an array-based operator instead of wrapping that one element in an array. To make our typings more consistent and logical and to conform to the new JSON Protocol, we now require array values for these operators.
...
### `CockroachDB` provider is now required when 連線到 a CockroachDB 資料庫
...
4. Review changes between your Prisma schema backup and the new Prisma schema generated by `db pull`. 您可以 either use the new schema as is, or update it to include your preferred spacing, comments, etc.
...
Importing from `@prisma/client/runtime` is no longer 可用於 Prisma ORM 5. If you were using public APIs 可用於 this namespace before, 您可以 instead import `Prisma` and access them. 例如：
...
We highly discourage the use of internal private APIs as they can change without warning and are not guaranteed to be supported. If your usage 需要 a private API that was previous available [please reach out to us on GitHub.](https://github.com/prisma/prisma/discussions/new?category=q-a)

### Generated type changes

#### Changes to `RelationFilterInput` to account for nullability

Prior to Prisma ORM 5, there was a long-standing bug that caused nullable reverse relations to not be marked as nullable in our generated types. 例如, take 以下 schema:
...
If you import generated types in your code, you will need 更新 instances like this to utilize the new `Nullable` types.

#### Changes to `UncheckedUpdateManyInput` to avoid name collisions

In certain instances it was possible for name collisions to occur when one model had two foreign keys to two other models that had the same property name for the reverse relation. As an example, 以下 schema:
...
If you import generated types in your code, you will need 更新 instances like this to the corrected types.

## Other changes

以下 changes may cause an application to initially throw an error message after upgrading to Prisma ORM 5. Fortunately, they are easy to solve, as the underlying functionality has been removed for a while or the change is a simple string replace.

### Removal of 已棄用 Prisma CLI flags
...
If you're using the `beforeExit` hook in NestJS, 您可以 upgrade to Prisma ORM 5 by removing the custom `enableShutdownHooks` method in your service:
...
Instead, use the built-in `enableShutdownHooks` method in NestJS if 您需要 handle lifecycle events:
...
### Removal of 已棄用 `prisma2` executable
...
### Removal of 已棄用 `experimentalFeatures` property
...
In Prisma ORM 5, you will need 更新 references of `experimentalFeatures` to `previewFeatures` manually or use the new code action in the Prisma VSCode extension.

### `遷移-engine` renamed to `綱要-engine`

The engine responsible for commands like `prisma migrate` and `prisma db` has been renamed from `migration-engine` to `schema-engine` to better describe its use. For many users, no changes will be required. However, if 您需要 explicitly include or exclude this engine file, or 參閱 the engine name for any other reason, you will need 更新 your code references.

#### Example with the 無伺服器 Framework

One example we have seen is projects using the Serverless Framework. In these instances, you will need 更新 any patterns that reference `migration-engine` to instead reference `schema-engine`.
