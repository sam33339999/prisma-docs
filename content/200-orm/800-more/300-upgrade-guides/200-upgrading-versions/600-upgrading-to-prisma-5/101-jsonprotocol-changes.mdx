...
metaTitle: '升級 to jsonProtocol in Prisma ORM 5'
...
### Removal of `jsonProtocol` 預覽 Feature
...
Due to the switch to the new protocol, several error messages have been improved. 例如, 以下 error message in Prisma ORM 4 and below:
...
Several array shortcuts were removed as a part of this major update. These shortcuts were a way 添加 a single element as a value to an array-based operator.

#### `OR` operators

以下 code in Prisma ORM 4 and lower:
...
Will need to be changed to 以下 in Prisma ORM 5:
...
If your `in` and `notIn` values are only one element, 您可以 also update your code to not use these operators at all:
...
注意： This `path` argument change only affects PostgreSQL databases. MySQL databases are not affected as they use a different syntax.
