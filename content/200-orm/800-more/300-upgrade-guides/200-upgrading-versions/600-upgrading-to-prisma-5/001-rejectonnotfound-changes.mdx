...
metaTitle: '如何 處理 removal of rejectOnNotFound in Prisma ORM 5'
metaDescription: 'Sub-指南 explaining 如何 更新 your code due to the removal of rejectOnNotFound in Prisma ORM 5'
tocDepth: 2
toc: true
---
...
## Replacing `rejectOnNotFound` enabled at the 查詢 level

If you previously enabled `rejectOnNotFound` on a per-query basis, you will need to replace your usage at the _query level_. 您可以 use our `*OrThrow` query variants, `findFirstOrThrow` or `findUniqueOrThrow` instead of supplying the parameter to `findFirst` and `findUnique()`.
...
If you use a custom error handler like 以下:
...
If your error handler is used in multiple places, 您可以 also create a reusable error adapter which could then be used within a `.catch()` called on your function.
...
If you previously enabled `rejectOnNotFound` globally via configuration in the Prisma Client constructor, like in these 範例：
...
You will need 更新 your codebase 使用 `findUniqueOrThrow` and `findFirstOrThrow` instead of `findUnique()` and `findFirst`, depending on which calls you would like to throw.

### `rejectOnNotFound` via Prisma Client Constructor with custom error handler

If instead you use a custom error handler with the `rejectOnNotFound` property, like these 範例：
...
You will need 更新 your method usage to `...OrThrow` and then use a [Client Extension](/orm/prisma-client/client-extensions) in order to get the same behavior.

As an example, 以下 extension would give the same behavior in Prisma ORM 5 that `Example 5` gave in Prisma ORM 4 and lower.
