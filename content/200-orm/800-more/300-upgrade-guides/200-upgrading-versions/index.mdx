---
title: '升級 versions'
metaTitle: '升級 versions (指南s)'
metaDescription: '升級 your project to the latest version of Prisma ORM.'
---
...
1. Upgrade **both** of 以下 packages to the same version:
...
## Testing new 功能, without 升級

### `dev` distribution tag

The `dev` [npm distribution tag](https://docs.npmjs.com/adding-dist-tags-to-packages/) points to the most recent development version of the package, which is published for each commit to 主要 branch of `prisma/prisma`. 您可以 use the `dev` distribution tag to verify a fix or test a feature before it is officially released.

安裝 the latest `dev` distribution tag:
...
Do not use the `dev` distribution tag in production - wait until the official release that contains the features and fixes you are interested in is released. 例如, fixes present `@prisma/client@2.23.0-dev.25` will eventually be released as part of `@prisma/client@2.23.0`.
