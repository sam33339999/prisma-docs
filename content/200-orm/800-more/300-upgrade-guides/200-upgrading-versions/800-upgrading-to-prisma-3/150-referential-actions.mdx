---
title: 'Referential actions 升級 path'
metaTitle: 'Referential actions 升級 path'
metaDescription: '指南s on 如何 deal with referential actions 使用 Prisma Introspect or Prisma Migrate when 升級 to Prisma 3'
tocDepth: 4
...
If 您需要 actually use the cascade behavior configured in the database, you _can_ use [`raw`](/orm/prisma-client/using-raw-sql/raw-queries) SQL queries to [delete multiple referenced records](/orm/prisma-client/queries/crud#deleting-all-data-with-raw-sql--truncate). This is because Prisma Client will **not** perform runtime checks on raw queries.
...
On top of the database referential actions, 以下 actions are enforced in Prisma Client versions 2.x:
...
There are a couple of paths 您可以 take when upgrading which will give different results depending on the desired outcome.

If you currently use the migration workflow, 您可以 run `prisma db pull` to check how the defaults are reflected in your schema. 您可以 then manually update your database if 您需要.

您可以 also decide to skip checking the defaults and run a migration 更新 your database with the new default values.
...
After introspecting, 您可以 review the non-default clauses in your schema. 最重要 clause to review is `onDelete`, which defaults to `Cascade` in version 2.25.0 and earlier.
...
以下範例 would result in a cascading delete, meaning that if the `User` is deleted then all of their `Post`'s will be deleted too.
...
**After** upgrading, Prisma ORM no longer performs runtime checks. 您可以 instead specify a custom referential action to preserve the referential integrity between relations.

When you use [`NoAction`](/orm/prisma-schema/data-model/relations/referential-actions#noaction) or [`Restrict`](/orm/prisma-schema/data-model/relations/referential-actions#restrict) to prevent the deletion of records, the error messages will be different in versions 3.0.1 and above (or 2.26.0 with the `referentialActions` feature flag enabled) compared to versions prior to that. This is because they are now triggered by the database and **not** Prisma Client. The new error code that can be expected is `P2003`, so 您應該 check your code to make adjustments accordingly.

#### Example of catching errors

以下範例 uses the below blog schema with a 1-m relationship between `Post` and `User` and sets a [`Restrict`](/orm/prisma-schema/data-model/relations/referential-actions#restrict) referential actions on the `author` field.
...
Prior to upgrading, the error code you would receive when trying 刪除 a user which has posts would be `P2014` and it's message:
