---
title: '升級 to Prisma ORM 3'
metaTitle: '升級 to Prisma ORM 3'
metaDescription: '指南s on 如何 升級 to Prisma ORM 3'
tocDepth: 3
toc: true
---
...
Below you will find a list of the breaking changes and 如何 handle them.

</TopBlock>

## 破壞性變更

### [Referential actions](/orm/prisma-綱要/data-模型/relations/referential-actions)
...
As a result, depending on which workflow you are using to work on your application, you could be impacted. We advise you to check your schema and decide if 您需要 define referential actions explicitly.

See [Referential action upgrade path](/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-3/referential-actions) to understand 如何 proceed.

### [Named constraints](/orm/prisma-綱要/data-模型/資料庫-mapping)
...
您可以 查看 the [Named constraints upgrade path](/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-3/named-constraints) 欲了解更多資訊 on 如何 proceed.

### [$queryRaw](/orm/prisma-client/using-raw-sql/raw-查詢)

From version 3.x onwards, the `$queryRaw` method now only supports a template literal.

This means that if your application relied on `$queryRaw` calls using _strings_, those calls will **not** work anymore. We advise you 使用 template literals wherever possible for security reasons or resort to `$queryRawUnsafe` otherwise, after carefully escaping queries to prevent SQL injections.

您可以 learn more about the new `$queryRaw` and `$queryRawUnsafe` methods in the [Raw database access](/orm/prisma-client/using-raw-sql/raw-queries) section of the docs.

### [Json Null Equality](/orm/prisma-client/special-fields-and-types/working-with-json-fields#filtering-by-null-values)

您可以not filter a `Json` field by a null value. [See this GitHub issue](https://github.com/prisma/prisma/issues/8399).
...
Given 以下 model in your Prisma Schema:
...
## 升級 the `prisma` and `@prisma/client` packages to Prisma ORM 3

To upgrade from version 2.x to 3.x, 您需要 update both the `prisma` and `@prisma/client` packages. Both the `prisma` and `@prisma/client` packages install with a caret `^` in their version number to safe guard against breaking changes.

To ignore the caret `^` and upgrade across major versions, 您可以 use the `@3` tag when upgrading with `npm`, or `yarn` .
