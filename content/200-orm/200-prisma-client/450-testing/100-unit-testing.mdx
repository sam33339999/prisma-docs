---
title: 'Unit 測試'
metaTitle: 'Unit 測試 with Prisma ORM'
metaDescription: '學習如何 to 設定 and 執行 unit tests with Prisma 客戶端'
tocDepth: 3
---
...
This means that 您應該 aim 移除 as many dependencies as possible, such as external services and databases, to keep the tests and their environments as lightweight as possible.
...
To ensure your unit tests are isolated from external factors 您可以 mock Prisma Client, this means you get the benefits of being able 使用 your schema (**_type-safety_**), without having to make actual calls to your database when your tests are run.
...
以下 steps guide you through mocking Prisma Client using a singleton pattern.

1. Create a file at your projects root called `client.ts` and add 以下 code. 這將 instantiate a Prisma Client instance.
...
2. Next create a file named `singleton.ts` at your projects root and add 以下:
...
The singleton file tells Jest to mock a default export (the Prisma Client instance in `./client.ts`), and uses the `mockDeep` method from `jest-mock-extended` 啟用 access to the objects and methods available on Prisma Client. It then resets the mocked instance before each test is run.
...
1. Create a `context.ts` file and add 以下:
...
2. 使用 the context, you would do 以下 in your test file:
...
這將 create a new context before each test is run via the `createMockContext` function. This (`mockCtx`) context will be used to make a mock call to Prisma Client and run a query to test. The `ctx` context will be used to run a scenario query that is tested against.
...
All of the examples that follow use 以下 schema model:
...
以下 unit tests will mock the process of

- Creating a new user
- Updating a users name
- Failing 建立 a user if terms are not accepted
