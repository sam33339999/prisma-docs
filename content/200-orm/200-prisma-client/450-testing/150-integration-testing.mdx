---
title: '整合 測試'
metaTitle: '整合 測試 with Prisma'
metaDescription: '學習如何 to 設定 and 執行 整合 tests with Prisma and Docker'
tocDepth: 3
---
...
One way to simulate a real world environment is 使用 [Docker](https://www.docker.com/get-started/) to encapsulate a database and some test data. This can be spun up and torn down with the tests and so operate as an isolated environment away from your production databases.

</TopBlock>

> **注意：** This [blog post](https://www.prisma.io/blog/testing-series-2-xPhjjmIEsM) offers a comprehensive guide on setting up an integration testing environment and writing integration tests against a real database, providing valuable insights for those looking to explore this topic.
...
以下 ecommerce schema will be used throughout the guide. This varies from the traditional `User` and `Post` models used in other parts of the docs, mainly because it is unlikely you will be running integration tests against your blog.
...
The guide uses a singleton pattern for Prisma Client setup. 參閱 the [singleton](/orm/prisma-client/testing/unit-testing#singleton) docs for a walk through of 如何 set that up.
...
With Docker and Docker compose both installed on your machine 您可以 use them in your project.
...
> **Note**: The compose version used here (`3.9`) is the latest at the time of writing, if you are following along be sure 使用 the same version for consistency.

The `docker-compose.yml` file defines 以下:

- The Postgres image (`postgres`) and version tag (`:13`). 這將 be downloaded if you do not have it locally available.
- The port `5433` is mapped to the internal (Postgres default) port `5432`. 這將 be the port number the database is exposed on externally.
- The database user credentials are set and the database given a name.

2. To connect to the database in the container, create a new connection string with the credentials defined in the `docker-compose.yml` file. 例如：
...
3. 建立 the container in a detached state so that 您可以 continue 使用 the terminal tab, run 以下 command:

```terminal
docker compose up -d
```

4. Next 您可以 check that the database has been created by executing a `psql` command inside the container. Make a note of the container id.
...
The ecommerce application you are testing has a function which creates an order. This function does 以下:
...
以下 tests will check if the `createOrder` function works as it should do. They will test:
...
This setup isolates a real world scenario so that 您可以 test your applications functionality against real data in a controlled environment.

您可以 add some scripts to your projects `package.json` file which will setup the database and run the tests, then afterwards manually destroy the container.
...
The `test` script does 以下:

1. Runs `docker compose up -d` 建立 the container with the Postgres image and database.
1. Applies the migrations found in `./prisma/migrations/` directory to the database, 這會建立 the tables in the container's database.
1. Executes the tests.

Once you are satisfied 您可以 run `yarn docker:down` to destroy the container, its database and any test data.
