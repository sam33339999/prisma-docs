---
title: 'Prisma 驗證器'
metaTitle: 'Prisma 驗證器'
metaDescription: 'The Prisma 驗證器 is a utility function that takes a generated 型別 and returns a 型別安全 物件 which adheres to the generated types 模型 fields.'
---
...
This page introduces the `Prisma.validator` and offers some motivations behind why you might choose 使用 it.
...
:::warning  
	...
	In most cases, 您可以 achieve the same behavior using TypeScript’s native `satisfies` operator instead of `Prisma.validator`.  
	
	If you previously used `Prisma.validator`, 查看 this [blog post](https://www.prisma.io/blog/satisfies-operator-ur8ys8ccq7zb) to learn how the new `satisfies` keyword can improve and simplify your Prisma Client workflows.  
	:::
## Creating a typed 查詢 statement
...
## Using the `Prisma.驗證器`

以下範例 passes the `UserSelect` generated type into the `Prisma.validator` utility function and defines the expected return type in much the same way as the previous example.
...
Alternatively, 您可以 use 以下 syntax that uses a "selector" pattern using an existing instance of Prisma Client:
...
## Combining `Prisma.驗證器` with form input

以下範例 creates a type-safe function from the `Prisma.validator` which can be used when interacting with user created data, such as form inputs.
