---
title: '`查詢`: 建立 自訂 Prisma 客戶端 queries'
metaTitle: 'Prisma 客戶端擴充功能: 查詢 component'
metaDescription: 'Extend the functionality of Prisma 客戶端, 查詢 component'
...
您可以 use the `query` [Prisma Client extensions](/orm/prisma-client/client-extensions) component type to hook into the query life-cycle and modify an incoming query or its result.

您可以 use Prisma Client extensions `query` component 建立 independent clients with customized behavior. 您可以 bind one client to a specific filter or user, and another client to another filter or user. 例如, you might do this to get [user isolation](/orm/prisma-client/client-extensions#extended-clients) in a row-level security (RLS) extension. The `query` extension component provides end-to-end type safety for all your custom queries.

</TopBlock>

## Extend Prisma Client query operations

Use the `$extends` [client-level method](/orm/reference/prisma-client-reference#client-methods) 建立 an [extended client](/orm/prisma-client/client-extensions#about-prisma-client-extensions). An extended client is a variant of the standard Prisma Client that is wrapped by one or more extensions.

Use the `query` extension component to modify queries. 您可以 modify a custom query in 以下:
...
建立 a custom query, use 以下 structure:
...
The `query` object can contain functions that map to the names of the [Prisma Client operations](/orm/reference/prisma-client-reference#model-queries), such as `findUnique()`, `findFirst`, `findMany`, `count`, and `create`. 以下範例 modifies `user.findMany` to a use a customized query that finds only users who are older than 18 years:
...
In the above example, a call to `prisma.user.findMany` triggers `query.user.findMany`. Each callback receives a type-safe `{ model, operation, args, query }` object that describes the query. This object has 以下 properties:
...
- `args`: the specific query input information to be extended.
  ...
  This is a type-safe object that 您可以 mutate before the query happens. 您可以 mutate any of the properties in `args`. Exception: 您可以not mutate `include` or `select` because that would change the expected output type and break type safety.

- `query`: a promise for 結果 of the query.

  - 您可以 use `await` and then mutate 結果 of this promise, because its value is type-safe. TypeScript catches any unsafe mutations on the object.

### Modify a specific operation in all models of your schema

To extend the queries in all the models of your schema, use `$allModels` instead of a specific model name. 例如：
...
例如, 以下 code applies a custom query to all operations on the `user` model:
...
您可以 modify all methods as follows:
...
例如, 您可以 use the `$allOperations` method to log queries as follows:
...
### Mutate 結果 of a query

您可以 use `await` and then mutate 結果 of the `query` promise.
...
您可以 wrap your extended queries into a [batch transaction](/orm/prisma-client/queries/transactions). 例如, 您可以 use this to enact row-level security (RLS).

以下範例 extends `findFirst` so that it runs in a batch transaction.
