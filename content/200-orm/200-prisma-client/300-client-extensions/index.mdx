---
title: '擴充'
metaTitle: 'Prisma 客戶端擴充功能'
metaDescription: 'Extend the functionality of Prisma 客戶端'
...
您可以 use Prisma Client extensions 添加 functionality to your models, result objects, and queries, or 添加 client-level methods.

您可以 create an extension with one or more of 以下 component types:
...
例如, you might create an extension that uses the `model` and `client` component types.

</TopBlock>

## 關於 Prisma Client 擴充

When you use a Prisma Client extension, you create an _extended client_. An extended client is a lightweight variant of the standard Prisma Client that is wrapped by one or more extensions. The standard client is not mutated. 您可以 add as many extended clients as you want to your project. [Learn more about extended clients](#extended-clients).

您可以 associate a single extension, or multiple extensions, with an extended client. [Learn more about multiple extensions](#multiple-extensions).

您可以 [share your Prisma Client extensions](/orm/prisma-client/client-extensions/shared-extensions) with other Prisma ORM users, and [import Prisma Client extensions developed by other users](/orm/prisma-client/client-extensions/shared-extensions#install-a-shared-packaged-extension) into your Prisma ORM project.
...
> **Note**: The author of an extension can modify this behavior since they're able to run arbitrary code as part of an extension. 例如, an extension might actually create an entirely new `PrismaClient` instance (including its own query engine and connection pool). Be sure to check the documentation of the extension you're using to 學習關於 any specific behavior it might implement.

### Example use cases for extended clients

Because extended clients operate in isolated instances, they can be a good way to do 以下, 例如：
...
- Attach a unique request id to all logs so that 您可以 correlate them later, 例如 to help you analyze the operations that Prisma Client carries out.
...
您可以 create an extension using two primary ways:
...
您可以 name your extensions to help identify them in error logs. To do so, use the optional field `name`. 例如：
...
您可以 associate an extension with an [extended client](#about-prisma-client-extensions) in one of two ways:

- 您可以 associate it with an extended client on its own, or
- 您可以 combine the extension with other extensions and associate all of these extensions with an extended client. The functionality from these combined extensions applies to the same extended client.
  注意： [Combined extensions can conflict](#conflicts-in-combined-extensions).

您可以 combine the two approaches above. 例如, you might associate one extension with its own extended client and associate two other extensions with another extended client. [Learn more about how client instances interact](#extended-clients).

### Apply multiple 擴充 to an extended client

In 以下範例, suppose that you have two extensions, `extensionA` and `extensionB`. There are two ways to combine these.
...
您可以 then 參閱 `prismaAB` in your code, 例如 `prismaAB.myExtensionMethod()`.

#### Option 2: Declare multiple extended clients

The advantage of this option is that 您可以 call any of the extended clients separately.
...
In your code, 您可以 call any of these clients separately, 例如 `prismaA.myExtensionMethod()`, `prismaB.myExtensionMethod()`, or `prismaAB.myExtensionMethod()`.

### Conflicts in combined 擴充
...
您可以 infer the type of an extended Prisma Client instance using the [`typeof`](https://www.typescriptlang.org/docs/handbook/2/typeof-types.html) utility as follows:
...
If you're using Prisma Client as a singleton, 您可以 get the type of the extended Prisma Client instance using the `typeof` and [`ReturnType`](https://www.typescriptlang.org/docs/handbook/utility-types.html#returntypetype) utilities as follows:
...
## Extending 模型 types with `Prisma.Result`

您可以 use the `Prisma.Result` type utility to extend model types to include properties added via client extensions. This allows you to infer the type of the extended model, including the extended properties.

### Example

以下範例 demonstrates 如何使用 `Prisma.Result` to extend the `User` model type to include a `__typename` property added via a client extension.
