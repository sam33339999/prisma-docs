---
title: 'TypedSQL'
metaTitle: '使用 TypedSQL 和 Prisma 客戶端 編寫型別安全的 SQL'
metaDescription: '學習如何使用 TypedSQL 編寫與任何 SQL 控制台和 Prisma 客戶端 相容的完全型別安全 SQL 查詢。'
sidebar_class_name: preview-badge
---

## TypedSQL 入門

要在 Prisma 專案中開始使用 TypedSQL,請遵循以下步驟:

1. 確保您已安裝 `@prisma/client` 和 `prisma` 並更新至至少 `5.19.0` 版本。
...
1. Create a `sql` directory inside your `prisma` directory. This is where you'll write your SQL queries.
   ...
   Starting with Prisma 6.12.0, 您可以 configure a custom location for your SQL files using the Prisma config file. Create a `prisma.config.ts` file in your project root and specify the `typedSql.path` option:
...
1. Create a new `.sql` file in your `prisma/sql` directory. 例如, `getUsersWithPosts.sql`. 請注意 the file name must be a valid JS identifier and cannot start with a `$`.

1. Write your SQL queries in your new `.sql` file. 例如：
...
1. Generate Prisma Client with the `sql` flag to ensure TypeScript functions and types for your SQL queries are created:
   ...
   If you don't want to regenerate the client after every change, this command also 適用於 the existing `--watch` flag:

   ```terminal
   prisma generate --sql --watch
   ```

1. Now 您可以 import and use your SQL queries in your TypeScript code:
  ...
  const prisma = new PrismaClient()
   ...
   If you do not customize the generator `output`, 您可以 import from `@prisma/client` and `@prisma/client/sql` instead.

   :::

## Passing Arguments to TypedSQL 查詢

To pass arguments to your TypedSQL queries, 您可以 use parameterized queries. This allows you to write flexible and reusable SQL statements while maintaining type safety. Here's 如何 do it:
...
1. In your SQL file, use placeholders for the parameters you want to pass. The syntax for placeholders depends on your database engine:
   ...
   In SQLite, there are a number of different placeholders 您可以 use. Positional placeholders (`$1`, `$2`, etc.), general placeholders (`?`), and named placeholders (`:minAge`, `:maxAge`, etc.) are all available. For this example, we'll use named placeholders `:minAge` and `:maxAge`:
   ...
   See below for information on 如何 [define argument types in your SQL files](#defining-argument-types-in-your-sql-files).
...
As an example, if you needed to type a single string argument with the alias `name` and the description "The name of the user", you would add 以下 comment to your SQL file:
```sql
...
For practical examples of 如何使用 TypedSQL, please 參閱 the [TypedSQL example in the Prisma Examples repo](https://github.com/prisma/prisma-examples/tree/latest/generator-prisma-client/basic-typedsql).
...
### Active 資料庫 連線 Required

TypedSQL 需要 an active database connection to function properly. This means 您需要 have a running database instance that Prisma can connect to when generating the client with the `--sql` flag. TypedSQL uses the connection string defined in `prisma.config.ts` (`datasource.url`) to establish this connection.

### Dynamic SQL 查詢 with Dynamic Columns

TypedSQL does not natively support constructing SQL queries with dynamically added columns. When 您需要 create a query where the columns are determined at runtime, 您必須 use the `$queryRaw` and `$executeRaw` methods. These methods allow for the execution of raw SQL, which can include dynamic column selections.
...
In this example, the columns to be selected are defined dynamically and included in the SQL query. While this approach provides flexibility, it 需要 careful attention to security, particularly to [avoid SQL injection vulnerabilities](/orm/prisma-client/using-raw-sql/raw-queries#sql-injection-prevention). Additionally, using raw SQL queries means foregoing the type-safety and DX of TypedSQL.
