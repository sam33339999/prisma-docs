...
title: '指標'
metaTitle: '指標 (預覽)'
metaDescription: 'Diagnose application 效能 with insights into Prisma 客戶端 資料庫 activity.'
sidebar_class_name: preview-badge
toc_max_heading_level: 4
---


Prisma Client metrics give you a detailed insight into how Prisma Client interacts with your database. 您可以 use this insight to help diagnose performance issues with your application.
...
If you need visibility into query or connection pool behavior, we recommend using the *native metrics provided by your database driver* (例如, pool statistics) or setting up *OpenTelemetry* for a more complete observability solution.  
...
您可以 export metrics in JSON or Prometheus formats and view them in a console log, or integrate them into an external metrics system, such as [StatsD](https://github.com/statsd/statsd) or [Prometheus](https://prometheus.io/). If you integrate them into an external metrics system, then 您可以 view the metrics data over time. 例如, 您可以 use metrics to help diagnose how your application's number of idle and active connections changes.

Prisma Client provides 以下 metrics:
...
您可以 [add global labels to your metrics data](#global-labels) to help you group and separate your metrics, 例如 by infrastructure region or server.

## 前置需求

使用 Prisma Client metrics, 您必須 do 以下:
...
您必須 use a version between `3.15.0` and `6.13.x` of the `prisma` and `@prisma/client` npm packages.
...
### 2. Enable the feature flag in the Prisma 綱要 file
...
## Retrieve 指標 in JSON format

When you retrieve metrics in JSON format, 您可以 use them in the format they are returned, or [send them to StatSD](#use-prisma-client-metrics-with-statsd) to visualize how they change over time.

To retrieve metrics in JSON format, add 以下 lines to your application code:
...
Each histogram "bucket" has two values. The first one is the upper bound of the bucket, and the second one is the count (the number of data values that fall into that bucket). In 以下範例, there are two instances of values between 11 and 20, and five instances of values between 21 and 30:
...
### Use Prisma Client 指標 with StatsD

您可以 send JSON-formatted metrics to [StatsD](https://github.com/statsd/statsd) to visualize your metrics data over time.

:::info

注意： 您必須 provide counter metrics to StatsD as a series of values that are incremented or decremented from a previous retrieval of the metrics. However, Prisma Client's counter
metrics return absolute values. Therefore, 您必須 convert your counter metrics to a series of incremented and decremented values and send them to StatsD as gauge data. In the 程式碼範例 below, we convert counter metrics into incremented and decremented gauge data in `diffHistograms`.

:::

In 以下範例, we send metrics to StatsD every 10 seconds. This timing aligns with the default 10s flush rate of StatsD.
...
## Retrieve 指標 in Prometheus format

When you retrieve Prisma Client metrics in Prometheus format, 您可以 use them in the format they are returned, or [send them to the Prometheus metrics system](#use-prisma-client-metrics-with-the-prometheus-metrics-system) to visualize how they change over time.

To retrieve metrics in Prometheus format, add 以下 lines to your application code:
...
1. Multiple cumulative counters for observation buckets. These counters are suffixed with `_bucket{le="<upper inclusive bound>"}`. 例如, `prisma_datasource_queries_duration_histogram_ms` has a counter exposed as `prisma_datasource_queries_duration_histogram_ms_bucket{le="1"}`
/* Lines 502-504 omitted */

2. A single **total sum** for all observed values. This counter is suffixed with `_sum`. 例如 the total sum of `prisma_datasource_queries_duration_histogram_ms` is exposed as `prisma_datasource_queries_duration_histogram_ms_sum`.
3. The **count** of the number of events that have been observed. This counter is suffixed with `_count`. 例如 the total count of `prisma_datasource_queries_duration_histogram_ms` events is exposed as `prisma_datasource_queries_duration_histogram_ms_count`.

欲了解更多資訊, read the Prometheus documentation on [metric types](https://prometheus.io/docs/concepts/metric_types/#histogram).

### Use Prisma Client 指標 with the Prometheus 指標 system

In the majority of cases, Prometheus must scrape an endpoint to retrieve metrics. 以下範例 shows 如何 send data with `Express.js`:
...
以下範例 shows 如何 combine Prisma Client metrics with other Prometheus client libraries that are also served with a REST API endpoint in conjunction with `Express.js`:
...
您可以 add global labels to your metrics to help you group and separate your metrics. Each instance of Prisma Client adds these labels to the metrics that it generates. 例如, 您可以 group your metrics by infrastructure region, or by server, with a label like `{ server: us_server1', 'app_version': 'one' }`.

Global labels work with JSON and Prometheus-formatted metrics.

例如, 添加 global labels to JSON-format metrics, add 以下 code to your application:
