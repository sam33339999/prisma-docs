...
title: '日誌'
metaTitle: '日誌'
metaDescription: '學習 如何 configure Prisma 客戶端 to log the 原始 SQL queries it sends to the 資料庫 and other information.'
---

<TopBlock>

Use the `PrismaClient` [`log`](/orm/reference/prisma-client-reference#log) parameter 配置 [log levels](/orm/reference/prisma-client-reference#log-levels) , including warnings, errors, and information about the queries sent to the database.
...
您可以 also use the `DEBUG` environment variable 啟用 debugging output in Prisma Client. See [Debugging](/orm/prisma-client/debugging-and-troubleshooting/debugging) 欲了解更多資訊.
