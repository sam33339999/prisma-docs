...
title: '排除欄位'
metaTitle: '排除欄位'
metaDescription: '本頁說明 如何 exclude sensitive fields from Prisma 客戶端'
---

預設情況下 Prisma Client returns all fields from a model. 您可以 use [`select`](/orm/prisma-client/queries/select-fields) to narrow 結果 set, but that can be unwieldy if you have a large model and you only want to exclude a small number of fields.

:::info

As of Prisma ORM 6.2.0, excluding fields is supported via the `omit` option that 您可以 pass to Prisma Client. From versions 5.16.0 through 6.1.0, 您必須 use the `omitApi` Preview feature to access this option.

:::

## Excluding a field globally using `omit`

以下 is a type-safe way to exclude a field _globally_ (i.e. for _all_ queries against a given model):
...
以下 is a type-safe way to exclude a field _locally_ (i.e. for a _single_ query):
...
## 如何 select a previously omitted field

If you [omit a field globally](#excluding-a-field-globally-using-omit), 您可以 "override" by either selecting the field specifically or by setting `omit` to `false` in a query.
...
建議 fix is 使用 `as const` to ensure the type is exact:
...
## 何時使用 `omit` globally or locally

It's important to understand when to omit a field globally or locally:

- If you are omitting a field in order to prevent it from accidentally being included in a query, it's best to omit it _globally_. 例如： Globally omitting the `password` field from a `User` model so that sensitive information doesn't accidentally get exposed.
