---
title: '交易 and batch queries'
metaTitle: '交易 and batch queries (參考)'
metaDescription: '本頁說明 the transactions API of Prisma 客戶端.'
tocDepth: 3
---

<TopBlock>

A database transaction refers to a sequence of read/write operations that are _guaranteed_ to either succeed or fail as a whole. 本節描述 the ways in which the Prisma Client API supports transactions.
...
While there's a lot of ambiguity and nuance to each of these properties (例如, consistency could actually be considered an _application-level responsibility_ rather than a database property or isolation is typically guaranteed in terms of stronger and weaker _isolation levels_), overall they serve as a good high-level guideline for expectations developers have when thinking about database transactions.
...
Prisma Client provides 以下 options for using transactions:
...
A [nested write](/orm/prisma-client/queries/relation-queries#nested-writes) lets you perform a single Prisma Client API call with multiple _operations_ that touch multiple [_related_](/orm/prisma-schema/data-model/relations) records. 例如, creating a _user_ together with a _post_ or updating an _order_ together with an _invoice_. Prisma Client ensures that all operations succeed or fail as a whole.

以下範例 demonstrates a nested write with `create`:
...
以下範例 demonstrates a nested write with `update`:
...
以下 bulk operations run as transactions:
...
> 參閱 the section about [bulk operations](#bulk-operations) for more examples.
...
以下 query returns all posts that match the provided filter as well as a count of all posts:
...
您可以 also use raw queries inside of a `$transaction`:
...
Instead of immediately awaiting 結果 of each operation when it's performed, the operation itself is stored in a variable first which later is submitted to the database with a method called `$transaction`. Prisma Client will ensure that either all three `create` operations succeed or none of them succeed.
...
>
> 參閱 the section about the [transactions API](#transaction-api) for more examples.

From version 4.4.0, the sequential operations transaction API has a second parameter. 您可以 use 以下 optional configuration option in this parameter:

- `isolationLevel`: Sets the [transaction isolation level](#transaction-isolation-level). 預設情況下 this is set to the value currently configured in your database.
...
If you use interactive transactions in preview from version 2.29.0 to 4.6.1 (inclusive), 您需要 add the `interactiveTransactions` preview feature to the generator block of your Prisma schema.

:::

使用 interactive transactions, 您可以 pass an async function into [`$transaction`](#transaction-api).
...
To catch the exception, 您可以 wrap `$transaction` in a try-catch block:
...
The transaction API has a second parameter. For interactive transactions, 您可以 use 以下 optional configuration options in this parameter:
...
您可以 also set these globally on the constructor-level:
...
您可以 set the transaction [isolation level](https://www.prisma.io/dataguide/intro/database-glossary#isolation-levels) for transactions.

:::info

This is 可用於 以下 Prisma ORM versions for interactive transactions from version 4.2.0, for sequential operations from version 4.4.0.

In versions before 4.2.0 (for interactive transactions), or 4.4.0 (for sequential operations), 您可以not configure the transaction isolation level at a Prisma ORM level. Prisma ORM does not explicitly set the isolation level, so the [isolation level configured in your database](#database-specific-information-on-isolation-levels) is used.
...
Prisma Client supports 以下 isolation levels if they are 可用於 the underlying database:
...
預設情況下, Prisma Client sets the isolation level to the value currently configured in your database.

The isolation levels configured 預設情況下 in each database are as follows:
...
- The solution 在本節中 does not apply to MongoDB, because MongoDB does not support [isolation levels](https://www.prisma.io/dataguide/intro/database-glossary#isolation-levels).
- The timing issues discussed 在本節中 do not apply to CockroachDB and SQLite, because these databases only support the highest `Serializable` isolation level.

:::

When two or more transactions run concurrently in certain [isolation levels](https://www.prisma.io/dataguide/intro/database-glossary#isolation-levels), timing issues can cause write conflicts or deadlocks, such as the violation of unique constraints. 例如, consider 以下 sequence of events where Transaction A and Transaction B both attempt to execute a `deleteMany` and a `createMany` operation:
...
This conflict can occur at the isolation level `ReadCommited`, which is the default isolation level in PostgreSQL and Microsoft SQL Server. To avoid this problem, 您可以 set a higher isolation level (`RepeatableRead` or `Serializable`). 您可以 set the isolation level on a transaction. This overrides your database isolation level for that transaction.
...
2. In your application code, add a retry around your transaction to handle any P2034 errors, as shown in this 範例：
...
- Operations depend on 結果 of a preceding operation (例如, the database generating an ID)

The most common scenario is creating a record and using the generated ID 建立 or update a related record. Examples include:
...
Prisma Client's solution to dependent writes is the **nested writes** feature, which is 支援 `create` and `update`. 以下 nested write creates one user and two blog posts:
...
If any operation fails, Prisma Client rolls back the entire transaction. Nested writes are not currently 支援 top-level bulk operations like `client.user.deleteMany` and `client.user.updateMany`.

#### 何時使用 nested writes

Consider using nested writes if:

- ✔ You want 建立 two or more records related by ID at the same time (例如, create a blog post and a user)
- ✔ You want 更新 and create records related by ID at the same time (例如, change a user's name and create a new blog post)

:::tip

If you [pre-compute your IDs, 您可以 choose between a nested write or using the `$transaction([])` API](#scenario-pre-computed-ids-and-the-transaction-api).
...
This scenario can be represented by 以下 schema - 請注意 users can belong to many teams, and teams can have many users (a many-to-many relationship):
...
The most straightforward approach is 建立 a team, then create and attach a user to that team:
...
However, this code has a problem - consider 以下 scenario:
...
To implement atomic writes in a low-level database clients, 您必須 wrap your inserts in `BEGIN`, `COMMIT` and `ROLLBACK` statements. Prisma Client solves the problem with [nested writes](/orm/prisma-client/queries/relation-queries#nested-writes). 以下 query creates a team, creates a user, and connects the records in a single transaction:
...
The `$transaction([])` API does not allow you to pass IDs between distinct operations. In 以下範例, `createUserOperation.id` is not available yet:
...
It is correct to say that because you know the ID of the team, 您可以 update the team and its team members independently within a `$transaction([])`. 以下範例 performs both operations in a `$transaction([])`:
...
However, 您可以 achieve the same result with a nested write:
...
##### Can I perform multiple nested writes - 例如, create two new teams and assign users?
...
Writes are considered **independent** if they do not rely on 結果 of a previous operation. 以下 groups of independent writes can occur in any order:

- Updating the status field of a list of orders to "Dispatched"
- Marking a list of emails as "Read"

> **Note**: Independent writes may have to occur in a specific order if constraints are present - 例如, 您必須 delete blog posts before the blog author if the post have a mandatory `authorId` field. However, they are still considered independent writes because no operations depend on the _result_ of a previous operation, such as the database returning a generated ID.

Depending on your 需求, Prisma Client has four options for handling independent writes that should succeed or fail together.
...
Based on this schema, 您可以 use `updateMany` to mark all unread emails as read:
...
No - neither `updateMany` nor `deleteMany` currently supports nested writes. 例如, 您可以not delete multiple teams and all of their members (a cascading delete):
...
Yes — 例如, 您可以 include multiple `deleteMany` operations inside a `$transaction([])`.
...
#### 何時使用 the `$transaction([])` API

Consider the `$transaction([])` API if:

- ✔ You want 更新 a batch that includes different types of records, such as emails and users. The records do not need to be related in any way.
- ✔ You want to batch raw SQL queries (`$executeRaw`) - 例如, for features that Prisma Client does not yet support.

#### Scenario: Privacy legislation

GDPR and other privacy legislation give users the right to request that an organization deletes all of their personal data. In 以下範例 schema, a `User` can have many posts and private messages:
...
If a user invokes the right to be forgotten, we must delete three records: the user record, private messages, and posts. It is critical that _all_ delete operations succeed together or not at all, which makes this a use case for a transaction. However, using a single bulk operation like `deleteMany` is not possible in this scenario because we need 刪除 across three models. Instead, we can use the `$transaction([])` API to run three operations together - two `deleteMany` and one `delete`:
...
Dependent writes are not 支援 the `$transaction([])` API - if operation A relies on the ID generated by operation B, use [nested writes](#nested-writes). However, if you _pre-computed_ IDs (例如, by generating GUIDs), your writes become independent. Consider the sign-up flow from the nested writes 範例：
...
Refactor the sign-up flow example 使用 the `$transaction([])` API instead of nested writes:
...
Technically 您可以 still use nested writes with pre-computed APIs if you prefer that syntax:
...
In some cases you may need to perform custom logic as part of an atomic operation - also known as the [read-modify-write pattern](https://en.wikipedia.org/wiki/Read%E2%80%93modify%E2%80%93write). 以下 is an example of the read-modify-write pattern:

- Read a value from the database
- Run some logic to manipulate that value (例如, contacting an external API)
- Write the value back to the database

All operations should **succeed or fail together** without making unwanted changes to the database, but you do not necessarily need 使用 an actual database transaction. This section of the guide describes two ways to work with Prisma Client and the read-modify-write pattern:
...
Idempotency is the ability to run the same logic with the same parameters multiple times with the same result: the **effect on the database** is the same whether you run the logic once or one thousand times. 例如：
...
Idempotency is something 您可以 and should actively design into your application wherever possible.

#### When to design an idempotent API

- ✔ 您需要 be able to retry the same logic without creating unwanted side-effects in the databases
...
This example has a problem: 您可以 only run the logic _once_. Consider 以下 scenario:
...
您可以not re-run this code in case of an error and 您可以not change to another plan without being charged twice.

以下 refactor (highlighted) introduces a mechanism that checks if a subscription already exists, and either creates the description or updates the existing subscription (which will remain unchanged if the input is identical):
...
您可以 now retry the same logic multiple times with the same input without adverse effect. To further enhance this example, 您可以 introduce a mechanism whereby the subscription is cancelled or temporarily deactivated if the update does not succeed after a set number of attempts.
...
If a ❌ conflict occurs (someone else has changed the record since you read it), 您可以cel the transaction. Depending on your scenario, 您可以 then:
...
本節描述 如何 build your own optimistic concurrency control. 另請參閱: Plans for [application-level optimistic concurrency control on GitHub](https://github.com/prisma/prisma/issues/4988)

:::info

- If you use version 4.4.0 or earlier, 您可以not use optimistic concurrency control on `update` operations, because 您可以not filter on non-unique fields. The `version` field 您需要 use with optimistic concurrency control is a non-unique field.
...
#### 何時使用 optimistic concurrency control
...
You are creating a booking system for a cinema. Each movie has a set number of seats. 以下 schema models movies and seats:
...
以下 sample code finds the first available seat and assigns that seat to a user:
...
If you have an existing application, it can be a significant undertaking to refactor your application 使用 optimistic concurrency control. Interactive Transactions offers a useful escape hatch for cases like this.

建立 an interactive transaction, pass an async function into [$transaction](#transaction-api).
...
您可以 learn more about interactive transactions in this [section](#interactive-transactions).
