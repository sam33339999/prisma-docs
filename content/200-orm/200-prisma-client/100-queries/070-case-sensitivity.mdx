...
title: '區分大小寫'
metaTitle: '區分大小寫 (參考)'
metaDescription: 'How Prisma 客戶端 handles case sensitivity when filtering and sorting.'
preview: false
---
...
If you use the MongoDB connector, [Prisma Client](/orm/prisma-client/queries) uses RegEx rules 啟用 case-insensitive filtering. The connector _does not_ use [MongoDB collation](https://www.mongodb.com/docs/manual/reference/collation/).
...
In the context of Prisma Client, 以下 section refers to relational database connectors only.
...
以下範例 demonstrates 如何 view the collation of a MySQL database:
...
建議 way to support case-insensitive filtering with Prisma Client depends on your underlying provider.

### PostgreSQL provider

PostgreSQL uses [deterministic collation](https://www.postgresql.org/docs/current/collation.html#COLLATION-NONDETERMINISTIC) 預設情況下, which means that filtering is **case-sensitive**. To support case-insensitive filtering, use the `mode: 'insensitive'` property on a per-field basis.
...
另請參閱: [Filtering (Case-insensitive filtering)](/orm/prisma-client/queries/filtering-and-sorting#case-insensitive-filtering)

#### Caveats

- 您可以not use case-insensitive filtering with C collation
...
MySQL uses **case-insensitive collation** 預設情況下. Therefore, filtering with Prisma Client and MySQL is case-insensitive 預設情況下.

`mode: 'insensitive'` property is not required and therefore not 可用於 the generated Prisma Client API.
...
預設情況下, text fields created by Prisma Client in SQLite databases do not support case-insensitive filtering. In SQLite, only [case-insensitive comparisons of ASCII characters](https://www.sqlite.org/faq.html#q18) are possible.

啟用 limited support (ASCII only) for case-insensitive filtering on a per-column basis, you will need 添加 `COLLATE NOCASE` when you define a text column.

#### Adding case-insensitive filtering to a new column.

添加 case-insensitive filtering to a new column, you will need to modify the migration file that is created by Prisma Client.

Taking 以下 Prisma Schema model:
...
and using `prisma migrate dev --create-only` 建立 以下 migration file:
...
You would need 添加 `COLLATE NOCASE` to the `email` column in order to make case-insensitive filtering possible:
...
and using `prisma migrate dev --create-only` 建立 an empty migration file, you will need to rename the current `User` table and create a new `User` table with `COLLATE NOCASE`.
...
Microsoft SQL Server uses **case-insensitive collation** 預設情況下. Therefore, filtering with Prisma Client and Microsoft SQL Server is case-insensitive 預設情況下.
