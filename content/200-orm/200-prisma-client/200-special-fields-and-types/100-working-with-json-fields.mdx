---
title: '使用 Json fields'
metaTitle: '使用 Json fields (Concepts)'
metaDescription: '如何 讀取, 寫入, and 篩選 by Json fields.'
tocDepth: 3
---

Use the [`Json`](/orm/reference/prisma-schema-reference#json) Prisma ORM field type to read, write, and perform basic filtering on JSON types in the underlying database. In 以下範例, the `User` model has an optional `Json` field named `extendedPetsData`:
...
The `Json` field supports a few additional types, such as `string` and `boolean`. These additional types exist to match the types 支援 [`JSON.parse()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse):
...
- 您需要 store data that does not have a consistent structure
- You are importing data from another system and do not want to map that data to Prisma models

## Reading a `Json` field

您可以 use the `Prisma.JsonArray` and `Prisma.JsonObject` utility classes to work with the contents of a `Json` field:
...
另請參閱: [Advanced 範例： Update a nested JSON key value](#advanced-example-update-a-nested-json-key-value)

## Writing to a `Json` field

以下範例 writes a JSON object to the `extendedPetsData` field:
...
> **Note**: JavaScript objects (例如, `{ extendedPetsData: "none"}`) are automatically converted to JSON.

另請參閱: [Advanced 範例： Update a nested JSON key value](#advanced-example-update-a-nested-json-key-value)

## Filter on a `Json` field (simple)

您可以 filter rows of `Json` type.

### Filter on exact field value

以下 query returns all users where the value of `extendedPetsData` matches the `json` variable exactly:
...
以下 query returns all users where the value of `extendedPetsData` does **not** match the `json` variable exactly:
...
您可以 also filter rows by the data inside a `Json` field. We call this **advanced `Json` filtering**. This functionality is 支援 [PostgreSQL](/orm/overview/databases/postgresql) and [MySQL](/orm/overview/databases/mysql) only with [different syntaxes for the `path` option](#path-syntax-depending-on-database).
...
- Before v2.23.0: 您可以 [filter on the exact `Json` field value](#filter-on-exact-field-value), but 您可以not use the other features described 在本節中.
...
例如, 以下 is a valid MySQL `path` value:

```
$petFeatures.petName
```

以下 is a valid PostgreSQL `path` value:
...
您可以 filter on a specific property inside a block of JSON. In 以下範例, the value of `extendedPetsData` is a one-dimensional, unnested JSON object:
...
以下 query returns all users where the value of `petName` is `"Claudine"`:
...
以下 query returns all users where the value of `petType` _contains_ `"cat"`:
...
以下 string filters are available:
...
使用 case insensitive filter with these, 您可以 use the [`mode`](/orm/reference/prisma-client-reference#mode) option:
...
您可以 filter on nested JSON properties. In 以下範例, the value of `extendedPetsData` is a JSON object with several levels of nesting.
...
以下 query returns all users where `"pet2"` &rarr; `"petName"` is `"Sunny"`:
...
以下 query returns all users where:
...
您可以 filter on the presence of a specific value in a scalar array (strings, integers). In 以下範例, the value of `extendedPetsData` is an array of strings:

```json
["Claudine", "Sunny"]
```

以下 query returns all users with a pet named `"Claudine"`:
...
以下 array filters are available:
...
您可以 filter on the presence of a specific value in a scalar array (strings, integers). In 以下範例, the value of `extendedPetsData` includes nested scalar arrays of names:
...
以下 query returns all users that foster a cat named `"Fido"`:
...
以下 query returns all users that foster cats named `"Fido"` _and_ `"Bob"`:
...
- If you are using PostgreSQL, 您必須 pass in an array of objects to match, even if that array only contains one object:
  ...
  If you are using MySQL, 您必須 pass in a single object to match:
...
- 您必須 set `array_contains` to a JSON object, not a string. If you use a string, Prisma Client escapes the quotation marks and the query will not return results. 例如：
...
您可以 filter on the value of an element in a specific position.
...
Depending on your provider, 您可以 filter on the key value of an object inside an array.

:::warning

Filtering on object key values within an array is **only** 支援 the [MySQL database connector](/orm/overview/databases/mysql). However, 您可以 still [filter on the presence of entire JSON objects](#json-object-arrays).

:::

In 以下範例, the value of `extendedPetsData` is an array of objects with a nested `insurances` array, which contains two objects:
...
以下 query returns all users where at least one pet is a moose:
...
以下 query returns all users where at least one pet has an expired insurance:
...
## Advanced 範例： Update a nested JSON key value

以下範例 assumes that the value of `extendedPetsData` is some variation of 以下:
...
To differentiate between these possibilities, we've introduced three _null enums_ 您可以 use:
...
- When filtering using any of the _null enums_ 您可以 not use a shorthand and leave the `equals` operator off.
...
Prisma's `Json` fields are untyped 預設情況下. 添加 strong typing, 您可以 use the external package [prisma-json-types-generator](https://www.npmjs.com/package/prisma-json-types-generator).
...
您可以 also apply these techniques to `String` fields. This is especially useful for creating string-based enums directly in your schema when your database does not support enum types.
...
For a complete guide on configuration, monorepo setup, and other advanced features, please 參閱 the [official `prisma-json-types-generator` documentation](https://github.com/arthurfiorette/prisma-json-types-generator#readme).
...
### 如何 set a default value for JSON fields?

When you want to set a `@default` value the `Json` type, 您需要 enclose it with double-quotes inside the `@default` attribute (and potentially escape any "inner" double-quotes using a backslash), 例如：
