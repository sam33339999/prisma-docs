...
metaDescription: '學習如何 to use 關於 特殊欄位和型別 with Prisma 客戶端.'
tocDepth: 3
---

本節涵蓋 various special fields and types 您可以 use with Prisma Client.

## Working with `Decimal`

`Decimal` fields are represented by the [`Decimal.js` library](https://mikemcl.github.io/decimal.js/). 以下範例 demonstrates 如何 import and use `Prisma.Decimal`:
...
您可以 also perform arithmetic operations:
...
`BigInt` fields are represented by the [`BigInt` type](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt) (Node.js 10.4.0+ required). 以下範例 demonstrates 如何使用 the `BigInt` type:
...
Prisma Client returns records as plain JavaScript objects. If you attempt 使用 `JSON.stringify` on an object that includes a `BigInt` field, you will see 以下 error:
...
`Bytes` fields are represented by the [`Uint8Array`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Uint8Array) type. 以下範例 demonstrates 如何使用 the `Uint8Array` type:
...
請注意 **before Prisma v6**, `Bytes` were represented by the [`Buffer`](https://nodejs.org/api/buffer.html) type:
