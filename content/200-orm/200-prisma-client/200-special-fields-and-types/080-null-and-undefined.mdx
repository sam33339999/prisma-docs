...
metaTitle: 'null 和 undefined in Prisma 客戶端 (參考)'
metaDescription: 'How Prisma 客戶端 handles null 和 undefined'
preview: false
---
...
## Strict undefined checks (預覽 feature)
...
啟用 this feature, add 以下 to your Prisma schema:
...
### `null` and `undefined` in 查詢 that affect _many_ records
...
Consider 以下 Prisma Client query which searches for all users whose `name` value matches the provided `null` value:
...
### `null` and `undefined` in 查詢 that affect _one_ record
...
Consider 以下 query where `null` is used to filter the `name` column:
...
For this example, consider a database based on 以下 Prisma schema:
...
In 以下 GraphQL mutation that updates a user, both `authorEmail` and `name` accept `null`. From a GraphQL perspective, this means that fields are **optional**:
...
However, if you pass `null` values for `authorEmail` or `authorName` on to Prisma Client, 以下 will happen:
...
以下 table provides a high-level overview of how the different operators handle 0, 1 and `n` filters.
...
This example shows how an `undefined` parameter impacts 結果s returned by a query that uses the [`OR`](/orm/reference/prisma-client-reference#or) operator.
