---
title: '使用 scalar lists'
metaTitle: '使用 scalar lists/arrays (Concepts)'
metaDescription: '如何 讀取, 寫入, and 篩選 by scalar lists / arrays.'
tocDepth: 3
---
...
以下範例 demonstrates 如何 [`set`](/orm/reference/prisma-client-reference#set-1) the value of a scalar list (`coinflips`) when you create a model:
...
以下範例 demonstrates 如何 [`unset`](/orm/reference/prisma-client-reference#unset) the value of a scalar list (`coinflips`):
...
Use the [`push`](/orm/reference/prisma-client-reference#push) method 添加 a single value to a scalar list:
...
In earlier versions, you have to overwrite the entire value. 以下範例 retrieves user, uses `push()` 添加 three new coin flips, and overwrites the `coinflips` field in an `update`:
...
Use [scalar list filters](/orm/reference/prisma-client-reference#scalar-list-filters) to filter for records with scalar lists that match a specific condition. 以下範例 returns all posts where the tags list includes `databases` _and_ `typescript`:
...
When using scalar list filters with a relational database connector, array fields with a `NULL` value are not considered by 以下 conditions:

- `NOT` (array does not contain X)
- `isEmpty` (array is empty)

This means that records you might expect to see are not returned. Consider 以下範例:

- 以下 query returns all posts where the `tags` **do not** include `databases`:
...
以下 query returns all posts where `tags` is empty:
...
To work around this issue, 您可以 set the default value of array fields to `[]`.
