---
title: '部署 to AWS Lambda'
metaTitle: '部署 your application 使用 Prisma ORM to AWS Lambda'
metaDescription: '學習如何 to 部署 your Prisma ORM-backed applications to AWS Lambda with AWS SAM, Serverless 框架, or SST'
tocDepth: 3
---


:::info Quick summary
本指南說明如何 avoid common issues when deploying a project using Prisma ORM to [AWS Lambda](https://aws.amazon.com/lambda/).
:::

<details>
<summary>Questions answered in this page</summary>

- 如何 deploy Prisma to AWS Lambda?
- Which binaryTargets should I configure?
- 如何 handle connection pooling on Lambda?

</details>

While a deployment framework is not required to deploy to AWS Lambda, this guide covers deploying with:

- [AWS Serverless Application Model (SAM)](https://aws.amazon.com/serverless/sam/) is an open-source framework from AWS that can be used in the creation of serverless applications. AWS SAM includes the [AWS SAM CLI](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-reference.html#serverless-sam-cli), which 您可以 use to build, test, and deploy your application.
...
本節涵蓋 changes you will need to make to your application, regardless of framework. After following these steps, follow the steps for your framework.
...
In the `generator` block of your `schema.prisma` file, add 以下:
...
While we do not recommend running migrations within AWS Lambda, some applications will require it. In these cases, 您可以 use the [PRISMA_CLI_BINARY_TARGETS](/orm/reference/environment-variables-reference#prisma_cli_binary_targets) environment variable to make sure that Prisma CLI commands, including `prisma migrate`, have access to the correct schema engine.

In the case of AWS lambda, you will have 添加 以下 environment variable:
...
`prisma migrate` is a command in the `prisma` package. Normally, this package is installed as a dev dependency. Depending on your setup, you may need 安裝 this package as a dependency instead so that it is included in the bundle or archive that is uploaded to Lambda and executed.
...
您可以 use [Accelerate](/accelerate) for connection pooling or [Prisma Postgres](/postgres), which has built-in connection pooling, to solve this issue. For other solutions, 參見 [connection management guide for serverless environments](/orm/prisma-client/setup-and-configuration/databases-connections#serverless-environments-faas).

## Deploying with AWS SAM

### Loading environment variables

AWS SAM does not directly support loading values from a `.env` file. You will have 使用 one of AWS's services to store and retrieve these parameters. [This guide](https://medium.com/bip-xtech/a-practical-guide-to-surviving-aws-sam-d8ab141b3d25) provides a great overview of your options and 如何 store and retrieve values in Parameters, SSM, Secrets Manager, and more.
...
To get around this, 您需要 directly reference the needed files in your code to bundle them correctly. In your application, you could add 以下 lines to your application where Prisma ORM is instantiated.
...
Your functions will need the `DATABASE_URL` environment variable to access the database. The `serverless-dotenv-plugin` will allow you 使用 your `.env` file in your deployments.
...
If you are deploying to [Lambda functions with ARM64 architecture](#lambda-functions-with-arm64-architectures) 您應該 update the Serverless configuration file to package the `arm64` engine file, as follows:
...
First, ensure 以下 webpack dependencies are installed:
...
In your `webpack.config.js`, make sure that you set `externals` to `nodeExternals()` like 以下:
...
This plugin will allow you to copy your `schema.prisma` file into your bundled code. Prisma ORM 需要 that your `schema.prisma` be present in order make sure that queries are encoded and decoded according to your schema. In most cases, bundlers will not include this file 預設情況下 and will cause your application to fail to run.

:::info

Depending on how your application is bundled, you may need to copy the schema to a location other than `./`. Use the `serverless package` command to package your code locally so 您可以 review where your schema should be put.

:::

參閱 the [Serverless Webpack documentation](https://www.serverless.com/plugins/serverless-webpack) for additional configuration.
...
這將 ensure that, after webpack bundles your code, the Prisma Client is generated according to your schema. Without this step, your app will fail to run.

Lastly, you will want to exclude [Prisma ORM query engines](/orm/more/under-the-hood/engines) that do not match the AWS Lambda runtime. Update your `serverless.yml` by adding 以下 script that makes sure only the required query engine, `rhel-openssl-1.0.x`, is included in the final packaged archive.
...
If you are deploying to [Lambda functions with ARM64 architecture](#lambda-functions-with-arm64-architectures) 您應該 update the `find` command to 以下:
...
您可以 now re-package and re-deploy your application. To do so, run `serverless deploy`. Webpack output will show the schema being moved with `copy-webpack-plugin`:
...
The SST guide [available here](https://v2.sst.dev/config#overview) is a step-by-step guide 開始使用 with `Config`. Assuming you have created a new secret called `DATABASE_URL` and have [bound that secret to your app](https://v2.sst.dev/config#bind-the-config), 您可以 set up `PrismaClient` with 以下:
