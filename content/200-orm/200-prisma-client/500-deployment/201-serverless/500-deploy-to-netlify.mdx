---
title: '部署 to Netlify'
metaTitle: '部署 to Netlify'
metaDescription: '學習如何 to 部署 Node.js and TypeScript applications that are 使用 Prisma 客戶端 to Netlify.'
---
...
We recommend keeping `.env` files in your `.gitignore` in order to prevent leakage of sensitive connection strings. Instead, 您可以 use the Netlify CLI to [import values into netlify directly](https://docs.netlify.com/environment-variables/get-started/#import-variables-with-the-netlify-cli).


Assuming you have a file like 以下:
...
您可以 upload the file as environment variables using the `env:import` command:
...
1. Open the Netlify admin UI for the site. 您可以 use Netlify CLI as follows:
   ```terminal
   netlify open --admin
   ```
...
您可以 now test the deployed application.
