---
title: 'Deploying 邊緣 functions with Prisma ORM'
metaTitle: '概述: 部署 Prisma ORM at the 邊緣'
metaDescription: '學習如何 to 部署 your Prisma-backed apps to 邊緣 functions like Cloudflare Workers or Vercel 邊緣 Functions'
sidebar_label: '概述'
tocDepth: 2
---

您可以 deploy an application that uses Prisma ORM to the edge. Depending on which edge function provider and which database you use, there are different considerations and things to be aware of.
...
- 何時使用 Prisma Postgres or Accelerate?

</details>

Here is a brief overview of all the edge function providers that are currently 支援 Prisma ORM:
...
- [Neon Serverless](https://neon.tech/docs/serverless/serverless-driver) uses HTTP to access the database. It 適用於 Cloudflare Workers and Vercel Edge Functions.
- [PlanetScale Serverless](https://planetscale.com/docs/tutorials/planetscale-serverless-driver) uses HTTP to access the database. It 適用於 Cloudflare Workers and Vercel Edge Functions.
- [`node-postgres`](https://node-postgres.com/) (`pg`) uses Cloudflare's `connect()` (TCP) to access the database. It is only 相容於 Cloudflare Workers, not with Vercel Edge Functions.
- [`@libsql/client`](https://github.com/tursodatabase/libsql-client-ts) is used to access Turso databases. It 適用於 Cloudflare Workers and Vercel Edge Functions.
- [Cloudflare D1](https://developers.cloudflare.com/d1/) is used to access D1 databases. It is only 相容於 Cloudflare Workers, not with Vercel Edge Functions.
...
您可以 use all of these drivers with Prisma ORM using the respective [driver adapters](/orm/overview/databases/database-drivers).

Depending on which deployment provider and database/driver you use, there may be special considerations. Please 查看 the deployment docs for your respective scenario to make sure 您可以 deploy your application successfully:
...
If you want to deploy an app using Turso, 您可以 follow the instructions [here](/orm/overview/databases/turso).
