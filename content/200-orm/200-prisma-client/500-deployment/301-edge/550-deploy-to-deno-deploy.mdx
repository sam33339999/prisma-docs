---
title: '部署 to Deno 部署'
metaTitle: '部署 to Deno 部署'
metaDescription: '學習如何 to 部署 a TypeScript application to Deno 部署.'
---



With this guide, 您可以 學習如何 build and deploy a simple application to [Deno Deploy](https://deno.com/deploy). The application uses Prisma ORM to save a log of each request to a [Prisma Postgres](/postgres) database.
...
設定 your application, open your terminal and navigate to a location of your choice. Then, run 以下 commands 設定 your application:
...
Prisma Client does not read `.env` files 預設情況下 on Deno, so 您必須 also install `dotenv-cli` locally:
...
With the data model in place and your database connection configured, 您可以 now apply the data model to your database.
...
您可以 now create a local Deno application. Create `index.ts` in the root folder of your project and add the content below:
...
You need a GitHub repository 添加 your project to Deno Deploy and enable automated deployments whenever you push changes.

設定 a GitHub repository:

1. [Create a private GitHub repository](https://github.com/new).

2. Initialize your repository locally and push your changes to GitHub, with 以下 commands:
...
Use the GitHub repository 添加 your application to Deno Deploy:
...
The deployment should fail as you have 添加 the `DATABASE_URL` environment variable. 
...
You have 添加 some code and create another commit to trigger a re-dployment.

Add 以下 code in your `index.ts` file:
