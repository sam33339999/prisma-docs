...
The focus of this guide is showing 如何 deploy projects using Prisma ORM to Heroku. The starting point will be the [Prisma Heroku example](https://github.com/prisma/prisma-examples/tree/latest/deployment-platforms/heroku), which contains an Express.js server with a couple of preconfigured REST endpoints and a simple frontend.

> **注意：** The various **checkpoints** throughout the guide allowing you to validate whether you performed the steps correctly.

</TopBlock>

## A note on deploying GraphQL servers to Heroku

While the example uses REST, the same principles apply to a GraphQL server, with 主要 difference being that you typically have a single GraphQL API endpoint rather than a route for every resource as with REST.
...
> **注意：** Heroku doesn't provide a free plan, so billing information is required.
...
在本指南中, you will use [Prisma Migrate](/orm/prisma-migrate) 建立 the database schema. Prisma Migrate is based on the Prisma schema and works by generating `.sql` migration files that are executed against the database.
...
> **注意：** The `Procfile` tells Heroku the command needed to start the application, i.e. `npm start`, and the command to run during the release phase, i.e., `npx prisma migrate deploy`

## 2. Create a Git repository for the application

In the previous step, you downloaded the code. In this step, you will create a repository from the code so that 您可以 push it to Heroku for deployment.
...
使用 the `main` branch as the default branch, run 以下 command:
...
這將 allow you to deploy to Heroku from the terminal.
...
To deploy an application to Heroku, 您需要 create an app. 您可以 do so with 以下 command:

```no-lines
heroku apps:create your-app-name
```

> **注意：** Use a unique name of your choice instead of `your-app-name`.

**Checkpoint:** 您應該 參見 URL and the repository for your Heroku app:
...
If you don't 參見 heroku remote, use 以下 command 添加 it:
...
Create the database with 以下 command:

```no-lines
heroku addons:create heroku-postgresql:hobby-dev
```

**Checkpoint:** To verify the database was created 您應該 see 以下:
...
> **注意：** Heroku automatically sets the `DATABASE_URL` environment variable when the app is running on Heroku. Prisma ORM uses this environment variable because it's declared in the _datasource_ block of the Prisma schema (`prisma/schema.prisma`) with `env("DATABASE_URL")`.
...
這將 trigger a build and deploy your application to Heroku. Heroku will also run the `npx prisma migrate deploy` command which executes the migrations 建立 the database schema before deploying the app (as defined in the `release` step of the `Procfile`).
...
> **注意：** Heroku will also set the `PORT` environment variable to which your application is bound.

## 7. Test your deployed application

您可以 use the static frontend to interact with the API you deployed via the preview URL.

Open up the preview URL in your browser, the URL should like this: `https://APP_NAME.herokuapp.com`. 您應該 see 以下:
...
您可以 view the application's logs with the `heroku logs --tail` command:
...
您可以 find the source code for the example in [this GitHub repository](https://github.com/prisma/prisma-examples/tree/latest/deployment-platforms/heroku).
