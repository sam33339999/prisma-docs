---
title: '部署 to Render'
metaTitle: '部署 a Prisma app to Render'
metaDescription: '學習如何 to 部署 a Node.js server that uses Prisma ORM to Render.'
---


本指南說明如何 deploy a Node.js server that uses Prisma ORM and PostgreSQL to Render.

<details>
<summary>Questions answered in this page</summary>

- 如何 deploy Prisma apps on Render?
- 如何 run migrations before start?
...
- Render lets you deploy long-running, "serverful" full-stack applications. 您可以 configure Render services to [autoscale](https://docs.render.com/scaling) based on CPU and/or memory usage. This is one of several [deployment paradigms](/orm/prisma-client/deployment/deploy-prisma) 您可以 choose from.
- Render natively supports [common runtimes](https://docs.render.com/language-support), including Node.js and Bun. 在本指南中, we'll use the Node.js runtime.
- Render [integrates with Git repos](https://docs.render.com/github) for automatic deployments upon commits. 您可以 deploy to Render from GitHub, GitLab, or Bitbucket. 在本指南中, we'll deploy from a Git repository.
...
Before we deploy the app, let's 查看 the example code.
...
- `prisma/migrations/<migration name>/migration.sql`: The SQL commands that construct this schema in a PostgreSQL database. 您可以 auto-generate migration files like this one by running [`prisma migrate dev`](/orm/prisma-migrate/understanding-prisma-migrate/mental-model#what-is-prisma-migrate).

### Render Blueprint

The `render.yaml` file is a [Render blueprint](https://docs.render.com/infrastructure-as-code). Blueprints are Render's Infrastructure as Code format. 您可以 use a Blueprint to programmatically create and modify services on Render.
...
您可以 use the Pre-Deploy Command setting in a Render deploy to run any commands, such as database migrations, before the app is started.
...
4. Provide 以下 values during service creation:

| **Setting**           | **Value**                    |
| :-------------------- | :--------------------------- |
| Language              | `Node`                       |
| Build Command         | `npm install --production=false` |
| Pre-Deploy Command (注意： this may be in the "Advanced" tab) | `npx prisma migrate deploy`  |
| Start Command         | `npm run start`              |
...
您可以 also deploy the example using the Render Blueprint. Follow Render's [Blueprint setup guide] and use the `render.yaml` in the example.
...
添加 these users to the database, we can either:
