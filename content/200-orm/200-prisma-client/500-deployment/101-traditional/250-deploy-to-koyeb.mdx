---
title: '部署 to Koyeb'
metaTitle: '部署 a Prisma ORM app to Koyeb'
metaDescription: '學習如何 to 部署 a Node.js server that uses Prisma ORM to Koyeb Serverless Platform.'
---

<TopBlock>

在本指南中, you will set up and deploy a Node.js server that uses Prisma ORM with PostgreSQL to [Koyeb](https://www.koyeb.com/). The application exposes a REST API and uses Prisma Client to handle fetching, creating, and deleting records from a database.
...
The application has 以下 components:
...
The focus of this guide is showing 如何 deploy projects using Prisma ORM to Koyeb. The starting point will be the [Prisma Koyeb example](https://github.com/koyeb/example-prisma), which contains an Express.js server with a couple of preconfigured REST endpoints and a simple frontend.

> **注意：** The various **checkpoints** throughout the guide allow you to validate whether you performed the steps correctly.

</TopBlock>

## 前置需求

- Hosted PostgreSQL database and a URL from which it can be accessed, e.g. `postgresql://username:password@your_postgres_db.cloud.com/db_identifier` (您可以 use Supabase, which offers a [free plan](https://dev.to/prisma/set-up-a-free-postgresql-database-on-supabase-to-use-with-prisma-3pk6)).
...
在本指南中, you will create the database schema with [Prisma Migrate](/orm/prisma-migrate) 建立 the database schema. Prisma Migrate is based on the Prisma schema and works by generating `.sql` migration files that are executed against the database.
...
To learn more about how migrations are created with Prisma Migrate, 查看 the [start from scratch guide](/getting-started/prisma-orm/quickstart/postgresql)
...
**Checkpoint:** Executing the `tree` command should show 以下 directories and files:
...
In the previous step, you downloaded the code. In this step, you will create a repository from the code so that 您可以 push it to a GitHub repository for deployment.
...
You land on the Koyeb App creation page where you are asked for information about the application to deploy such as the deployment method 使用, the repository URL, the branch to deploy, the build and run commands to execute.
...
> **注意：** If this is your first time using Koyeb, you will be prompted 安裝 the Koyeb app in your GitHub account.
...
**Checkpoint:** Once the build and deployment are completed, 您可以 access your application by clicking the App URL ending with koyeb.app in the Koyeb control panel. Once on the app page loads, Once the page loads, click on the **Check API status** button, which should return: `{"up":true}`

## 4. Test your deployed application

您可以 use the static frontend to interact with the API you deployed via the preview URL.

Open up the preview URL in your browser, the URL should like this: `https://APP_NAME-ORG_NAME.koyeb.app`. 您應該 see 以下:
...
您可以 view the application's logs clicking the `Runtime logs` tab on your app service from the Koyeb control panel:
...
預設情況下, for applications using the Node.js runtime, if the `package.json` contains a `build` script, Koyeb automatically executes it after the dependencies installation.
In the example, the `build` script is used to run `prisma generate && prisma migrate deploy && next build`.

### Deployment

預設情況下, for applications using the Node.js runtime, if the `package.json` contains a `start` script, Koyeb automatically executes it to launch the application.
...
在本指南中, we kept pre-set values for the region, instance size, and horizontal scaling. 您可以 customize them according to your needs.

> **注意：** The Ports section is used to let Koyeb know which port your application is listening to and properly route incoming HTTP requests. A default `PORT` environment variable is set to `8080` and incoming HTTP requests are routed to the `/` path when creating a new application.
> If your application is listening on another port, 您可以 define another port to route incoming HTTP requests.

## Summary

Congratulations! You have successfully deployed a Node.js app with Prisma ORM to Koyeb.

您可以 find the source code for the example in [this GitHub repository](https://github.com/koyeb/example-prisma).
