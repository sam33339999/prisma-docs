---
title: 'Deploying 資料庫 changes with Prisma Migrate'
metaDescription: '學習如何 to 部署 資料庫 changes with Prisma Migrate.'
sidebar_label: 'Deploying 資料庫 changes'
---
...
Exactly when to run `prisma migrate deploy` depends on your platform. 例如, a simplified [Heroku](/orm/prisma-client/deployment/traditional/deploy-to-heroku) workflow includes:
...
Ideally, `migrate deploy` should be part of an automated CI/CD pipeline, and we do not generally recommend running this command locally to deploy changes to a production database (例如, by temporarily changing the `DATABASE_URL` environment variable). It is not generally considered good practice to store the production database URL locally.
...
欲了解更多資訊 about the `migrate deploy` command, see:
...
As part of your CI/CD, 您可以 run `prisma migrate deploy` as part of your pipeline to apply pending migrations to your production database.
