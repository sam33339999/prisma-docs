...
metaDescription: "學習 如何 use Prisma ORM without Rust engines"
---
...
This page gives an overview of 如何使用 this version of Prisma ORM.

## Prisma ORM without Rust engines

主要 technical differences if you're using Prisma ORM without a Rust engine are:
...
Depending on the database you use, 您需要 install a different driver adapter library:
...
Finally, instantiate Prisma Client which 您可以 do using the driver adapter and the connection URL for the database instance you're using:
...
If you went through the previous steps, 您可以 query your database as you're used to with Prisma Client. No other changes are needed.

## Usage with Prisma Accelerate or Prisma Postgres

When using the Rust-free version of Prisma ORM with [Prisma Accelerate](/accelerate) or [Prisma Postgres](/postgres), you **should not** use driver adapters. Instead, 您可以 directly instantiate Prisma Client with the appropriate extension.
...
Usage of the new architecture 需要 the `driverAdapters` and `queryCompiler` feature flags to be set:
...
Finally, 您需要 instantiate Prisma Client which 您可以 do using the driver adapter and the connection URL for the database instance you're using.
