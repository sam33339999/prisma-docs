...
title: '連線 管理'
metaTitle: '連線 管理'
metaDescription: '本頁說明 how 資料庫 connections are handled with Prisma 客戶端 and 如何 manually 連接 and 斷開 your 資料庫.'
tocDepth: 3
---

:::info Quick summary
本頁說明 how Prisma Client manages database connections, including how and 何時使用 the `$connect()` and `$disconnect()` methods, connection pooling behavior, and best practices for both long-running and serverless environments.
:::


<TopBlock>

`PrismaClient` connects and disconnects from your data source using 以下 two methods:
...
參見 [connection management guide](/orm/prisma-client/setup-and-configuration/databases-connections) for information about managing connections for different deployment paradigms (long-running processes and serverless functions).

<details>
<summary>Questions answered in this page</summary>

- When should I call $connect and $disconnect?
- 如何 Prisma manage connection pools?
- 如何 handle connections in serverless?
...
If you need the first request to respond instantly and cannot wait for a lazy connection to be established, 您可以 explicitly call `prisma.$connect()` to establish a connection to the data source:
...
One scenario where 您應該 call `$disconnect()` explicitly is where a script:

1. Runs **infrequently** (例如, a scheduled job to send emails each night), which means it does not benefit from a long-running connection to the database _and_
...
以下 script creates a new instance of `PrismaClient`, performs a task, and then disconnects - which closes the connection pool:
...
The `beforeExit` hook runs when Prisma ORM is triggered externally (e.g. via a `SIGINT` signal) to shut down, and allows you to run code _before_ Prisma Client disconnects - 例如, to issue queries as part of a graceful shutdown of a service:
