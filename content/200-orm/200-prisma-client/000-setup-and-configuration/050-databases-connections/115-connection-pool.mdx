---
title: 連線 pool
metaDescription: Prisma ORM's 查詢 引擎 creates a 連線 pool to store and 管理 資料庫 connections.
tocDepth: 4
toc_max_heading_level: 4
---

:::info Quick summary
本頁說明 how Prisma ORM manages database connections using a connection pool, and how 您可以 configure limits and timeouts for optimal performance.
:::
...
Starting with Prisma ORM v7, relational datasources instantiate Prisma Client with [driver adapters](/orm/overview/databases/database-drivers) 預設情況下. Driver adapters rely on the Node.js driver you supply, so connection pooling defaults (and configuration) now come from the driver itself. 
...
以下 tables document the default connection pool settings for each driver adapter.

:::tip[Prisma timeouts]

Prisma ORM also has its own configurable timeouts that are separate from the database driver timeouts. If you see a timeout error and are unsure whether it comes from the driver or from Prisma Client, 參見 [Prisma Client timeouts and transaction options documentation](/orm/prisma-client/queries/transactions#transaction-options).
...
:::tip
參見 [node-postgres pool documentation](https://node-postgres.com/apis/pool) for details on every available option.
:::
...
:::tip
參閱 the [MariaDB Connector/Node.js pool options](https://mariadb.com/docs/connectors/mariadb-connector-nodejs/connector-nodejs-promise-api#pool-options) for configuration and tuning guidance.
:::
...
:::tip
參見 [`node-mssql` pool docs](https://tediousjs.github.io/node-mssql/#general-same-for-all-drivers) for details on these fields.
:::
...
您可以not increase the `connection_limit` beyond what the underlying database can support. This is a particular challenge in serverless environments, where each function manages an instance of `PrismaClient` - and its own connection pool.
...
以下 steps describe how the query engine uses the connection pool:
...
1. If there are no idle connections 可用於 the connection pool, the query engine opens additional database connections and adds them to the connection pool until the number of database connections reaches the limit defined by `connection_limit`.
...
If you consistently experience pool timeout errors, 您需要 [optimize the connection pool](/orm/prisma-client/setup-and-configuration/databases-connections#optimizing-the-connection-pool) .

### 連線池 size

#### Default connection pool size

The default number of connections (pool size) is calculated with 以下 formula:
...
您可以 specify the number of connections by explicitly setting the `connection_limit` parameter in your database connection URL. 例如, with 以下 `datasource` configuration in your [Prisma schema](/orm/prisma-schema) the connection pool will have exactly five connections:
...
Using the `info` [logging level](/orm/reference/prisma-client-reference#log-levels), 您可以 log the number of connections in a connection pool that are opened when Prisma Client is instantiated.

例如, consider 以下 Prisma Client instance and invocation:
...
請注意 輸出 generated by `log: ['info']` can change in any release without notice. Be aware of this in case you are relying on 輸出 in your application or a tool that you're building.

:::

If you need even more insights into the size of your connection pool and the amount of in-use and idle connection, 您可以 use the [metrics](/orm/prisma-client/observability-and-logging/metrics) feature (which is currently in Preview).
...
For more details on 什麼是 可用於 the metrics output, 參見 [About metrics](/orm/prisma-client/observability-and-logging/metrics#about-metrics) section.
...
您可以 specify the pool timeout by explicitly setting the `pool_timeout` parameter in your database connection URL. In 以下範例, the pool times out after `2` seconds:
...
您可以 choose to [disable the connection pool timeout if queries **must** remain in the queue](/orm/prisma-client/setup-and-configuration/databases-connections#disabling-the-pool-timeout) - 例如, if you are importing a large number of records in parallel and are confident that the queue will not use up all available RAM before the job is complete.
