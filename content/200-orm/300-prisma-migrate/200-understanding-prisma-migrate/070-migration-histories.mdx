---
title: '關於 遷移 histories'
metaTitle: '關於 遷移 histories'
metaDescription: '關於 遷移 histories'
tocDepth: 3
---

<TopBlock>

本頁說明 how Prisma ORM uses migration histories to track changes to your schema.
...
- A `_prisma_migrations` table in the database, which is used to check:
  ...
  If you change or delete a migration (**not** recommended), the 後續步驟 depend on whether you are in a [development environment](/orm/prisma-migrate/workflows/development-and-production#development-environments) (and therefore using `migrate dev`) or a [production / testing environment](/orm/prisma-migrate/workflows/development-and-production#production-and-testing-environments) (and therefore using `migrate deploy`).
...
以下 scenario simulates a change that creates a seemingly harmless inconsistency:
...
A change that does not appear to break anything after a `migrate reset` can hide problems - you may end up with a bug in production that 您可以not replicate in development, or the other way around - particularly if the change concerns a highly customized migration.
...
您必須 commit the entire `prisma/migrations` folder to source control. This includes the `prisma/migrations/migration_lock.toml` file, which is used to detect if you have [attempted to change providers](/orm/prisma-migrate/understanding-prisma-migrate/limitations-and-known-issues#you-cannot-automatically-switch-database-providers).

Source-controlling the `schema.prisma` file is not enough - 您必須 include your migration history. This is because:

- As you start to [customize migrations](/orm/prisma-migrate/workflows/development-and-production#customizing-migrations), your migration history contains **information that cannot be represented in the Prisma schema**. 例如, 您可以 customize a migration to mitigate data loss that would be caused by a breaking change.
