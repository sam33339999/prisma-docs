---
title: '舊版 Prisma Migrate'
metaTitle: '舊版 Prisma Migrate (參考)'
metaDescription: '舊版 Prisma Migrate is a declarative 資料 modeling and 綱要 遷移 tool that is available via the Prisma CLI.'
tocDepth: 3
unlisted: true
---
...
> **Important!** This page documents legacy Prisma Migrate (Experimental) 可用於 version 2.12.0 and earlier. [Prisma Migrate](/orm/prisma-migrate) is 可用於 version [2.13.0](https://github.com/prisma/prisma/releases/tag/2.13.0) and Generally 可用於 [2.19.0](https://github.com/prisma/prisma/releases/tag/2.19.0).
...
### 舊版 Prisma Migrate vs the `db push` command
...
### 舊版 Prisma Migrate vs SQL migrations
...
Here's a quick comparison. Assume you have 以下 scenario:

1. 您需要 create the `User` table to store user information (name, email, ...)
...
以下 table shows which SQL operations are currently 支援 legacy Prisma Migrate.
...
請注意 this table assumes that the operation is also 支援 the underlying database. 例如, `ENUM` is not supported in SQLite. This means that you also can't use `enum` when using legacy Prisma Migrate.
...
The `migrations` directory stores information about the migrations that have been or will be executed against your database. 您應該 never make any manual changes to the files in `migrations`. The only way to change the content of this directory should be using the `legacy Prisma Migrate save` command.
...
> 警告： The `migrate` command is still considered Experimental. As such, there are no guarantees about API stability or production-readiness. Access to this command is provided for evaluation and experimentation. To access Experimental commands, 您必須 add the `--experimental` flag.

The `migrate` command creates and manages database migrations. It can be used 建立, apply, and rollback database schema updates in a controlled manner.

The `migrate` command includes a number of subcommands to specify the desired action.

### `migrate save`

Saves a migration that defines the steps necessary 更新 your current schema.

#### 前置需求

Before using the `migrate save` command, 您必須 define a valid [`datasource`](/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.

例如, 以下 `datasource` defines a SQLite database file within the current directory:
...
The `migrate save` command recognizes 以下 options to modify its behavior:
...
The `migrate save` command generates 以下 directories and files as necessary:
...
##### Create a 遷移 with a specific name

```terminal
prisma migrate save --name "First migration" --experimental
```

##### Create the 資料庫 if it does not already exist

```terminal
prisma migrate save --create-db --experimental
```

##### 預覽 the 遷移 that would be created by running the `migrate save` command
...
Before using the `migrate up` command, 您必須 define a valid [`datasource`](/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.
...
The point to migrate the database up to can be defined in any of 以下 three ways:
...
Additionally, 以下 options modify the behavior of the `migrate up` command:
...
##### Migrate the 資料庫 up to the latest available 遷移

```terminal
prisma migrate up --experimental
```

##### Apply the next two migrations to the 資料庫

```terminal
prisma migrate up 2 --experimental
```

##### Apply all migrations necessary up to and including a 遷移 by name

```terminal
prisma migrate up "First migration" --experimental
```

##### Apply all migrations necessary up to and including a 遷移 by timestamp

```terminal
prisma migrate up 20200223181448 --experimental
```

##### Create the 資料庫 if it does not already exist before applying the migrations

```terminal
prisma migrate up --create-db --experimental
```

##### 預覽 the 遷移 that would be applied by running the `migrate up` command
...
Before using the `migrate down` command, 您必須 define a valid [`datasource`](/orm/prisma-schema/overview/data-sources) within your `schema.prisma` file.
...
The point to migrate back to can be defined in any of 以下 three ways:
...
Additionally, 以下 options modify the behavior of the `migrate down` command:
...
##### Migrate the 資料庫 backwards by a single 遷移

```terminal
prisma migrate down --experimental
```

##### Migrate the 資料庫 backwards by two migrations

```terminal
prisma migrate down 2 --experimental
```

##### Migrate backwards through all migrations up to and including a 遷移 by name

```terminal
prisma migrate down "First migration" --experimental
```

##### Migrate backwards through all migrations up to and including a 遷移 by timestamp

```terminal
prisma migrate down 20200223181448 --experimental
```

##### 預覽 the 遷移 that would be applied by running the `migrate down` command
