...
metaTitle: 'Legacy Prisma Migrate (參考)'
metaDescription: 'Legacy Prisma Migrate is a declarative 資料 modeling and Schema 遷移 tool that is available via the Prisma CLI.'
tocDepth: 3
unlisted: true
---
...
##### 預覽 the migration that would be created by running the `migrate save` command
...
##### 預覽 the migration that would be applied by running the `migrate up` command
...
##### 預覽 the migration that would be applied by running the `migrate down` command
