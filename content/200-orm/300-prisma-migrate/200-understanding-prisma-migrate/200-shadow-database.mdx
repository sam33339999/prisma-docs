---
title: '關於 the shadow 資料庫'
metaTitle: '關於 the shadow 資料庫'
metaDescription: '關於 the shadow 資料庫'
---
...
* If your database does not allow creation and deleting of databases (e.g. in a cloud-hosted environment), 您需要 [create and configure the shadow database manually](#cloud-hosted-shadow-databases-must-be-created-manually).
...
When you run `prisma migrate dev` 建立 a new migration, Prisma Migrate uses the shadow database to:
...
1. Reports **schema drift** if the end state of the current migration history (via the shadow database) does not match the development database (例如, due to a manual change)
...
If Prisma Migrate detects schema drift, it outputs detailed information about which parts of the database have drifted. 以下範例 output could be shown when the development database has been modified manually: The `Color` enum is missing the expected variant `RED` and includes the unexpected variant `TRANSPARENT`:
...
In some cases it might make sense (e.g. when [creating and dropping databases is not allowed on cloud-hosted databases](#cloud-hosted-shadow-databases-must-be-created-manually)) to manually define the connection string and name of the database that should be used as the shadow database for `migrate dev`. In such a case 您可以:
...
Some cloud providers do not allow you to drop and create databases with SQL. Some require 建立 or drop the database via an online interface, and some really limit you to 1 database. If you **develop** in such a cloud-hosted environment, 您必須:
...
In order 建立 and delete the shadow database when using `migrate dev`, Prisma Migrate currently 需要 that the database user defined in your `datasource` has permission to **create databases**.

| Database             | Database user 需求                                                                                                                                                                                            |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SQLite               | No special 需求.                                                                                                                                                                                              |
...
| Microsoft SQL Server | The user must be a site admin or have the `SERVER` securable. 參見 [official documentation](https://learn.microsoft.com/en-us/sql/relational-databases/security/permissions-database-engine?view=sql-server-ver15). |
...
> 注意： The automatic creation of shadow databases is disabled on Azure SQL 例如.

Prisma Migrate throws 以下 error if it cannot create the shadow database with the credentials your connection URL supplied:
...
- If you are developing against a cloud-based database (例如, on Heroku, Digital Ocean, or Vercel Postgres) see: [Cloud-hosted shadow databases](#cloud-hosted-shadow-databases-must-be-created-manually).
- If you are developing against a cloud-based database (例如, on Heroku, Digital Ocean, or Vercel Postgres) and are currently **prototyping** such that you don't care about generated migration files and only need to apply your Prisma schema to the database schema, 您可以 run [`prisma db push`](/orm/reference/prisma-cli-reference#db) instead of the `prisma migrate dev` command.
