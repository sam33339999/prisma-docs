---
title: 壓縮 migrations
metaTitle: 壓縮 migrations
metaDescription: 如何 squash 多個 遷移 files into a single 遷移
toc_max_heading_level: 2
---

<TopBlock>

This guide describes 如何 squash multiple [migration files](/orm/prisma-migrate/understanding-prisma-migrate/migration-histories) into a single migration.
...
For detailed steps on 如何 achieve this using `migrate dev`, 參見 section on [如何 migrate cleanly from a development environment](#how-to-migrate-cleanly-from-a-development-environment).
...
For detailed steps on 如何 achieve this using `migrate diff` and `migrate resolve` 參見 section on [how 建立 a clean history in a production environment](#how-to-create-a-clean-history-in-a-production-environment).

## Considerations when 壓縮 migrations

:::warning

When squashing migrations, 請注意 any manually changed or added SQL in your `migration.sql` files will not be retained. If you have migration files with custom additions such as a view or a trigger, ensure to re-add them after your migrations were squashed.

:::

## 如何 squash migrations

This section provides step-by-step instructions on 如何 squash migrations in the two scenarios discussed above:
...
### 如何遷移 cleanly from a development environment

Before squashing your migrations, make sure you have 以下 starting conditions:
...
2. Create a new migration:
   ...
   這會建立 a single migration that takes you:
...
### 如何建立 a clean history in a production environment
...
2. Create a new empty directory in the `./prisma/migrations` directory. 在本指南中 這將 be called `000000000000_squashed_migrations`. Inside this, add a new empty `migration.sql` file.

   :::info

   We name the migration `000000000000_squashed_migrations` with all the leading zeroes because we want it to be the first migration in the migrations directory. Migrate runs the migrations in the directory in lexicographic (alphabetical) order. This is why it generates migrations with the date and time as a prefix when you use `migrate dev`. 您可以 give the migration another name, as long as it it sorts lower than later migrations, 例如 `0_squashed` or `202207180000_squashed`.
...
3. Create a single migration that takes you:
   ...
   您可以 do this using the `migrate diff` command. From the root directory of your project, run 以下 command:
...
4. Mark this migration as having been applied on production, to prevent it from being run there:
   ...
   您可以 do this using the [`migrate resolve`](/orm/reference/prisma-cli-reference#migrate-resolve) command to mark the migration in the `000000000000_squashed_migrations` directory as already applied:
...
您應該 now have a single migration file that is marked as having been applied on production. New checkouts only get one single migration taking them to the state of the production database schema.

The production database still contains the history of applied migrations in the migrations table. The history of the migrations folder and data models is also still 可用於 source control.
