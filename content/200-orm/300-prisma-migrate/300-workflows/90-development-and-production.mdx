...
title: '開發與生產'
metaTitle: '開發與生產'
metaDescription: '開發與生產'
tocDepth: 3
---

<TopBlock>

本頁說明 如何使用 Prisma Migrate commands in development and production environments.
...
The `migrate dev` command will prompt you to reset the database in 以下 scenarios:
...
### Reset the development 資料庫

您可以 also `reset` the database yourself to undo manual changes or `db push` experiments by running:
...
> **Note**: For a simple and integrated way to re-create data in your development database as often as needed, 查看 our [seeding guide](/orm/prisma-migrate/workflows/seeding).

### 自訂 migrations

Sometimes, 您需要 modify a migration **before applying it**. 例如：

- You want to introduce a significant refactor, such as changing blog post tags from a `String[]` to a `Tag[]`
- You want to [rename a field](/orm/prisma-migrate/workflows/customizing-migrations#example-rename-a-field) (預設情況下, Prisma Migrate will drop the existing field)
...
- You want 添加 features that cannot be represented in Prisma Schema Language - such as a partial index or a stored procedure.

The `--create-only` command allows you 建立 a migration without applying it:
...
參閱 [Customizing migrations](/orm/prisma-migrate/workflows/customizing-migrations) 例如s.
...
- **Does not** detect drift (production database schema differs from migration history end state - 例如, due to a hotfix)
...
This safeguard ensures that multiple commands cannot run at the same time - 例如, if you merge two pull requests in quick succession.

Advisory locking has a **10 second timeout** (not configurable), and uses the default advisory locking mechanism 可用於 the underlying provider:
