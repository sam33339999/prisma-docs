...
title: 疑難排解
metaTitle: 疑難排解
metaDescription: 疑難排解 issues with Prisma Migrate in a development 環境.
---

<TopBlock>

This guide describes 如何 resolve issues with Prisma Migrate in a development environment, which often involves resetting your database. For production-focused troubleshooting, see:
...
> **Note**: 您應該 [never purposefully delete or edit a migration](/orm/prisma-migrate/understanding-prisma-migrate/migration-histories#do-not-edit-or-delete-migrations-that-have-been-applied), as this might result in discrepancies between development and production.
...
- The database schema was changed _without_ using migrations - 例如, by using [`prisma db push`](/orm/reference/prisma-cli-reference#db-push) or manually changing the database schema.
...
If you made manual changes to the database that you want to keep, 您可以:
...
The easiest way to handle a failed migration in a developer environment is 添加ress the root cause and reset the database. 例如：
...
- If you introduced a change in the Prisma schema that cannot be applied to a database with data (例如, a mandatory column in a table with data):

  1. Delete the `migration.sql` file.

  2. Modify the schema - 例如, add a default value to the mandatory field.
