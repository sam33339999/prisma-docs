---
title: '原型設計 your 綱要'
metaTitle: '原型設計 your 綱要'
metaDescription: '原型設計 your 綱要'
codeStyle: false
---
...
2. 預設情況下, after changes have been applied to the database schema, generators are triggered (例如, Prisma Client). You do not need to manually invoke `prisma generate`.
...
> - When working with PlanetScale, we recommend that you use `db push` instead of `migrate`. For details 參閱 our Getting started documentation, either [Start from scratch guide](/getting-started/prisma-orm/quickstart/planetscale) or [Add to existing project guide](/getting-started/prisma-orm/add-to-existing-project/planetscale) depending on your situation.
...
- You do not need to control how schema changes impact data. There is no way to orchestrate schema and data migrations—if `db push` anticipates that changes will result in data loss, 您可以 either accept data loss with the `--accept-data-loss` option or stop the process. There is no way to customize the changes.

See [Schema prototyping with `db push`](/orm/prisma-migrate/workflows/prototyping-your-schema) for an example of 如何使用 `db push` in this way.

`db push` is **not recommended** if:

- You want to replicate your schema changes in other environments without losing data. 您可以 use `db push` for prototyping, but 您應該 use migrations to commit the schema changes and apply these in your other environments.
- You want fine-grained control over how the schema changes are executed - 例如, [renaming a column instead of dropping it and creating a new one](/orm/prisma-migrate/workflows/customizing-migrations#example-rename-a-field).
...
- You want the schema changes to be reversible. 您可以 use `db push` again to revert to the original state, but this might result in data loss.

## Can I use Prisma Migrate and `db push` together?

Yes, 您可以 [use `db push` and Prisma Migrate together in your development workflow](/orm/prisma-migrate/workflows/prototyping-your-schema) . 例如, 您可以:
...
以下 scenario demonstrates 如何使用 `db push` to synchronize a new schema with an empty database, and evolve that schema - including what happens when `db push` detects that a change will result in data loss.
...
4.  Make an additive change - 例如, create a new required field:
...
5.  Push the changes:
    ...
    `db push` will fail because 您可以not add a required field to a table with existing content unless you provide a default value.
...
6.  Reset **all data** in your database and re-apply migrations.
    ...
    > **Note**: Unlike Prisma Migrate, `db push` does not generate migrations that 您可以 modify to preserve data, and is therefore best suited for prototyping in a development environment.
...
## 原型設計 with an existing 遷移 history

以下 scenario demonstrates 如何使用 `db push` to prototype a change to a Prisma schema where a migration history already exists.

1. 查看 the latest Prisma schema and migration history:
...
2. Prototype your new feature, which can involve any number of steps. 例如, you might:
   ...
   - Make a manual change to the `tags` field in the database - 例如, adding a constraint
...
3. 建立 a migration that adds the new `tags` field, run the `migrate dev` command:
...
When using `migrate dev`, if your schema changes mean that seed scripts will no longer work, 您可以 use the `--skip-seed` flag to ignore seed scripts.
