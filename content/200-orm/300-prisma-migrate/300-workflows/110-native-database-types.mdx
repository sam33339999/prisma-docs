---
title: 原生 資料庫 types
metaTitle: 原生 資料庫 types
metaDescription: 原生 資料庫 types
...
Every¹ feature in your [data model](/orm/prisma-schema/data-model/models) maps to a corresponding feature in the underlying database. **If 您可以 define a feature in the Prisma schema, it is 支援 Prisma Migrate.**

For a complete list of Prisma schema features, 參閱:
...
Each Prisma ORM type maps to a default underlying database type - 例如, the PostgreSQL connector maps `String` to `text` 預設情況下. [Native database type attributes](/orm/prisma-schema/data-model/models#native-types-mapping) determines which _specific_ native type should be created in the database.
...
In 以下範例, the `name` and `title` fields have a `@db.VarChar(X)` type attribute:
...
For type mappings organized by Prisma ORM type, 參閱 the [Prisma schema reference](/orm/reference/prisma-schema-reference#model-field-scalar-types) documentation.
...
Prisma Migrate cannot automatically create database features that have no equivalent in Prisma Schema Language (PSL). 例如, there is currently no way to define a stored procedure or a partial index in PSL. However, there are ways 添加 unsupported features to your database with Prisma Migrate:
...
- [如何使用 native database functions](/orm/prisma-schema/data-model/unsupported-database-features#native-database-functions)
