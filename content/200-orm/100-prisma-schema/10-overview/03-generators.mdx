...
title: "產生器"
metaTitle: "產生器 (參考)"
metaDescription: "Prisma 綱要 中的產生器（產生器）指定了在呼叫 `prisma generate` 命令時會產生哪些資源。本頁說明如何配置產生器。"
---

一個 Prisma schema 可以有一個或多個產生器，由 [`generator`](/orm/reference/prisma-schema-reference#generator) 區塊表示：
...
產生器決定了當您執行 `prisma generate` 命令時會建立哪些資源。

Prisma Client 的預設產生器是 `prisma-client`，它輸出純 TypeScript 程式碼並_需要_一個自訂的 `output` 路徑（在[此處](https://www.prisma.io/blog/why-prisma-orm-generates-code-into-node-modules-and-why-it-ll-change)閱讀更多相關資訊）。

或者，您可以配置任何符合我們產生器規格的 npm 套件。

## `prisma-client`

新的 `prisma-client` 產生器在不同的 JavaScript 環境（如 ESM、Bun、Deno 等）中使用 Prisma ORM 時提供了更大的控制和靈活性。

它將 Prisma Client 產生到您應用程式程式碼庫中的自訂目錄，該目錄透過 `generator` 區塊上的 `output` 欄位指定。這讓您對產生的程式碼擁有完全的可見性和控制權。它還將產生的 Prisma Client 函式庫[分割](#importing-types)成多個檔案。

這個產生器確保您可以完全按照您想要的方式打包應用程式程式碼，而無需依賴隱藏或自動的行為。

與 `prisma-client-js` 相比，主要差異如下：

- 需要一個 `output` 路徑；不再「魔法般地」產生到 `node_modules` 中
- 不會在執行時載入 `.env`；請改用 `dotenv` 或手動設定環境變數
- 透過 `moduleFormat` 欄位支援 ESM 和 CommonJS
- 由於新增了[欄位](#field-reference)而更加靈活
- 輸出純 TypeScript，與您應用程式程式碼的其餘部分一樣進行打包

`prisma-client` 產生器自 [v6.16.0](https://pris.ly/releases/6.16.0) 起已正式發布（GA），並且自 Prisma ORM v7 起成為預設產生器。

### 開始使用

按照以下步驟在您的專案中使用新的 `prisma-client` 產生器。

#### 1. 在 `綱要.prisma` 中配置 `prisma-client` 產生器

更新您的 [`generator`](/orm/prisma-schema/overview/generators) 區塊：
...
**`output` 選項是必需的**，它告訴 Prisma ORM 將產生的 Prisma Client 程式碼放在哪裡。您可以選擇適合您專案結構的任何位置。例如，如果您有以下佈局：
...
那麼 `../src/generated/prisma` 會將產生的程式碼放在相對於 `schema.prisma` 的 `src/generated/prisma` 中。

#### 2. 產生 Prisma Client

執行以下命令來產生 Prisma Client：
...
新的產生器包含了 TypeScript 客戶端程式碼_和_[查詢引擎](/orm/more/under-the-hood/engines#the-query-engine-file)。將查詢引擎包含在版本控制中可能會在不同機器上導致相容性問題。為避免這種情況，請將產生的目錄加入到 `.gitignore`：
...
未來，當 [Prisma ORM 完全從 Rust 過渡到 TypeScript](https://www.prisma.io/blog/rust-to-typescript-update-boosting-prisma-orm-performance?utm_source=docs&utm_medium=inline_text) 時，您就可以安全地將產生的目錄包含在版本控制中。
...
Prisma Client is now ready 使用 in your project.

#### Importing generated 模型 types

If you're importing types generated for your models, 您可以 do so as follows:
...
If you're importing types generated for your enums, 您可以 do so as follows:
...
If 您需要 access generated types in your frontend code, 您可以 import them as follows:

```ts file=src/index.ts
import { Role } from "./generated/prisma/browser";
```

請注意 `./generated/prisma/browser` does not expose a `PrismaClient`.

### Field 參考

Use 以下 options in the `generator client { ... }` block. Only `output` is required. The other fields have defaults or are inferred from your environment and `tsconfig.json`.
...
您可以 still use the top level barrel files that export all types through a single import.
...
The old `prisma-client-js` generator created a `node_modules` package and used export maps to dynamically provide a browser compatible export of the generated Prisma Client library. As the new `prisma-client` generator creates direct TypeScript source code and no `package.json` file anymore, this approach is not possible. Hence 您需要 be explicit about your imports and whether things run on server or client! 

您可以 still wrap the generated code in a package and use a similar approach as with `prisma-client-js` on your own.
...
Isolated access 使用r defined enum types and values.
...
Provides shared utility types that 您應該 rarely directly need.
...
### 破壞性變更 from `prisma-client-js`

- 需要 an `output` path on the `generator` block
- No `Prisma.validator` function;  您可以 use TypeScript native [`satisfies`](https://www.prisma.io/blog/satisfies-operator-ur8ys8ccq7zb) keyword instead

### 範例

To see what the new `prisma-client` generator looks like in practice, 查看 our minimal and [ready-to-run examples](https://github.com/prisma/prisma-examples):
...
The `prisma-client-js` generator 需要 the `@prisma/client` npm package and generates Prisma Client into `node_modules`.
...
- `binaryTargets`: Engine binary targets for `prisma-client-js` (例如, `debian-openssl-1.1.x` if you are deploying to Ubuntu 18+, or `native` if you are working locally)
...
As an example, assume you're running **macOS** and you specify 以下 generator:
...
> **Note**: The `native` binary target is the default. 您可以 set it explicitly if you wish to include additional [binary targets](/orm/reference/prisma-schema-reference#binarytargets-options) for deployment to different environments.
...
以下 is a list of community created generators.
