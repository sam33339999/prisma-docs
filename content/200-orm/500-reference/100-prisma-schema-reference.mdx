---
title: "Prisma Schema 參考"
metaTitle: "Prisma Schema API"
metaDescription: "API 參考 文件 for the Prisma Schema Language (PSL)."
...
A `datasource` block accepts 以下 fields:

| Name                | Required | Type                                                                            | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------- | -------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`          | **Yes**  | String (`postgresql`, `mysql`, `sqlite`, `sqlserver`, `mongodb`, `cockroachdb`) | Describes which data source connectors 使用.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `url`               | **Yes**  | String (URL)                                                                    | **Deprecated in Prisma ORM v7.** Configure the connection URL in Prisma Config instead: see [`datasource.url`](/orm/reference/prisma-config-reference#datasourceurl). Existing schemas continue to work, but 您應該 migrate to Prisma Config.                                                                                                                                                                                                                                                                     |
...
| `extensions`        | No       | List of strings (PostgreSQL extension names)                                    | Allows you to [represent PostgreSQL extensions in your schema](/orm/prisma-schema/postgresql-extensions). 可用於 preview for PostgreSQL only in Prisma ORM versions 4.5.0 and later.                                                                                                                                                                                                                                                              |
...
- 您可以 only have **one** `datasource` block in a schema.
- `datasource db` is convention - however, 您可以 give your data source any name - 例如, `datasource mysql` or `datasource data`.

### Examples

#### Specify a PostgreSQL data source

In this example, the target database is available with 以下 credentials:
...
When running a Prisma CLI command that needs the database connection URL (e.g. `prisma generate`), 您需要 make sure that the `DATABASE_URL` environment variable is set.

One way to do so is by creating a [`.env`](https://github.com/motdotla/dotenv) file with 以下 contents. 請注意 the file must be in the same directory as your `schema.prisma` file to automatically picked up the Prisma CLI.
...
A `generator` block accepts 以下 fields:

| Name              | Required | Type                                     | Description                                                                                                                                                                                                |
| :---------------- | :------- | :--------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | 
| `provider`        | **Yes**  | `prisma-client-js`                       | Describes which [generator](/orm/prisma-schema/overview/generators) 使用. This can point to a file that implements a generator or specify a built-in generator directly.                                 |
...
| Name                     | Required | Type                                                                                                                                        | Description                                                                                                                                                                                                                 |
| :----------------------- | :------- | :------------------------------------------------------------------------------------------------------------------------------------------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider`               | **Yes**  | `prisma-client`                                                                                                                            | Describes which [generator](/orm/prisma-schema/overview/generators) 使用. This can point to a file that implements a generator or specify a built-in generator directly.                                                  |
...
| `moduleFormat`           | No       | Enum (`esm` or `cjs`)                                                                                                                       | Determines whether the generated code supports ESM (uses `import`) or CommonJS (uses `require(...)`) modules. We always recommend `esm` unless you have a good reason 使用 `cjs`. **Default**: Inferred from environment. |
...
以下 tables list all supported operating systems with the name of platform to specify in [`binaryTargets`](/orm/prisma-schema/overview/generators#binary-targets).
...
\* 可用於 Prisma ORM versions 4.8.0 and later.
...
\* 可用於 Prisma ORM versions 4.10.0 and later.
...
請注意 the above `generator` definition is **equivalent** to 以下 because it uses the default values for `output`, `engineType` and `binaryTargets` (and implicitly `previewFeatures`):
...
This example shows 如何 define a custom `output` location of the generated asset to override the default one.
...
This example shows how 配置 Prisma Client to run on `Ubuntu 19.04 (disco)` based on the table [above](#linux-ubuntu-x86_64).
...
This example shows 如何使用 a custom generator that's located in a directory called `my-generator`.
...
- Every record of a model must be _uniquely_ identifiable. 您必須 define _at least_ one of 以下 attributes per model:
  - [`@unique`](#unique)
  - [`@@unique`](#unique-1)
  - [`@id`](#id)
  - [`@@id`](#id-1)
...
- Model names must adhere to 以下 regular expression: `[A-Za-z][A-Za-z0-9_]*`
- Model names must start with a letter and are typically spelled in [PascalCase](https://wiki.c2.com/?PascalCase)
- Model names should use the singular form (例如, `User` instead of `user`, `users` or `Users`)
- Prisma ORM has a number of **reserved words** that are being used by Prisma ORM internally and therefore cannot be used as a model name. 您可以 find the reserved words [here](https://github.com/prisma/prisma/blob/6.5.0/packages/client/src/generation/generateClient.ts#L556-L605) and [here](https://github.com/prisma/prisma-engines/blob/main/psl/parser-database/src/names/reserved_model_names.rs#L44).

> **Note**: 您可以 use the [`@@map` attribute](#map-1) to map a model (例如, `User`) to a table with a different name that does not match model naming conventions (例如, `users`).
...
- Must adhere to 以下 regular expression: `[A-Za-z][A-Za-z0-9_]*`

> **Note**: 您可以 use the [`@map` attribute](#map) to [map a field name to a column](/orm/prisma-client/setup-and-configuration/custom-model-and-field-names) with a different name that does not match field naming conventions: e.g. `myField @map("my_field")`.
...
您可以 use Prisma Migrate to map `@db.Bit(1)` to `String`:
...
請注意 the `xml` and `citext` types supported in PostgreSQL are not currently supported in CockroachDB.
...
| Native database types | Native database type attribute | Notes                                                                                                                                                     |
| --------------------- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TINYINT(1)`          | `@db.TinyInt(1)`               | `TINYINT` maps to `Int` if the max length is greater than 1 (例如, `TINYINT(2)`) _or_ the default value is anything other than `1`, `0`, or `NULL` |
| `BIT(1)`              | `@db.Bit`                      |
...
| `TINYINT`             | `@db.TinyInt`                  | `TINYINT` maps to `Int` if the max length is greater than 1 (例如, `TINYINT(2)`) _or_ the default value is anything other than `1`, `0`, or `NULL`. `TINYINT(1)` maps to `Boolean`. |
...
| Native database types        | Native database type attribute       | Notes                                                                                                             |
| ---------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| `INTEGER` \| `INT` \| `INT8` | `@db.Int8`                           | 請注意 this differs from PostgreSQL, where `integer` and `int` are aliases for `int4` and map to `@db.Integer` |
| `INT4`                       | `@db.Int4`                           |                                                                                                                   |
...
`BigInt` is 可用於 version [2.17.0](https://github.com/prisma/prisma/releases/tag/2.17.0) and later.
...
| Native database types       | Native database type attribute       | Notes                                                                      |
| --------------------------- | ------------------------------------ | -------------------------------------------------------------------------- |
| `BIGINT` \| `INT` \| `INT8` | `@db.Int8`                           | 請注意 this differs from PostgreSQL, where `int` is an alias for `int4` |
...
> `Float` maps to `Double` in [2.17.0](https://github.com/prisma/prisma/releases/tag/2.17.0) and later - see [release notes](https://github.com/prisma/prisma/releases/tag/2.17.0) and [Video: Changes to the default mapping of Float in Prisma ORM 2.17.0](https://www.youtube.com/watch?v=OsuGP_xNHco&amp%3Bab_channel=Prisma) 欲了解更多資訊 about this change.
...
| `money`                         | Not yet                        | PostgreSQL's `money` type is not yet 支援 CockroachDB |
...
您可以 find more info and examples 在本節中: [Working with `DateTime`](/orm/prisma-client/special-fields-and-types#working-with-datetime).
...
您可以 also use MySQL's `YEAR` type with `Int`:
...
`NUMERIC` or `STRING`. If the underlying data type is `STRING`, 您必須 use one of 以下 formats:
...
`Bytes` is 可用於 version [2.17.0](https://github.com/prisma/prisma/releases/tag/2.17.0) and later.
...
The `Unsupported` type was introduced in [2.17.0](https://github.com/prisma/prisma/releases/tag/2.17.0) and allows you to represent data types in the Prisma schema that are not 支援 Prisma Client. Fields of type `Unsupported` can be created during Introspection with `prisma db pull` or written by hand, and created in the database with Prisma Migrate or `db push`.

#### Remarks

- Fields with `Unsupported` types are not 可用於 the generated client.
- If a model contains a **required** `Unsupported` type, `prisma.model.create(..)`, `prisma.model.update(...)` and `prisma.model.upsert(...)` are not 可用於 Prisma Client.
- When you introspect a database that contains unsupported types, Prisma ORM will provide 以下 警告：
...
- Cannot be used with a list field (例如, `Posts[]`)
...
Attributes modify the behavior of a [field](#model-fields) or block (e.g. [models](#model)). There are two ways 添加 attributes to your data model:
...
- Can be defined on any scalar field (`String`, `Int`, `enum`) unless you want 使用 `ObjectId` in your database
- 使用 an [`ObjectId`](https://www.mongodb.com/docs/manual/reference/method/ObjectId/) as your ID, 您必須:
...
In most cases, you want your database 建立 the ID. To do this, annotate the ID field with the `@default` attribute and initialize the field with a [function](#attribute-functions).
...
您可以not use `cuid()` to generate a default value if your `id` field is of type `ObjectId`. Use 以下 syntax to generate a valid `ObjectId`:
...
您可以not use `uuid()` to generate a default value if your `id` field is of type `ObjectId`. Use 以下 syntax to generate a valid `ObjectId`:
...
您可以not use `ulid()` to generate a default value if your `id` field is of type `ObjectId`. Use 以下 syntax to generate a valid `ObjectId`:
...
In 以下範例, `id` does not have a default value:
...
請注意 in the above case, you _must_ provide your own ID values when creating new records for the `User` model using Prisma Client, e.g.:
...
In 以下範例, `authorId` is a both a relation scalar and the ID of `Profile`:
...
In this scenario, 您可以not create a `Profile` only - 您必須 use Prisma Client's [nested writes](/orm/prisma-client/queries/relation-queries#nested-writes) create a `User` **or** connect the profile to an existing user.

以下範例 creates a user and a profile:
...
以下範例 connects a new profile to a user:
...
- Can be defined on any scalar field (`String`, `Int`, `enum`)
- Cannot be defined on a relation field
- The name of the composite ID field in Prisma Client has 以下 pattern: `field1_field2_field3`

#### Arguments

| Name        | Required | Type               | Description                                                                                                                                                                                                                                                    |
| ----------- | -------- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fields`    | **Yes**  | `FieldReference[]` | A list of field names - 例如, `["firstname", "lastname"]`                                                                                                                                                                                               |
...
When you create a user, 您必須 provide a unique combination of `firstName` and `lastName`:
...
- Default values can be a static value (`4`, `"hello"`) or one of 以下 [functions](#attribute-functions):
  - [`autoincrement()`](#autoincrement)
...
- Default values are not allowed on relation fields in the Prisma schema. Note however that 您可以 still define default values on the fields backing a relation (the ones listed in the `fields` argument in the `@relation` attribute). A default value on the field backing a relation will mean that relation is populated automatically for you.
- Default values can be used with [scalar lists](/orm/prisma-client/special-fields-and-types/working-with-scalar-lists-arrays) in databases that natively support them.
- JSON data. 請注意 JSON needs to be enclosed with double-quotes inside the `@default` attribute, e.g.: `@default("[]")`. If you want to provide a JSON object, 您需要 enclose it with double-quotes and then escape any internal double quotes using a backslash, e.g.: `@default("{ \"hello\": \"world\" }")`.
...
請注意 static default values for `DateTime` are based on the [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) standard.
...
- All fields that make up the unique constraint **must** be mandatory fields. 以下 model is **not** valid because `id` could be `null`:
...
- A `@@unique` block cannot be used as the only unique identifier for a model - MongoDB 需要 an `@id` field

#### Arguments

| Name        | Required | Type               | Description                                                                                                                                                                                                                                           |
| ----------- | -------- | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fields`    | **Yes**  | `FieldReference[]` | A list of field names - 例如, `["firstname", "lastname"]`. Fields must be mandatory - see remarks.                                                                                                                                             |
...
While 您可以not configure these option in your Prisma schema, 您可以 still configure them on the database-level directly.

:::

##### MongoDB

- In version `3.12.0` and later, 您可以 define an index on a field of a [composite type](/orm/prisma-schema/data-model/models#defining-composite-types) using the syntax `@@index([compositeType.field])`. See [Defining composite type indexes](/orm/prisma-schema/data-model/models#defining-composite-type-indexes) for more details.
...
| `fields`    | **Yes**  | `FieldReference[]`           | A list of field names - 例如, `["firstname", "lastname"]`                                                                                                                                                                                                                                                         |
| `name`      | **No**   | `String`                     | The name that Prisma Client will expose for the argument covering all fields, e.g. `fullName` in `fullName: { firstName: "First", lastName: "Last"}`                                                                                                                                                                     |
| `map`       | **No**   | `map`                        | The name of the index in the underlying database (Prisma generates an index name that respects identifier length limits if you do not specify a name. Prisma uses 以下 naming convention: `tablename.field1_field2_field3_unique`)                                                                              |
| `length`    | **No**   | `number`                     | Allows you to specify a maximum length for the subpart of the value to be indexed.<br /><br />MySQL only. In preview in versions 3.5.0 and later, and in general availability in versions 4.0.0 and later.                                                                                                               |
...
Assume you want 添加 an index for the `title` field of the `Post` model
...
- If your model's primary key is of type `ObjectId` in the underlying database, both 主要 key _and_ the foreign key must have the `@db.ObjectId` attribute
...
Your `@id` field must include `@map("_id")`. 例如：
...
If you pass an empty update clause, the @updatedAt value will remain unchanged. 例如：
...
Add `@ignore` to a field that you want to exclude from Prisma Client (例如, a field that you do not want Prisma Client users 更新). Ignored fields are excluded from the generated Prisma Client. The model's `create` method is disabled when doing this for _required_ fields with no `@default` (because the database cannot create an entry without that data).

#### Remarks

- In [2.17.0](https://github.com/prisma/prisma/releases/tag/2.17.0) and later, Prisma ORM automatically adds `@ignore` to fields that _參閱_ invalid models when you introspect.

#### Examples

以下範例 demonstrates manually adding `@ignore` to exclude the `email` field from Prisma Client:
...
Add `@@ignore` to a model that you want to exclude from Prisma Client (例如, a model that you do not want Prisma users 更新). Ignored models are excluded from the generated Prisma Client.
...
In 以下範例, the `Post` model is invalid because it does not have a unique identifier. Use `@@ignore` to exclude it from the generated Prisma Client API:
...
欲了解更多資訊 about using the `multiSchema` feature, 參閱 [this guide](/orm/prisma-schema/data-model/multi-schema).
...
:::note
This features 需要 the `shardKeys` Preview feature flag on your `generator`:
```prisma
generator client {
  provider = "prisma-client"
...
The `@shardKey` attribute is only 相容於 [PlanetScale](http://planetscale.com/) databases. It enables you define a [shard key](https://planetscale.com/docs/vitess/sharding) on a field of your model:
...
The `@shardKey` attribute is only 相容於 [PlanetScale](http://planetscale.com/) databases. It enables you define a [shard key](https://planetscale.com/docs/vitess/sharding) on multiple fields of your model:
...
- 相容於 `Int` on most databases (`BigInt` on CockroachDB)
...
**Only 支援 CockroachDB** <br />
The sequence function is only 支援 [CockroachDB connector](/orm/overview/databases/cockroachdb).
...
- For **MongoDB**: `cuid()` does not generate a valid `ObjectId`. 您可以 use [`@db.ObjectId` syntax](#generate-objectid-as-ids-mongodb-only) if you want 使用 `ObjectId` in the underlying database. However, 您可以 still use `cuid()` if your `_id` field is not of type `ObjectId`.
...
- 相容於 `String`.
- Implemented by Prisma ORM and therefore not "visible" in the underlying database schema. 您可以 still use `uuid()` when using [introspection](/orm/prisma-schema/introspection) by [manually changing your Prisma schema](/orm/prisma-client/setup-and-configuration/custom-model-and-field-names) and [generating Prisma Client](/orm/prisma-client/setup-and-configuration/generating-prisma-client), in that case the values will be generated by Prisma ORM's [query engine](/orm/more/under-the-hood/engines).
- For **relational databases**: If you do not want 使用 Prisma ORM's `uuid()` function, 您可以 use [the native database function with `dbgenerated`](#override-default-value-behavior-for-supported-types).
- For **MongoDB**: `uuid()` does not generate a valid `ObjectId`. 您可以 use [`@db.ObjectId` syntax](#generate-objectid-as-ids-mongodb-only) if you want 使用 `ObjectId` in the underlying database. However, 您可以 still use `uuid()` if your `_id` field is not of type `ObjectId`.
...
- 相容於 `String`.
- Implemented by Prisma ORM and therefore not "visible" in the underlying database schema. 您可以 still use `uuid()` when using [introspection](/orm/prisma-schema/introspection) by [manually changing your Prisma schema](/orm/prisma-client/setup-and-configuration/custom-model-and-field-names) and [generating Prisma Client](/orm/prisma-client/setup-and-configuration/generating-prisma-client), in that case the values will be generated by Prisma ORM's [query engine](/orm/more/under-the-hood/engines).
- For **MongoDB**: `nanoid()` does not generate a valid `ObjectId`. 您可以 use [`@db.ObjectId` syntax](#generate-objectid-as-ids-mongodb-only) if you want 使用 `ObjectId` in the underlying database. However, 您可以 still use `nanoid()` if your `_id` field is not of type `ObjectId`.
...
您可以 also use `dbgenerated(...)` to set the default value for supported types. 例如, in PostgreSQL 您可以 generate UUIDs at the database level rather than rely on Prisma ORM's `uuid()`:
...
- Enums are natively 支援 [PostgreSQL](https://www.postgresql.org/docs/current/datatype-enum.html) and [MySQL](https://dev.mysql.com/doc/refman/8.0/en/enum.html)
...
Composite types are 可用於 versions 3.12.0 and later, and in versions 3.10.0 and later if you enable the `mongodb` Preview feature flag.
...
- adhere to 以下 regular expression: `[A-Za-z][A-Za-z0-9_]*`
