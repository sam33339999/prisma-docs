---
title: "Prisma Config 參考"
metaTitle: "參考 文件 for the prisma config file"
metaDescription: "This page gives an 概述 of all Prisma config options available for use."
...
您可以 define your config in either of two ways:
...
These have been removed in Prisma ORM v7. 參見 individual property sections below for migration guidance.
...
These options declare tables and enums in your database that are **managed externally** (not by Prisma Migrate). 您可以 still query them with Prisma Client, but they will be ignored by migrations.
...
In Prisma ORM v6.19 and earlier, the `experimental` object also included `adapter` and `studio` flags. These have been removed in Prisma ORM v7. 參見 [`adapter`](#adapter-removed) and [`studio`](#studio-removed) sections for details.
...
Connection URL to the shadow database used by Prisma Migrate. Allows you 使用 a cloud-hosted database as the shadow database.
...
If you use a connection pooler URL in the `url` argument (例如, pgBouncer), Prisma CLI commands that require a direct connection to the database use the URL in the `directUrl` argument. 

The `directUrl` property is 支援 Prisma Studio from version 5.1.0 upwards. The `directUrl` property is not needed when using [Prisma Postgres](/postgres) database.
...
預設情況下 it is set 使用 the classic engine, which 需要 that `datasource` be set in your `prisma.config.ts`. 
...
Prisma Studio now uses the connection configuration from the `datasource` property automatically. 參見 [Prisma Studio documentation](/orm/reference/prisma-cli-reference#studio) for more details.
...
開始使用 with Prisma Config, create a `prisma.config.ts` file in your project root. 您可以 use either of these approaches:
...
In Prisma ORM v7, when you run `prisma init`, the generated `prisma.config.ts` file includes `import 'dotenv/config'` 預設情況下. 您必須 install the `dotenv` package 使用 environment variables.
...
If using Node.js v20+ or `tsx`, 您可以 pass a `--env-file` flag to automatically load environment variables:
...
If you want to split your Prisma schema into multiple files, 您需要 specify the path to your Prisma schema folder via the `schema` property:
...
例如, assuming `schema.prisma` defines the `datasource`, here's how how need to place the migrations folder:
...
To fix this, 您可以 use the `--config` flag:
...
If Prisma is installed globally (`npm i -g prisma`), it may not find your `prisma.config.ts` or `prisma/config` module 預設情況下.
To avoid issues:
...
您可以 specify a custom location for your config file when running Prisma CLI commands:
...
In Prisma ORM v7, `prisma init` generates a `prisma.config.ts` file automatically. To load environment variables with `dotenv`, do 以下:
...
To load environment variables in your Prisma application, 您可以 use the `prisma.config.ts` file along with the `env` helper from `prisma/config`. This approach provides better type safety and configuration management.
