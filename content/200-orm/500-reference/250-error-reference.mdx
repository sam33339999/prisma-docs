...
title: '錯誤 message 參考'
metaTitle: 'Errors'
metaDescription: 'Prisma 客戶端, Migrate, Introspection 錯誤 message 參考'
...
欲了解更多資訊 about 如何 work with exceptions and error codes, see [Handling exceptions and errors](/orm/prisma-client/debugging-and-troubleshooting/handling-exceptions-and-errors).

</TopBlock>

## Prisma Client error types

Prisma Client throws different kinds of errors. 以下 lists the exception types, and their documented data fields:

### `PrismaClientKnownRequestError`

Prisma Client throws a `PrismaClientKnownRequestError` exception if the query engine returns a known error related to the request - 例如, a unique constraint violation.

| **Property**    | **Description**                                                                                                  |
| :-------------- | :--------------------------------------------------------------------------------------------------------------- |
| `code`          | A Prisma-specific [error code](#error-codes).                                                                    |
| `meta`          | Additional information about the error - 例如, the field that caused the error: `{ target: [ 'email' ] }` |
| `message`       | Error message associated with [error code](#error-codes).                                                        |
| `clientVersion` | Version of Prisma Client (例如, `2.19.0`)                                                                 |
...
Prisma Client throws a `PrismaClientValidationError` exception if validation fails - 例如：

- Missing field - 例如, an empty `data: {}` property when creating a new record
- Incorrect field type provided (例如, setting a `Boolean` field to `"Hello, I like cheese and gold!"`)
...
**注意：** If you get error code P1012 after you upgrade Prisma ORM to version 4.0.0 or later, 參見 [version 4.0.0 upgrade guide](/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-4#upgrade-your-prisma-schema). A schema that was valid before version 4.0.0 might be invalid in version 4.0.0 and later. The upgrade guide explains how 更新 your schema to make it valid.
...
"Cannot find a fulltext index 使用 for the search, try adding a @@fulltext([Fields...]) to your schema"

#### `P2031`

"Prisma needs to perform transactions, which 需要 your MongoDB server to be run as a replica set. See details: [https://pris.ly/d/mongodb-replica-set](/reference/errors/mongodb-replica-set)"
...
"Failed 建立 database: \{database_error}"
...
"The database schema is not empty. 閱讀更多 about 如何 baseline an existing production database: [https://pris.ly/d/migrate-baseline](/reference/errors/migrate-baseline)"
...
"migrate found failed migrations in the target database, new migrations will not be applied. 閱讀更多 about 如何 resolve migration issues in a production database: [https://pris.ly/d/migrate-resolve](/reference/errors/migrate-resolve)<br />\{details}"
...
"Migration `{migration_name}` cannot be rolled back because it was never applied to the database. Hint: did you pass in the whole migration name? (範例： \"20201207184859_initial_migration\")"
...
"Datasource provider arrays are no longer supported in migrate. Please change your datasource 使用 a single provider. 閱讀更多 at [https://pris.ly/multi-provider-deprecation](https://pris.ly/multi-provider-deprecation)"

#### `P3014`

"Prisma Migrate could not create the shadow database. Please make sure the database user has permission 建立 databases. 閱讀更多 about the shadow database (and workarounds) at [https://pris.ly/d/migrate-shadow](/reference/errors/migrate-shadow).
...
"The migration \{migration_name} could not be found. Please make sure that the migration exists, and that you included the whole name of the directory. (範例： \"20201207184859_initial_migration\")"

#### `P3018`

"A migration failed to apply. New migrations cannot be applied before the error is recovered from. 閱讀更多 about 如何 resolve migration issues in a production database: [https://pris.ly/d/migrate-resolve](/reference/errors/migrate-resolve)"<br /><br />Migration name: \{migration_name}<br /><br />Database error code: \{database_error_code}<br /><br />Database error:<br />\{database_error} "

#### `P3019`

"The datasource provider `{provider}` specified in your schema does not match the one specified in the migration_lock.toml, `{expected_provider}`. Please remove your current migration directory and start a new migration history with prisma migrate dev. 閱讀更多: [https://pris.ly/d/migrate-provider-switch](/reference/errors/migrate-provider-switch)"
...
"Foreign keys cannot be created on this database. Learn more 如何 handle this: [https://pris.ly/d/migrate-no-foreign-keys](/reference/errors/migrate-no-foreign-keys)"

#### `P3022`

"Direct execution of DDL (Data Definition Language) SQL statements is disabled on this database. Please 閱讀更多 here about 如何 handle this: [https://pris.ly/d/migrate-no-direct-ddl](/reference/errors/migrate-no-direct-ddl)"
...
The URL is malformed; 例如, it does not use the `prisma://` protocol.
...
The global timeout of Accelerate has been exceeded. 您可以 find the limit [here](/postgres/database/connection-pooling#query-timeout-limit).

> Also 參見 [troubleshooting guide](/accelerate/troubleshoot#p6004-querytimeout) 欲了解更多資訊.

#### `P6005` (`InvalidParameters`)

The user supplied invalid parameters. Currently only relevant for transaction methods. 例如, setting a timeout that is too high. 您可以 find the limit [here](/postgres/database/connection-pooling#interactive-transactions-query-timeout-limit).

#### `P6006` (`VersionNotSupported`)

The chosen Prisma version is not 相容於 Accelerate. This may occur when a user uses an unstable development version that we occasionally prune.

#### `P6008` (`ConnectionError|EngineStartError`)

The engine failed to start. 例如, it couldn't establish a connection to the database.

> Also 參見 [troubleshooting guide](/accelerate/troubleshoot#p6008-connectionerrorenginestarterror) 欲了解更多資訊.

#### `P6009` (`ResponseSizeLimitExceeded`)

The global response size limit of Accelerate has been exceeded. 您可以 find the limit [here](/postgres/database/connection-pooling#response-size-limit).

> Also 參見 [troubleshooting guide](/accelerate/troubleshoot#p6009-responsesizelimitexceeded) 欲了解更多資訊.

#### `P6010` (`ProjectDisabledError`)

Your accelerate project is disabled. Please [enable](/accelerate/getting-started#1-enable-accelerate) it again 使用 it.
