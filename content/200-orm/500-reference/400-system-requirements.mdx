...
title: "系統需求"
metaTitle: "系統需求 (參考)"
metaDescription: "系統需求 for running Prisma ORM"
tocDepth: 3
---

This page provides an overview of the system 需求 for Prisma ORM.

## 系統需求

This section lists the software that Prisma ORM 需要 and the supported operating systems, along with runtime dependency 需求 for specific operating systems.

### Software 需求

The latest version of Prisma ORM 需要 以下 software:
...
- When using **Yarn 1**, `1.19.2` is the minimum version 相容於 Prisma Client.

另請參閱: [Supported database versions](/orm/reference/supported-databases)
...
Prisma ORM 需要 以下 system libraries to be installed to work:
...
以下 two tables show the supported Linux distro families, OpenSSL versions and C standard libraries for each CPU architecture.
...
On Windows [Microsoft Visual C++ Redistributable 2015](https://download.microsoft.com/download/9/3/F/93FCF1E7-E6A4-478B-96E7-D4B285925B00/vc_redist.x64.exe) or newer must be installed (which is 預設情況下 the case on most modern installations).

#### macOS runtime dependencies

Prisma ORM supports macOS 10.15 or newer. There are no additional platform-specific 需求 on macOS other than 什麼是 listed for all platforms in the [Software 需求](#software-需求) section.

## 疑難排解

There are some common problems caused by using outdated versions of the system 需求:

### Unable to build a TypeScript project with `@prisma/client`

#### Problem

You see 以下 error when you try type-checking a project after you run `prisma generate`.
...
Upgrade the TypeScript dependency in your project to a [version 支援 Prisma ORM](#software-需求). `npm install -D typescript`.

### Unable 使用 `groupBy` 預覽 feature

#### Problem

You see 以下 console error when you attempt to run an app that uses the `groupBy` preview feature:
