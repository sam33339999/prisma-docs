---
title: '資料庫 功能 matrix'
metaTitle: '資料庫 功能 matrix'
metaDescription: '學習 which 資料庫 功能 are supported in Prisma ORM and how they map to the different Prisma ORM 工具.'
wide: true
---
...
本節描述 which database features exist on the relational databases that are currently 支援 Prisma ORM. The **Prisma schema** column indicates how a certain feature can be represented in the [Prisma schema](/orm/prisma-schema) and links to its documentation. 請注意 database features can be used in **Prisma Client** even though they might not yet be representable in the Prisma schema.
...
- \* Not 支援 Microsoft SQL Server
- † JSON and Enum types are supported in SQLite as of Prisma ORM 6.2.0.
- ‡ Not 支援 SQLite

## NoSQL database features

本節描述 which database features exist on the NoSQL databases that are currently 支援 Prisma ORM.

### MongoDB

以下 table lists common MongoDB features and describes the level of support offered by Prisma ORM:

| Feature                                   | 支援 Prisma ORM |                                              Notes                                              |
| ----------------------------------------- | :---------------------: | :---------------------------------------------------------------------------------------------: |
| Embedded documents                        |           ✔️            |                                                                                                 |
| Transactions                              |           ✔️            |                                                                                                 |
| Indexes                                   |     ✔️ with caveats     |    Indexes can only be introspected if the field they 參閱 includes at least some data.     |
