---
title: "Prisma CLI 參考"
metaTitle: "Prisma CLI 參考"
metaDescription: "This page gives an 概述 of all available Prisma CLI 命令, explains their options and shows numerous usage 範例."
...
The `version` command recognizes 以下 options to modify its behavior:
...
預設情況下, the project sets up a [local Prisma Postgres](/postgres/database/local-development) instance but 您可以 choose a different database using the `--datasource-provider` option.
...
| `--db`                   | No       | Shorthand syntax for `--datasource-provider prisma+postgres`; creates a new [Prisma Postgres](/postgres) instance. 需要 authentication in the [PDP Console](https://console.prisma.io).     |                               |
| `--prompt` (or `--vibe`) | No       | Scaffolds a Prisma schema based on the prompt and deploys it to a fresh Prisma Postgres instance. 需要 authentication in the [PDP Console](https://console.prisma.io).                      |                               |
| `--url`                  | No       | Define a custom `datasource` url.                                                                                                                                                                 |                               |
| `--generator-provider`   | No       | Define the generator provider 使用.                                                                                                                                                           | `prisma-client`            |
| `--preview-feature`      | No       | Define the [Preview features](/orm/reference/preview-features) 使用. To define multiple Preview features, you have to provide the flag multiple times for each Preview feature. See examples. |                               |
...
The command output contains helpful information on 如何使用 the generated files and begin using Prisma ORM with your project.
...
The command creates a new [Prisma Postgres](https://www.prisma.io/postgres) instance. 請注意 it 需要 you to be authenticated with the [PDP Console](https://console.prisma.io), If you run it for the first time without being authenticated, the command will open the browser for you to log into Console.
...
The command scaffolds a Prisma schema and deploys it to a fresh [Prisma Postgres](https://www.prisma.io/postgres) instance. 請注意 it 需要 you to be authenticated with the [PDP Console](https://console.prisma.io), If you run it for the first time without being authenticated, the command will open the browser for you to log into Console.
...
參見 [Prisma Config reference](/orm/reference/prisma-config-reference) for more details.
...
The `generate` command is most often used to generate Prisma Client with the `prisma-client` generator. This does 以下:

1. Inspects the current directory to find a Prisma Schema to process.
2. Generates a customized Prisma Client based on your schema into 輸出 directory specified in the generator block.

#### 前置需求

使用 the `generate` command, 您必須 add a generator definition in your `schema.prisma` file. The `prisma-client` generator, used to generate Prisma Client, can be added by including 以下 in your `schema.prisma` file:
...
| `--accelerate`      | No       | The `generate` command will generate Prisma Client for use with [Prisma Accelerate](/accelerate). Mutually exclusive with `--data-proxy` and `--no-engine`. 可用於 Prisma 5.1.0 and later.                                    |
| `--no-engine`       | No       | The `generate` command will generate Prisma Client without an accompanied engine for use with [Prisma Accelerate](/accelerate). Mutually exclusive with `--data-proxy` and `--accelerate`. 可用於 Prisma ORM 5.2.0 and later. |
| `--no-hints`        | No       | The `generate` command will generate Prisma Client without usage hints, surveys or info banners being printed to the terminal. 可用於 Prisma ORM 5.16.0 and later.                                                            |
...
As of Prisma 5.2.0, `--data-proxy` and `--accelerate` are deprecated in favor of `--no-engine` as Prisma Client no longer 需要 an option to work with Prisma Accelerate. All options are available and work similarly, but we recommend `--no-engine` as it prevents an engine from being downloaded which will greatly impact the size of apps deployed to serverless and edge functions.
...
| `--generator` | No       | Specifies which generator 使用 to generate assets. This option may be provided multiple times to include multiple generators. 預設情況下, all generators in the target schema will be run. |                                             |
...
The `prisma-client` generator creates a customized client for working with your database in a custom output directory specified by the `output` field - 您可以 [customize 輸出 folder](/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path).
...
If you're using an older version of Prisma, 您可以 use this command by running:
...
The `dev` command starts a [local Prisma Postgres](/postgres/database/local-development) database that 您可以 run Prisma ORM commands against. It is useful for development and testing purposes and also allows you to switch to [Prisma Postgres](/postgres) in production easily. 
...
Before using the `db pull` command, 您必須 configure your database connection in your `prisma.config.ts` file.
...
- [Conceptual overview of `db push` and 何時使用 it over Prisma Migrate](/orm/prisma-migrate/workflows/prototyping-your-schema)
- [Schema prototyping with `db push`](/orm/prisma-migrate/workflows/prototyping-your-schema)

#### 前置需求

Before using the `db push` command, 您必須 configure your database connection in your `prisma.config.ts` file.
...
| `--force-reset`      | No       | Resets the database and then updates the schema - useful if 您需要 start from scratch due to unexecutable migrations. |
...
The `db execute` command is Generally 可用於 versions 3.13.0 and later. If you're using a version between 3.9.0 and 3.13.0, it is available behind a `--preview-feature` CLI flag.
...
輸出 of the command is connector-specific, and is not meant for returning data, but only to report success or failure.
...
Before using the `db execute` command, if you do not use the `--url` option 您必須 configure your database connection in your `prisma.config.ts` file.
...
One of 以下 data source inputs is required:
...
One of 以下 script inputs is required:
...
**For use in development environments only, 需要 shadow database**
...
1. Applies pending migrations to the shadow database (例如, new migrations created by colleagues)
...
1. Triggers the generation of artifacts (例如, Prisma Client)
...
| `--skip-generate` | No       | Skip triggering generators (例如, Prisma Client)                                                                                                                                            |         |
...
請注意 this command can only be used with a failed migration. If you try 使用 it with a successful migration you will receive an error.
...
| Argument        | Required | Description                                                                                                                                       | Default                                          |
| :-------------- | :------- | :------------------------------------------------------------------------------------------------------------------------------------------------ | :----------------------------------------------- |
| `--applied`     | No\*     | Record a specific migration as applied - 例如 `--applied "20201231000000_add_users_table"`                                                 |                                                  |
| `--rolled-back` | No\*     | Record a specific migration as rolled back - 例如 `--rolled-back "20201231000000_add_users_table"`                                         | `./schema.prisma`<br /> `./prisma/schema.prisma` |
| `--schema`      | No       | Specifies the path to the desired schema.prisma file to be processed instead of the default path. Both absolute and relative paths are supported. | `./schema.prisma`<br /> `./prisma/schema.prisma` |

您必須 specify either `--rolled-back` _or_ `--applied`.
...
In versions 4.3.0 and later, `prisma migrate status` exits with exit code 1 in 以下 cases:
...
This command is only partially supported for [MongoDB](/orm/overview/databases/mongodb). 參見 command options below for details.
...
輸出 can be given either as a human-readable summary (the default) or an executable script.

:::warning

The `migrate diff` command can only compare database features that are [支援 Prisma](/orm/reference/database-features). If two databases differ only in unsupported features, such as views or triggers, then `migrate diff` will not show any difference between them.
...
Both schema sources must use the same database provider. 例如, a diff comparing a PostgreSQL data source with a SQLite data source is not supported.
...
Before using the `migrate diff` command, if you are using the `--from-schema-datasource` or `--to-schema-datasource` 您必須 configure your database connection in your `prisma.config.ts` file.
...
One of 以下 `--from-...` options is required:
...
One of 以下 `--to-...` options is required:
...
您可以 find the complete list of available commands with the arguments [here](/platform/platform-cli/commands).
...
For detailed database support information, including SQLite 需求, see [Databases 支援 Prisma Studio](/orm/tools/prisma-studio#databases-supported-by-prisma-studio).

:::

#### 前置需求

Before using the `studio` command, 您必須 configure your database connection in your `prisma.config.ts` file.
...
The `studio` command recognizes 以下 options:
...
To activate usage of the proxy, provide either of 以下 environment variables:

- [`HTTP_PROXY`](/orm/reference/environment-variables-reference#http_proxy) or `http_proxy`: Proxy URL for http traffic, 例如 `http://localhost:8080`
- [`HTTPS_PROXY`](/orm/reference/environment-variables-reference#https_proxy) or `https_proxy`: Proxy URL for https traffic, 例如 `https://localhost:8080`
...
* Is available for **24 hours** 預設情況下.
* Can be **claimed for free** to make it permanent using the URL displayed in the CLI output.

For full usage details, options (such as `--region` and `--interactive`), and examples, 參見 [documentation](/postgres/introduction/npx-create-db).
