---
title: 'Prisma 客戶端 API 參考'
metaTitle: 'Prisma 客戶端 API'
metaDescription: 'API 參考 文件 for Prisma 客戶端.'
sidebar_label: 'Prisma 客戶端 API'
tocDepth: 3
toc: true
---
...
The Prisma Client API reference documentation is based on 以下 schema:
...
本節描述 the `PrismaClient` constructor and its parameters.
...
Programmatically overrides properties of the `datasource` block in the `schema.prisma` file - 例如, as part of an integration test. 另請參閱: [Data sources](/orm/prisma-schema/overview/data-sources)

From version 5.2.0 and upwards, 您可以 also use the [`datasourceUrl`](#datasourceurl) property to programmatically override the database connection string.
...
- 您必須 re-generate Prisma Client each time you add or rename a data source. Datasource names are included in the generated client.
...
Determines the type and level of logging. 另請參閱: [Logging](/orm/prisma-client/observability-and-logging/logging)
...
| Name    | Example                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `query` | Logs all queries run by Prisma. <br /><br /> For relational databases this logs all SQL queries. 範例： <br />`prisma:query SELECT "public"."User"."id", "public"."User"."email" FROM "public"."User" WHERE ("public"."User"."id") IN (SELECT "t0"."id" FROM "public"."User" AS "t0" INNER JOIN "public"."Post" AS "j0" ON ("j0"."authorId") = ("t0"."id") WHERE ("j0"."views" > $1 AND "t0"."id" IS NOT NULL)) OFFSET $2` <br /><br /> For MongoDB this logs queries using the [`mongosh` shell](https://www.mongodb.com/docs/mongodb-shell/#mongodb-binary-bin.mongosh) format. 範例： <br /> `prisma:query db.User.deleteMany({ _id: ( $in: [ “6221ce49f756b0721fc00542”, ], }, })` |
| `info`  | 範例： <br />`prisma:info Started http server on http://127.0.0.1:58471`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `warn`  | Warnings.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `error` | Errors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `event`  | Raises an event that 您可以 subscribe to.                    |
...
請注意 for MongoDB, the `params` and `duration` fields will be undefined.
...
Defines an instance of a [driver adapter](/orm/overview/databases/database-drivers#driver-adapters). 另請參閱 [Database drivers](/orm/overview/databases/database-drivers) .
...
Use the `rejectOnNotFound` parameter 配置 `findUnique()` and/or `findFirst` to throw an error if the record was not found. 預設情況下, both operations return `null` if the record is not found.

#### Remarks

- 您可以 configure `rejectOnNotFound` on a per-request level for both [`findUnique()`](#findunique) and [`findFirst`](#findfirst)
...
| `isolationLevel` | Sets the [transaction isolation level](/orm/prisma-client/queries/transactions#transaction-isolation-level). 預設情況下 this is set to the value currently configured in your database. The available can vary depending on the database you use. |
...
Use model queries to perform CRUD operations on your models. 另請參閱: [CRUD](/orm/prisma-client/queries/crud)
...
- 您可以not use [filter conditions](#filter-conditions-and-operators) (e.g. `equals`, `contains`, `not`) to filter fields of the [JSON](/orm/reference/prisma-schema-reference#json) data type. Using filter conditions will likely result in a `null` response for that field.
...
| `relationLoadStrategy`          | `'join'` or `'query'`    | No       | **Default: `join`**. Specifies the [load strategy](/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview) for a relation query. Only 可用於 combination with `include` (or `select` on a relation field). In [Preview](/orm/more/releases#preview) since 5.9.0. |
...
請注意 [before Prisma v6](/orm/more/upgrade-guides/upgrading-versions/upgrading-to-prisma-6#removed-notfounderror), it would throw a `NotFoundError: No User found error`. 
...
- Its return type is non-nullable. 例如, `post.findUnique()` can return `post` or `null`, but `post.findUniqueOrThrow()` always returns `post`.
- It is not 相容於 sequential operations in the [`$transaction` API](/orm/prisma-client/queries/transactions#the-transaction-api). If the query throws a `PrismaClientKnownRequestError`, then the API will not roll back any operations in the array of calls. As a workaround, 您可以 use interactive transactions with the `$transaction` API, as follows:
...
See [Filter conditions and operators](#filter-conditions-and-operators) 例如s of 如何 filter results.
...
- Its return type is non-nullable. 例如, `post.findFirst()` can return `post` or `null`, but `post.findFirstOrThrow` always returns `post`.
...
| `distinct`             | `Enumerable<UserDistinctFieldEnum>`                           | No       | Lets you filter out duplicate rows by a specific field - 例如, return only distinct `Post` titles.                                                                                                                                                                                    |
...
- 您可以 also perform a nested [`create`](#create) - 例如, add a `User` and two `Post` records at the same time.
...
In most cases, 您可以 carry out batch inserts with the [`createMany()`](#createmany) or [`createManyAndReturn()`](#createmanyandreturn) queries. However, [there are scenarios where `create()` is the best option to insert multiple records](#remarks-11).

以下範例 results in **two** `INSERT` statements:
...
| JavaScript object (typed)  | `User`                         |                                                                 |
| JavaScript object (plain)  | `{ name: "Alice Wonderland" }` | Use `select` and `include` to determine which fields to return. |
| `PrismaClientKnownRequestError` (code `P2025`) |                                | Thrown if the record 更新 does not exist. See [Error reference](/orm/reference/error-reference#p2025)                   |
...
- 您可以 also perform a nested [`update`](#update-1) - 例如, update a user and that user's posts at the same time.
...
本節涵蓋 the usage of the `upsert()` operation. To 學習關於 using [nested upsert queries](#upsert-1) within `update()`, reference the linked documentation.
...
When your application tries to perform two or more concurrent upsert operations, then a race condition might happen where two or more operations do not find the record and therefore try 建立 that record. In this situation, one of the operations successfully creates the new record but the other operations fail and return a unique key constraint error.

##### Solution

Handle the P2002 error in your application code. When it occurs, retry the upsert operation 更新 the row.

#### Database upserts

Where possible, Prisma Client hands over an `upsert` query to the database. This is called a _database upsert_.

Database upserts have 以下 advantages:
...
使用 a database upsert, Prisma Client sends the SQL construction [`INSERT ... ON CONFLICT SET .. WHERE`](https://www.prisma.io/dataguide/postgresql/inserting-and-modifying-data/insert-on-conflict) to the database.

##### Database upsert 前置需求

Prisma Client can use database upserts if your stack meets 以下 criteria:
...
Prisma Client uses a database upsert for an `upsert` query when the query meets 以下 criteria:
...
以下 `upsert` query meets all of the criteria, so Prisma Client uses a database upsert.
...
In this situation, Prisma uses 以下 SQL query:
...
以下 query has multiple unique values in the `where` clause, so Prisma Client does _not_ use a database upsert:
...
In 以下 query, the values for `userName` in the `where` and `create` options are different, so Prisma Client does _not_ use a database upsert.
...
In 以下 query, the selection on the `title` field in `posts` is a nested read, so Prisma Client does _not_ use a database upsert.
...
`delete` deletes an existing database record. 您可以 delete a record:

- By _ID_
- By a _unique_ attribute

刪除 records that match a certain criteria, use [`deleteMany`](#deletemany) with a filter.
...
| `PrismaClientKnownRequestError` (code `P2025`) |                                | Thrown if the record 刪除 does not exist. See [Error reference](/orm/reference/error-reference#p2025)                   |                                                               |

#### Remarks

- 刪除 multiple records based on some criteria (例如, all `User` records with a `prisma.io` email address, use `deleteMany`)
...
以下 query deletes a specific user record and uses `select` to return the `name` and `email` of the deleted user:
...
| `skipDuplicates?` | `boolean`                         | No       | Do not insert records with unique fields or ID fields that already exist. Only 支援 databases that support [`ON CONFLICT DO NOTHING`](https://www.postgresql.org/docs/9.5/sql-insert.html#SQL-ON-CONFLICT). This excludes MongoDB and SQLServer |
...
- As of Prisma ORM version 5.12.0, `createMany()` is now 支援 SQLite.
- The `skipDuplicates` option is not 支援 MongoDB, SQLServer, or SQLite.
...
- 您可以 use a nested [`createMany`](#createmany-1) query inside an [`update()`](#update) or [`create()`](#create) query - 例如, add a `User` and two `Post` records with a nested `createMany` at the same time.
...
`createManyAndReturn` creates multiple records and returns 結果ing objects.

:::info

This feature is 可用於 Prisma ORM version 5.14.0 and later for PostgreSQL, CockroachDB and SQLite.
...
- The `skipDuplicates` option is not 支援 SQLite.
- 請注意 the order of elements returned by `createManyAndReturn` is not guaranteed.
...
| `limit` | `number`                                                                | No       | Limits the number of records 更新.                                                                                                                                                    |
...
This feature is 可用於 Prisma ORM version 6.2.0 and later for PostgreSQL, CockroachDB and SQLite.

:::

`updateManyAndReturn` updates multiple records and returns 結果ing objects.
...
See [Filter conditions and operators](#filter-conditions-and-operators) 例如s of 如何 filter the records 刪除.
...
另請參閱: [Aggregation, grouping, and summarizing](/orm/prisma-client/queries/aggregation-grouping-summarizing#aggregate)
...
另請參閱: [Aggregation, grouping, and summarizing](/orm/prisma-client/queries/aggregation-grouping-summarizing#group-by)
...
| `having`  | `UserScalarWhereWithAggregatesInput`                         | No       | Allows you to filter groups by an aggregate value - 例如, only return groups _having_ an average age less than 50.                                                                                 |
| `take`    | `number`                                                     | No       | Specifies how many objects should be returned in the list (as seen from the _beginning_ (positive value) or _end_ (negative value) **either** of the list **or** from the `cursor` position if mentioned) |
| `skip`    | `number`                                                     | No       | Specifies how many of the returned objects in the list should be skipped.                                                                                                                                 |
...
- 您可以not combine `select` and `include` on the same level.
- In [3.0.1](https://github.com/prisma/prisma/releases/3.0.1) and later, 您可以 [select a `_count` of relations](#select-a-_count-of-relations).
...
以下範例 demonstrates 如何使用 the [`validator`](/orm/prisma-client/type-safety/prisma-validator) with `select`:
...
`include` defines which relations are included in 結果 that Prisma Client returns. See: [Select fields and include relations](/orm/prisma-client/queries/select-fields) .

#### Remarks

- In [3.0.1](https://github.com/prisma/prisma/releases/3.0.1) and later, 您可以 [`include` a `_count` of relations](#include-a-_count-of-relations)
...
以下範例 demonstrates 如何使用 the [`validator`](/orm/prisma-client/type-safety/prisma-validator) with `include`:
...
- 您可以not combine `omit` and `select` since they serve opposite purposes
...
以下範例 demonstrates 如何使用 the [`validator`](/orm/prisma-client/type-safety/prisma-validator) with `omit`:
...
您可以 learn more about join strategies [here](/orm/prisma-client/queries/relation-queries#relation-load-strategies-preview).

Because the `relationLoadStrategy` option is currently in Preview, 您需要 enable it via the `relationJoins` preview feature flag in your Prisma schema file:
...
After adding this flag, 您需要 run `prisma generate` again to re-generate Prisma Client. The `relationJoins` feature is currently available on PostgreSQL, CockroachDB and MySQL.
...
- 您可以 only specify the `relationLoadStrategy` on the top-level in your query. The top-level choice will affect all nested sub-queries.
...
以下範例 demonstrate 如何使用 the [`validator`](/orm/prisma-client/type-safety/prisma-validator) with `where`:
...
- `UserWhereUniqueInput` This type works by exposing any unique fields on the model. A field assigned `@id` is considered unique,
  ...
  From version 4.5.0, this type exposes all fields on the model. This means that when you filter for a single record based on a unique field, 您可以 check additional non-unique and unique fields at the same time. [Learn more](#filter-on-non-unique-fields-with-userwhereuniqueinput).
...
Sorts a list of records. 另請參閱: [Sorting](/orm/prisma-client/queries/filtering-and-sorting)

#### Remarks

- In [2.16.0](https://github.com/prisma/prisma/releases/2.16.0) and later, 您可以 [order by relation fields](#sort-post-by-the-related-user-records-name) - 例如, order posts by the author's name.

- In [3.5.0](https://github.com/prisma/prisma/releases/3.5.0) and later, in PostgreSQL 您可以 [order by relevance](#sort-post-by-relevance-of-the-title). For details, see [Sort by relevance](/orm/prisma-client/queries/filtering-and-sorting#sort-by-relevance-postgresql-and-mysql).

- In [4.1.0](https://github.com/prisma/prisma/releases/4.1.0) and later, 您可以 [sort `null` records first or last](#sort-post-by-the-related-user-records-name-with-null-records-first). For details, see [Sort with nulls first or last](/orm/prisma-client/queries/filtering-and-sorting#sort-with-null-records-first-or-last).
...
- It is 可用於 version 4.1.0 and later, as a preview feature. See [sort with nulls first or last](/orm/prisma-client/queries/filtering-and-sorting#sort-with-null-records-first-or-last) for details of how 啟用 the feature.
...
以下範例 returns all `User` records sorted by `email` ascending:
...
以下範例 returns all `User` records sorted by `email` descending:
...
以下 query orders posts by user name:
...
以下 query orders posts by user name, with `null` records first:
...
For PostgreSQL, this feature is still in Preview. [Enable the `fullTextSearchPostgres` feature flag](/orm/prisma-client/queries/full-text-search#enabling-full-text-search-for-postgresql) in order 使用 it.

:::

以下 query orders posts by relevance of the search term `'database'` to the title:
...
以下 query orders users by post count:
...
以下範例 sorts users by two fields - first `email`, then `role`:
...
The order of sorting parameters matters - 以下 query sorts by `role`, then `email`. Note the difference in 結果s:
...
以下範例 returns all the `name` and `email` fields of all `User` records, sorted by `email`:
...
以下範例 retrieves a single `User` record by ID, as well as a list of nested `Post` records sorted by `title`:
...
以下 sorts all `User` records by `role` (an `enum`):
...
以下範例 demonstrate 如何使用 the [`validator`](/orm/prisma-client/type-safety/prisma-validator) with `orderBy`:
...
Deduplicate a list of records from [`findMany`](#findmany) or [`findFirst`](#findfirst). 另請參閱: [Aggregation, grouping, and summarizing](/orm/prisma-client/queries/aggregation-grouping-summarizing#select-distinct)

<!-- :::note
**Preview:** 您可以 enable the [`nativeDistinct`](<link-to-nativeDistinct-doc>) feature in Prisma to push this deduplication process down to the database (if supported). For details, 參見 [Preview Features guide](<link>).
::: -->
...
以下範例 returns all distinct `city` fields, and selects only the `city` and `country` fields:
...
以下範例 returns all distinct `city` _and_ `country` field combinations, and selects only the `city` and `country` fields:
...
請注意 there is now a "Paris, Denmark" in addition to "Paris, France":

##### Select distinct in combination with a filter

以下範例 returns all distinct `city` _and_ `country` field combinations where the user's email contains `"prisma.io"`, and selects only the `city` and `country` fields:
...
(where supported). This can significantly improve performance. However, 請注意:
...
- Multiple `connectOrCreate` queries that run _as concurrent transactions_ can result in a **race condition**. Consider the following example, where two queries attempt to `connectOrCreate` a blog post tag named `computing` at the same time (tag names must be unique):
  ...
  const createPost = await prisma.post.create({
    data: {
      title: '如何 handle schema drift in production',
      content: '...',
      author: {
        connect: {
          id: 15,
      ...
      some: {
        ...
        categories: {
          some: {
            ...
            name: {
              in: ['Food', 'Introductions'], // .. with a name that matches one of 以下.
            },
          },
        },
      },
...
await prisma.user.update({
  ...
  data: {
    to_one: {
      ...
      // From Prisma version 4.5.0, 您可以 also do 以下:
