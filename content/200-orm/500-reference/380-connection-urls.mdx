---
title: '連線 URLs'
metaTitle: '連線 URLs (參考)'
metaDescription: '學習 關於 the 格式 and syntax Prisma ORM uses for defining 資料庫 連線 URLs for PostgreSQL, MySQL and SQLite.'
tocDepth: 3
---
...
The connection URL is provided via the `url` field of a `datasource` block in your Prisma schema. It usually consists of 以下 components (except for SQLite and [Prisma Postgres](/postgres)):
...
- **Database name**: The name of the database you want 使用

Make sure you have this information at hand when getting started with Prisma ORM. If you don't have a database server running yet, 您可以 either use a local SQLite database file (參見 [Quickstart](/getting-started/prisma-orm/quickstart/sqlite)) or [setup a free PostgreSQL database with Prisma Postgres](/postgres).

## Format

The format of the connection URL depends on the _database connector_ you're using. Prisma ORM generally supports the standard formats for each database. 您可以 find out more about the connection URL of your database on the dedicated docs page:
...
For MySQL, PostgreSQL and CockroachDB 您必須 [percentage-encode special characters](https://developer.mozilla.org/en-US/docs/Glossary/Percent-encoding) in any part of your connection URL - including passwords. 例如, `p@$$w0rd` becomes `p%40%24%24w0rd`.

For Microsoft SQL Server, 您必須 [escape special characters](/orm/overview/databases/sql-server#connection-details) in any part of your connection string.
...
However, in this case the `API_KEY` doesn't provide authentication details. Instead, it encodes information about the local Prisma Postgres instance. 您可以 obtain a local connection string via the [`prisma dev`](/orm/reference/prisma-cli-reference#dev) command.
...
您可以 also provide the connection URL as an environment variable:
...
您可以 then either set the environment variable in your terminal or by providing a [dotenv](https://github.com/motdotla/dotenv) file named `.env`. 這將 automatically be picked up by the Prisma CLI.

Prisma ORM reads the connection URL from the dotenv file in 以下 situations:
