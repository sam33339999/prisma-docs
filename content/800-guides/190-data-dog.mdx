---
title: '如何 set up Datadog tracing with Prisma ORM'
...
在本指南中, you'll 學習如何 set up Datadog tracing for a new Prisma project. By combining the `@prisma/instrumentation` package with Prisma Client extensions, 您可以 capture detailed spans for every database query. These spans are enriched with query metadata and sent to Datadog [using `dd-trace`](https://www.npmjs.com/package/dd-trace), Datadog's official APM library for Node.js, enabling you to monitor, analyze, and gain visibility into your application's database activity.

### 什麼是 spans and tracing?
...
- **Tracing** ties these spans together to form a complete, end-to-end picture of a request’s lifecycle. With tracing, 您可以 visualize bottlenecks, identify problematic queries, and pinpoint where errors occur from your queries.

### 為什麼使用 Datadog with Prisma ORM?
...
While Prisma ORM abstracts away SQL and boosts developer productivity, it can obscure query performance without proper instrumentation. By integrating Datadog with Prisma using `@prisma/instrumentation` and [`dd-trace`](https://www.npmjs.com/package/dd-trace), 您可以 automatically capture spans for every database query.
...
Before you begin, ensure you have 以下:

- **Node.js** installed (v18+ recommended).
- A local or hosted **PostgreSQL** database.
- A **Datadog** account. If you do not have one, [sign up here](https://www.datadoghq.com/).
- The **Datadog Agent** installed and running on your machine or server where this application will run. 您可以 follow the [Datadog Agent installation docs](https://docs.datadoghq.com/agent/) to set it up.

## 1. Create a new project

We will start by creating a new Node.js project to demonstrate tracing with Datadog and Prisma ORM. 這將 be a minimal, standalone setup focused on running and tracing Prisma queries, to understand the instrumentation flow in isolation.

If you're integrating tracing into an existing Prisma project, 您可以 skip this step and directly follow from [the setup tracing section](#4-set-up-datadog-tracing). Just make sure you apply the changes in your project's equivalent folder structure.
...
在本節中, you will install Prisma, create your schema, and generate the Prisma Client. This prepares your application to run database queries—queries that you will trace with Datadog.

### 2.1. Install and initialize Prisma ORM

Run 以下 commands 安裝 Prisma and a minimal TypeScript runner:

```terminal
npm install -D prisma tsx
```

Then initialize Prisma with the `--db` flag 建立 a [new Prisma Postgres](/postgres) instance:
...
Now, open `prisma/schema.prisma` and update your generator block and models. Replace the `generator` block with 以下, and add a `User` and a `Post` model:
...
You'll need 安裝 the `dotenv` package if you haven't already:
...
這會建立 the tables according to your schema in the Postgres database and generates a client for you to interact with the database.

## 3. Install required dependencies for tracing

In addition to Prisma, you will need 以下 packages for Datadog tracing:
...
Open `src/tracer.ts` and add 以下 code:
...
To resolve this, add 以下 override to your package.json:
```json
//add-start
"overrides": {
...
## 6. Run your queries and 參見 traces
