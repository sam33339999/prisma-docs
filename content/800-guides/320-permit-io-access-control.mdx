---
title: '如何 use Prisma ORM with Permit.io'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with CPermit.io'
...
本指南說明如何 connect Permit.io to a new Express + Prisma app, define a 
[Relationship-Based Access Control (ReBAC)](https://www.permit.io/blog/what-is-rebac) policy, 
and automatically filter Prisma queries so users only 參見 data they're allowed to access. 

You'll build a small project-task API to demonstrate access inheritance in action - no manual `WHERE` clauses required.

您可以 find a complete example of this guide [here](https://www.permit.io/blog/prisma-orm-data-filtering-with-rebac).
...
These are 主要 data entities you'll protect:
...
- New tasks inherit project access—no need 更新 permissions manually.
...
To support permission-aware data filtering, 您需要 structure your database so that relationships are clearly defined. In this case, every `Task` belongs to a `Project`, and users gain access to tasks by being members of the parent project.

### 3.1 Update your Prisma schema

Open `prisma/schema.prisma` and replace the contents with 以下:
...
您可以 open Prisma Studio to inspect your database:
...
To test your data filtering logic, you'll create two projects, each with its own set of tasks. This separation simulates team ownership and will allow you to validate that users only 參見 data of their assigned project.

### 4.1 Create the seed script

Create a new file at `scripts/seed.ts` and add 以下:
...
At this point, if you run a query like `prisma.task.findMany()`, it will return all tasks. In the 後續步驟, you'll connect Permit.io to filter these results automatically based on the user's access rights.

## 5. Install and configure ReBAC filtering

在本節中, you'll install the `@permitio/permit-prisma` extension and configure it to automatically filter Prisma queries based on your access control policies.
...
:::tip API Key  
您可以 find your API key and PDP URL in your [Permit.io dashboard](https://app.permit.io/).
:::
...
This opens a browser window where 您可以 log in to your Permit.io account and link your CLI session to an environment.
...
To filter Prisma queries per user, 您需要:
...
Thanks to the Permit-Prisma integration, **you won't need 添加 any manual filtering logic**—it's handled automatically.
...
- 您可以 write **normal Prisma queries**
