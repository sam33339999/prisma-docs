---
title: '如何 use Prisma ORM with Next.js'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with Next.js 15 and Vercel'
...
- How 設定 Prisma with Next.js?
- 如何 deploy Next.js with Prisma to Vercel?
- 如何 handle migrations in production?

</details>

本指南向您展示如何 use Prisma with Next.js 15, a fullstack React framework. You'll 學習如何 create a [Prisma Postgres](/postgres/) instance, set up Prisma ORM with Next.js, handle migrations, and deploy your application to Vercel. 

您可以 find a [deployment-ready example on GitHub](https://github.com/prisma/prisma-examples/blob/latest/orm/nextjs).
...
From the directory where you want 建立 your project, run `create-next-app` 建立 a new Next.js app that you will be using for this guide.
...
In the `prisma/schema.prisma` file, add 以下 models:
...
Before starting the development server, 請注意 if you are using Next.js v15.2.0 or v15.2.1, do not use Turbopack as there is a known [issue](https://github.com/vercel/next.js/issues/76497). Remove Turbopack from your dev script by updating your `package.json`
...
Now that you have a database with some initial data, 您可以 set up Prisma Client and connect it to your database.
...
Now, add 以下 code to your `lib/prisma.ts` file:
...
Open the `app/page.tsx` file and replace the existing code with 以下:
...
Second, add 以下 code to the `app/posts/page.tsx` file:
...
This page will display a single post's title, content, and author. Just like your other pages, add 以下 code to the `app/posts/new/page.tsx` file:
...
It's not the prettiest page, but it's a good start. Try it out by navigating to `localhost:3000/posts/1` and `localhost:3000/posts/2`. 您可以 also test the 404 page by navigating to `localhost:3000/posts/999`.

## 6. Add a new Posts create page

To round out your application, you'll add a "create" page for posts. 這將 let you write your own posts and save them to the database. 
...
Now, add 以下 code to the `app/posts/new/page.tsx` file:
...
The quickest way to deploy your application to Vercel is 使用 the [Vercel CLI](https://vercel.com/docs/cli).
...
Before you deploy, you also need to tell Vercel to make sure that the Prisma Client is generated. 您可以 do this by adding a `postinstall` script to your `package.json` file.
...
After this change, 您可以 deploy your application to Vercel by running `vercel`.

```terminal
vercel
```

After the deployment is complete, 您可以 visit your application at the URL that Vercel provides. Congratulations, you've just deployed a Next.js application with Prisma ORM!

## 8. 後續步驟

Now that you have a working Next.js application with Prisma ORM, here are some ways 您可以 expand and improve your application:
