---
title: '如何 use AI SDK with Prisma and Next.js for chat applications'
metaTitle: '如何 use AI SDK with Prisma and Next.js for chat applications'
...
在本指南中, you'll learn to build a chat application using AI SDK with Next.js and Prisma ORM to store chat sessions and messages in a Prisma Postgres database. 您可以 find a complete example of this guide on [GitHub](https://github.com/prisma/prisma-examples/tree/latest/orm/ai-sdk-nextjs).
...
開始使用, you'll need 建立 a new Next.js project.
...
這會建立 three models: `Session`, `Message`, and `MessageRole`.
...
Create a `/lib` directory and a `prisma.ts` file inside it. This file will be used 建立 and export your Prisma Client instance.
...
使用 AI SDK, you'll need to obtain an API key from [OpenAI](https://platform.openai.com/api-keys).
...
您需要 create a route handler to handle the AI SDK requests. This handler will process chat messages and stream AI responses back to the client.
...
Now, add the logic 建立 a session with the given `id`:
...
1. Upserts a session with the given `id` 建立 a session if it doesn't exist
...
Replace the content of the `app/page.tsx` file with 以下:
...
This loads any existing messages from your database when the component first mounts, so users can 參見ir previous conversation history.
...
Now we need 建立 the input interface that allows users to type and send messages to the AI:
...
To test your application, run 以下 command:
...
Test it by sending a message to the AI and see if it's saved to the database. Check Prisma Studio to 參見 messages in the database.

```terminal
npx prisma studio
```

You're done! You've just created a AI SDK chat application with Next.js and Prisma. Below are some 後續步驟 to explore, as well as some more resources to help you get started expanding your project.

## 後續步驟

Now that you have a working AI SDK chat application connected to a Prisma Postgres database, 您可以:
