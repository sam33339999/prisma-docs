---
title: '如何 use Prisma ORM with Cloudflare D1'
metaTitle: '如何 use Prisma ORM with Cloudflare D1'
...
本指南向您展示如何 use Prisma ORM with Cloudflare D1, a serverless SQL database that runs on Cloudflare's edge network. You'll 學習如何 set up Prisma ORM with D1, handle migrations, and deploy your application to Cloudflare Workers. 您可以 find a [deployment-ready example on GitHub](https://github.com/prisma/prisma-examples/blob/latest/deployment-platforms/edge/cloudflare-workers/with-d1).
...
Run 以下 command 建立 a [new Cloudflare Worker project](https://developers.cloudflare.com/d1/get-started/#1-create-a-worker):
...
In your Prisma schema, set the `provider` of the `datasource` to `sqlite`. If you just bootstrapped the Prisma schema with `prisma init`, also be sure 添加 the `runtime = "cloudflare"` to the generator block and 以下 `User` model:
...
Also, be sure 使用 a version of the Wrangler CLI that's above [`wrangler@^3.39.0`](https://github.com/cloudflare/workers-sdk/releases/tag/wrangler%403.39.0), otherwise the `--remote` flag that's used in the next sections won't be available.

## 4. Create a D1 database

Run 以下 command 建立 a new D1 database:
...
The `__YOUR_D1_DATABASE_NAME__` is a placeholder that should be replaced with the name you want to give your D1 database. 例如, 您可以 use `prisma-d1-example`.
...
Copy the terminal output and add the content to your `wrangler.jsonc` file. This file is used 配置 your Cloudflare Worker and its bindings.


To connect your Workers with the D1 instance, add 以下 binding to your `wrangler.jsonc`:
...
If you weren't able to grab the database ID from the terminal output, 您可以 also find it in the Cloudflare Dashboard or by running `npx wrangler d1 list` and `npx wrangler d1 info __YOUR_D1_DATABASE_NAME__` in your terminal.
...
Now use `prisma migrate diff` to generate the SQL needed 建立 your database schema:
...
This command generates a SQL file that contains the statements needed 建立 your database tables. 您可以 inspect the generated SQL in `prisma/migrations/0001_init.sql`.
...
For future schema changes, 您可以 generate new migration files using:
...
Before adding a Prisma Client query to your Worker, 您需要 generate Prisma Client with 以下 command:

```
npx prisma generate
```

In order to query your database from the Worker using Prisma ORM, 您需要:

1. Add the `DB` binding to the `Env` interface. This `DB` name matches the binding name you configured in `wrangler.jsonc`. (Alternatively, 您可以 run [`npx wrangler types`](https://developers.cloudflare.com/workers/wrangler/commands/#types) to generate the `Env` type from the binding in a separate file called `worker-configuration.d.ts`.)
2. Instantiate `PrismaClient` using the `PrismaD1` driver adapter, passing `env.DB` which accesses your D1 database binding.
3. Send a query using Prisma Client and return 結果.

Open `src/index.ts` and replace the entire content with 以下:
...
With the database query in place and Prisma Client generated, 您可以 run the Worker locally.
...
Now 您可以 open your browser at [`http://localhost:8787`](http://localhost:8787/) to see 結果 of the database query:
...
To deploy the Worker, run 以下 command:

```
npm run deploy
```

Your deployed Worker is accessible via `https://d1-tutorial.USERNAME.workers.dev` (replace `USERNAME` with your Cloudflare account username). If you navigate your browser to that URL, 您應該 see 以下 data that's queried from your remote D1 database:
...
Now that you've set up Prisma ORM with Cloudflare D1, 您可以:
