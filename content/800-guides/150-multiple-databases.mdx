---
title: '如何 use 多個 databases in a single app'
metaTitle: '如何 use 多個 databases in a single app'
...
本指南向您展示如何 use multiple databases using Prisma ORM in a single [Next.js app](https://nextjs.org/). You will 學習如何 connect to two different Prisma Postgres databases, manage migrations, and deploy your application to Vercel. This approach is useful for multi-tenant applications or when 您需要 separate concerns when managing connections to multiple databases.

## 前置需求

Before you begin, make sure that you have 以下:
...
在本節中, you will create two separate Prisma Postgres instances—one for user data and one for post data. You will also configure the Prisma schema and environment variables for each.
...
## 3. Prepare the application 使用 multiple Prisma Clients
...
In `lib/user-prisma-client.ts`, add 以下 code:
...
In a separate terminal window, open two instances of [Prisma Studio](/orm/tools/prisma-studio) 添加 data to your databases by running the script:

```terminal
npm run studio
```

這將 open up two browser windows, one in `http://localhost:5555` and one in `http://localhost:5556`. Navigate to those windows and add sample data to both databases.

### 4.2. Run the development server

Before starting the development server, 請注意 if you are using Next.js `v15.2.0`, do not use Turbopack as there is a known [issue](https://github.com/vercel/next.js/issues/76497). Remove Turbopack from your dev script by updating your `package.json`:
...
1. Ensure your project is version-controlled and pushed to a GitHub repository. If you do not have a repository yet, [create one on GitHub](https://docs.github.com/en/repositories/creating-and-managing-repositories/creating-a-new-repository). Once the repository is ready, run 以下 commands:
   ```terminal
   git add .
...
在本指南中, you learned 如何使用 multiple databases using Prisma ORM in a single Next.js app by:
...
For further improvements in managing your project, consider using a monorepo setup. 查看 our related guides:

- [如何使用 Prisma ORM in a pnpm workspaces monorepo](/guides/use-prisma-in-pnpm-workspaces)
- [如何使用 Prisma ORM with Turborepo](/guides/turborepo)
