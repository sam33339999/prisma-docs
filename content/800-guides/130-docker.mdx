---
title: "如何 use Prisma in Docker"
metaTitle: "如何 use Prisma in Docker"
...
- 如何 run Prisma in Docker containers?
- How 配置 Docker Compose with Prisma?
- 如何 handle migrations in Docker?

</details>

This guide walks you through setting up a Prisma ORM application within a Docker environment. You'll 學習如何 configure a Node.js project, integrate Prisma for database management, and orchestrate the application using Docker Compose. By the end, you'll have a fully functional Prisma application running in a Docker container.
...
Before starting, ensure that no PostgreSQL services are running locally, and that 以下 ports are free to avoid conflicts: `5432` (PostgreSQL), `3000` (application server) or `5555` (Prisma Studio server).
...
這將 generate a `package.json` file:
...
You'll need 安裝 the `dotenv` package to load environment variables:
...
Add 以下 code 設定 a basic Express server:
...
Run 以下 command to start the database:
...
With the database running, update the `.env` file with 以下 database connection url:
...
Run the migration 建立 the database schema:
...
Visit [`http://localhost:3000`](http://localhost:3000) to 參見 message:
...
For the next step, you'll need to choose between two options for the base image: `node:alpine` (lightweight) or `node:slim` (stable). Both options are fully 支援 Prisma ORM, but may have to be configured differently.
...
Add 以下 content to the `Dockerfile`:
...
The `node:slim` image is based on Linux Debian, a stable and widely supported distribution that uses the `glibc` C standard library. It is mostly supported out of the box on `amd64` and `arm64`, making it a good choice if you're running into compatibility issues with Alpine or need a more production-ready environment. However, some older versions of this image may come without `libssl` installed, so it's sometimes necessary 安裝 it manually.
...
Add 以下 configuration to the file:
...
Before running the app, we need 配置 the environment variables. Create a `.env.prod` file:

```
touch .env.prod
```

Add 以下 database connection url to the `.env.prod` file:
...
With everything set up, it's time to build and run the app using Docker Compose. Run 以下 command:
...
添加 Prisma Studio to your Docker setup, update the `docker-compose.yml` file:
...
這將 start Prisma Studio at [`http://localhost:5555`](http://localhost:5555) alongside 主要 app at [`http://localhost:3000`](http://localhost:3000). 您可以 use Prisma Studio to manage your database with a GUI.

Run 以下 command to start everything:
