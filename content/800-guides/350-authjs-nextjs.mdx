---
title: '如何 use Prisma ORM with Auth.js and Next.js'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with Auth.js and Next.js'
...
在本指南中, you'll wire Auth.js into a brand-new [Next.js](https://nextjs.org/) app and persist users in a [Prisma Postgres](https://prisma.io/postgres) database. 您可以 find a complete example of this guide on [GitHub](https://github.com/prisma/prisma-examples/tree/latest/orm/authjs-nextjs).
...
- *Would you like 使用 App Router?* (recommended) `Yes`
- *Would you like 使用 Turbopack for `next dev`?* `Yes`
...
Add 以下 models to the `schema.prisma` file, these models are provided by Auth.js:
...
To get the `AUTH_SECRET`, 您可以 run 以下 command:

```terminal
npx auth secret --copy
```

- `--copy` will copy the secret to your clipboard. _(Normally, just running `npx auth secret` will add the secret to your `.env.local` file. To keep it tidy, 您可以 use `--copy` and add it to the `.env` file that Prisma created earlier.)_

Add 以下 to the `.env` file:
...
To get the `CLIENT_ID` and `CLIENT_SECRET`, 您可以 create a new OAuth application on Github.
...
In the `/lib` folder, create a new file called `auth.ts` and add 以下 code:
...
Next, you'll need 添加 the Github provider to the `auth.ts` file:
...
Users will now be able to sign in with Github. 添加 them to your database, you'll need 使用 the [Prisma Adapter](https://authjs.dev/getting-started/adapters/prisma):
...
In the root, create a new file called `middleware.ts`. 這將 protect your routes and ensure that only authenticated users can access them:
...
That's it! Your app is now secured. To see more configuration options, 查看 the [Auth.js Middleware documentation](https://next-auth.js.org/configuration/nextjs#middleware).
...
添加 functionality to both of the buttons, add an action to the form that calls the `signIn` and `signOut` functions respectively:
...
In the `/app` folder, replace the `page.tsx` file with 以下 code:
...
If the user is signed in, 您可以 fetch the user data from the database and display it on the page.
...
2. Navigate to `http://localhost:3000` in your browser. 您應該 參見 home page with a "Sign In with github" button.

3. Click on **Sign In with github**, authorize the app, and 您應該 be redirected to the dashboard. 您可以 then sign out and sign back in.
