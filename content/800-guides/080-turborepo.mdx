---
title: '如何 use Prisma ORM with Turborepo'
metaTitle: '如何 use Prisma ORM with Turborepo'
...
Prisma is a powerful ORM for managing databases, and [Turborepo](https://turbo.build/) simplifies monorepo workflows. By combining these tools, 您可以 create a scalable, modular architecture for your projects. 

This guide will show you how 設定 Prisma as a standalone package in a Turborepo monorepo, enabling efficient configuration, type sharing, and database management across multiple apps.

#### What you'll learn:

- How 設定 Prisma in a Turborepo monorepo.
...
設定 a Turborepo monorepo named `turborepo-prisma`, run 以下 command:
...
- *Which package manager do you want 使用?* `npm`
...
欲了解更多資訊 about installing Turborepo, 參閱 the [official Turborepo guide](https://turbo.build/repo/docs/getting-started/installation).
...
Next, install the required dependencies 使用 Prisma ORM. Use your preferred package manager:
...
這將 create several files inside `packages/database`:
...
In the `packages/database/prisma/schema.prisma` file, add 以下 models:
...
It is recommended 添加 `../generated/prisma` to the `.gitignore` file because it contains platform-specific binaries that can cause compatibility issues across different environments.
...
在本指南中, the types will be generated in the `database/generated/prisma` directory.
...
Navigate to the project root and run 以下 command to automatically migrate our database:
...
In the `apps/web/app` directory, open the `page.tsx` file and add 以下 code:
...
If you want 使用 a single `.env` file in the root directory across your apps and packages in a Turborepo setup, consider using a package like [`dotenvx`](https://dotenvx.com/docs/monorepos/turborepo).

To implement this, update the `package.json` files for each package or app to ensure they load the required environment variables from the shared `.env` file. For detailed instructions, 參閱 the [`dotenvx` guide for Turborepo](https://dotenvx.com/docs/monorepos/turborepo).

請記住 that Turborepo [recommends using separate `.env` files for each package](https://turbo.build/repo/docs/crafting-your-repository/using-environment-variables#use-env-files-in-packages) to promote modularity and avoid potential conflicts.
...
To prevent this, ensure that `db:generate` is always executed before running `dev` or `build`. Additionally, make sure both `db:deploy` and `db:generate` are executed before `db:build`. Here's how 配置 this in your `turbo.json` file:
...
Before starting the development server, 請注意 if you are using Next.js v15.2.0, do not use Turbopack as there is a known [issue](https://github.com/vercel/next.js/issues/76497). Remove Turbopack from your dev script by updating your `apps/web/package.json`
...
Navigate to the `http://localhost:3000` and 您應該 參見 message:
...
您可以 add users to your database by creating a seed script or manually by using [Prisma Studio](/orm/tools/prisma-studio). 

使用 Prisma Studio 添加 manually data via a GUI, navigate inside the `packages/database` directory and run `prisma studio` using your package manager:
...
- 查看 [Prisma Postgres](https://www.prisma.io/postgres) to see how 您可以 scale your application.
