---
title: '如何 use Prisma ORM in a pnpm workspaces monorepo'
metaTitle: '如何 use Prisma ORM in a pnpm workspaces monorepo'
...
Prisma is a powerful ORM for managing your database, and when combined with [pnpm Workspaces](https://pnpm.io/workspaces), 您可以 maintain a lean and modular monorepo architecture. 在本指南中, we’ll walk through setting up Prisma in its own package within a pnpm Workspaces monorepo, enabling maintainable type sharing and efficient database management across your apps.

### What you'll learn:

- 如何 initialize a monorepo using pnpm Workspaces.
- Steps to integrate Prisma as a standalone package.
- 如何 generate and share the Prisma Client across packages.
...
Before integrating Prisma, 您需要 set up your project structure. Start by creating a new directory for your project (例如, `my-monorepo`) and initialize a Node.js project:
...
Add 以下 configuration to `pnpm-workspace.yaml`:
...
The `catalogs` help you pin a certain version of prisma across your repositories. 您可以 learn more about them [here](https://pnpm.io/catalogs). *Explictly* pin the lastest version of [prisma](https://www.npmjs.com/package/prisma) in the `pnpm-workspace.yaml` file. At the time of writing, this is version `6.3.1`.
...
本節涵蓋 creating a standalone database package that uses Prisma. The package will house all database models and the generated Prisma Client, making it reusable across your monorepo.
...
Initialize Prisma ORM with an instance of [Prisma Postgres](/postgres) in the `database` package by running 以下 command: 
...
We're going to be using [Prisma Postgres](/postgres/getting-started) 在本指南中. If you're not using a Prisma Postgres database, you won't need 添加 the `--db` flag.
...
Now, create a `prisma.config.ts` file in the `database` package 配置 Prisma:
...
You'll need 安裝 the `dotenv` package to load environment variables from the `.env` file:
...
The `--yes` flag uses default configurations to bootstrap the Next.js app (which 在本指南中 uses the app router without a `src/` directory and `pnpm` as the installer). 
...
Run 以下 command 安裝 the `database` package:
...
Modify your Next.js application code 使用 Prisma Client from the database package. Update `app/page.tsx` as follows:
...
Add 以下 scripts to the root `package.json` of your monorepo. They ensure that database migrations, type generation, and app builds run in the proper order:
...
There shouldn't be data in your database yet. 您可以 execute `pnpm run studio` in your CLI to start a [Prisma Studio](/orm/tools/prisma-studio) in [`http://localhost:5555`](http://localhost:5555) to interact with your database and add data to it.
...
For further exploration and to enhance your setup, consider reading the [如何使用 Prisma ORM with Turborepo](/guides/turborepo) guide.
