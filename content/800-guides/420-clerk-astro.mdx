---
title: '如何 use Prisma ORM with Clerk Auth and Astro'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with Clerk Auth and Astro'
...
在本指南中 you'll wire Clerk into a brand-new [Astro](https://astro.build/) app and persist users in a [Prisma Postgres](https://prisma.io/postgres) database. 您可以 find a complete example of this guide on [GitHub](https://github.com/prisma/prisma-examples/tree/latest/orm/clerk-astro).
...
Run 以下 command 建立 the database tables:
...
這會產生 the Prisma Client in the `src/generated/prisma` directory.
...
You'll do that by checking if the event type is `user.created` and then using Prisma's `upsert` method 建立 a new user if they don't exist:
...
You'll need to expose your local app for webhooks with [ngrok](https://ngrok.com/). 這將 allow Clerk to reach your `/api/webhooks/clerk` route to push events like `user.created`.
...
Copy the ngrok `Forwarding URL` (e.g., `https://a65a60261342.ngrok-free.app`). 這將 be used 配置 the webhook URL in Clerk.
...
Replace `<your-ngrok-subdomain>` with the subdomain from your ngrok URL. 例如, if your ngrok URL is `https://a65a60261342.ngrok-free.app`, use `a65a60261342.ngrok-free.app`.
...
您應該 see a new user record with the Clerk ID, email, and name from your sign-up.
...
Now that you have a working Astro app with Clerk authentication and Prisma connected to a Prisma Postgres database, 您可以:

- Add user profile management and update functionality
- Build protected API routes that require authentication
- Extend your schema with additional models related 使用rs
