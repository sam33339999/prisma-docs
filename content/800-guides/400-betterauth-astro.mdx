---
title: '如何 use Prisma ORM with Better Auth and Astro'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with Better Auth and Astro'
...
在本指南中, you'll wire Better Auth into a brand-new [Astro](https://astro.build/) app and persist users in a [Prisma Postgres](https://prisma.io/postgres) database. 您可以 find a complete example of this guide on [GitHub](https://github.com/prisma/prisma-examples/tree/latest/orm/betterauth-astro).
...
Run 以下 command to generate the Prisma Client:
...
Add 以下 code to the newly created `[...all].ts` file. This code uses the Better Auth handler to process authentication requests.
...
Add 以下 code, which creates the client functions you'll use in your UI:
...
In the `src` directory, create a `middleware.ts` file to check authentication status on every request. 這將 make the user and session data available to all your pages.
...
Now, let's build the user interface for authentication. In the `src/pages` directory, create 以下 folder structure:
...
This page allows new users 建立 an account. Start with the basic HTML structure in `src/pages/sign-up/index.astro`.
...
Finally, update the home page to provide simple navigation. Replace the contents of `src/pages/index.astro` with 以下:
...
2. Navigate to `http://localhost:4321` in your browser. 您應該 參見 home page with "Sign Up" and "Sign In" links.
