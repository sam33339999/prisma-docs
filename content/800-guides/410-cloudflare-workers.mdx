---
title: '如何 use Prisma ORM with Cloudflare Workers'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with Cloudflare Workers'
...
在本指南中, you'll learn to integrate Prisma ORM with a Prisma Postgres database in a Cloudflare Workers project. 您可以 find a complete example of this guide on [GitHub](https://github.com/prisma/prisma-examples/tree/latest/orm/cloudflare-workers).
...
In the `prisma/schema.prisma` file, add 以下 `User` model and set the runtime to `cloudflare`:
...
Both the `cloudflare` and `workerd` runtimes are supported. 閱讀更多 about runtimes [here](/orm/prisma-schema/overview/generators#field-reference).

:::

這會建立 a `User` model with an auto-incrementing ID, email, and name.

### 2.4. Configure Prisma scripts

Add 以下 scripts to your `package.json` to work with Prisma in the Cloudflare Workers environment:
...
這會產生 the Prisma Client in the `src/generated/prisma/client` directory.
...
Now use Prisma Client 建立 a new user and count the total number of users:
...
Open [`http://localhost:8787`](http://localhost:8787) in your browser. Each time you refresh the page, a new user will be created. 您應該 see output similar to:
...
這將 open a browser window where 您可以 view and edit your `User` table data.

## 4. Deploy to Cloudflare Workers

### 4.1. Set your database URL as a secret

Before deploying, 您需要 set your `DATABASE_URL` as a secret in Cloudflare Workers. This keeps your database connection string secure in production.
...
Now that you have a working Cloudflare Workers app connected to a Prisma Postgres database, 您可以:
