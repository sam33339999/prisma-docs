---
title: '如何 use Prisma ORM with SvelteKit'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with SvelteKit'
...
在本指南中, you'll learn to integrate Prisma ORM with a Prisma Postgres database in a SvelteKit project from scratch. 您可以 find a complete example of this guide on [GitHub](https://github.com/prisma/prisma-examples/tree/latest/orm/sveltekit).
...
- *What would you like 添加 to your project?* 
  - `prettier`
  - `eslint`
- *Which package manager do you want 安裝 dependencies with?* `npm`
...
Inside your `/src/lib` directory, rename `index.ts` to `prisma.ts`. This file will be used 建立 and export your Prisma Client instance.
...
To also fetch each user's posts, we can expand the query using the `include` option. This tells Prisma to join the related `Posts` table in 結果.
...
Now, every user in 結果 will also include a `posts` array.
...
You're done! You've just created a SvelteKit app with Prisma ORM. Below are some 後續步驟 to explore, as well as some more resources to help you get started expanding your project.


## 後續步驟

Now that you have a working SvelteKit app connected to a Prisma Postgres database, 您可以:
