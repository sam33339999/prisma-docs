---
title: '如何 use Prisma ORM with Nuxt'
metaTitle: '建置 a Nuxt app with Prisma ORM and Prisma Postgres'
...
本指南說明如何 set up a Nuxt application, configure [Prisma Postgres](https://prisma.io/postgres) with Prisma ORM, and deploy the project to [Vercel](https://vercel.com/) for production.

Here's what you'll learn:

- How 設定 a Nuxt project that uses Prisma ORM directly.
- How 配置 and use Prisma Postgres with Prisma ORM in your Nuxt app.
- 如何 deploy the project to Vercel.

## 前置需求

To follow this guide, ensure you have 以下:  
...
With Prisma initialized, define your data model and run a migration 建立 the PostgreSQL database.

1. Update the `prisma/schema.prisma` file with 以下 configuration:
...
With Prisma configured, the next step is 更新 your application code to fetch and display data from your database.
...
4. Run 以下 command to start the development server again:  
   ```terminal
   npm run dev
   ```
...
If you want sample data, create a `prisma/seed.ts` file. 以下 script uses the PostgreSQL adapter:
...
Follow these steps 建立 your Prisma Postgres database:
...
Once the green **`CONNECTED`** label appears, your database is ready 使用!
...
The `DATABASE_URL` environment variable will be required in the 後續步驟. 

## 7. Set up Prisma Postgres in your Nuxt app

Now that the Prisma Postgres instance is ready, update your Nuxt application 使用 this database:
...
4. Start the development server again:
   ...
   npm run dev
   ```
5. Verify the data in the application by opening `https://localhost:3000`. If you seeded the database, 您應該 參見 first user's name displayed.
...
1. Ensure your project is version-controlled and pushed to a GitHub repository. If you don't have a repository yet, [create one on GitHub](https://docs.github.com/en/repositories/creating-and-managing-repositories/creating-a-new-repository). Once the repository is ready, run 以下 commands:
   ```terminal
   git add .
