---
title: 'Instant app 部署 with Vercel and Prisma Postgres'
metaTitle: 'Instant app 部署 with Vercel and Prisma Postgres'
...
本指南向您展示如何 implement instant app deployment using [Vercel](https://vercel.com)'s API with integrated [Prisma Postgres](/postgres) databases. You'll learn to programmatically create, deploy, and transfer full-stack applications with a single API call.

Instant app deployment solves a critical problem for AI coding platforms, no-code tools, and educational platforms: getting from generated code to a live, production-ready application. Instead of requiring users to manually set up hosting infrastructure, 您可以 offer one-click deployments with both application and database.

By the end of this guide, you'll understand 如何 integrate Vercel's deployment API with Prisma Postgres 建立 a smooth deployment experience for your users.

## Try the live demo

Experience the instant deployment flow with [our interactive demo](https://pris.ly/vercel-app-demo-live). 您可以 deploy and claim real applications to 參見 complete process in action.
...
- **Authorization**: Permission 建立 resources on behalf of your team
...
預設情況下, every new partner is on our free plan which limited to 5 dbs per account, so if you are trying out this API and need higher db creation limits (which we suspect that most of you will), then please [contact us](https://www.prisma.io/partners#contact-us) to get partner level access.
:::
...
Vercel 需要 a credit card to be attached to your account (even on the Hobby plan) 使用 the deployment APIs. Make sure 添加 payment information in your Vercel account settings before proceeding.
:::
...
參見 complete list of [supported Prisma Postgres regions](/postgres/more/faq#what-regions-is-prisma-postgres-available-in) and their corresponding [Vercel region codes](https://vercel.com/docs/regions#region-list).
...
Before creating databases, you need authorization 使用 Prisma on behalf of your team.
...
**Key parameters:**
- `files`: Array of uploaded files (需要 prior file upload in `tgz` format)
- `framework`: "nextjs", "react", "vue", etc. for automatic configuration
- `projectSettings`: Framework-specific build and runtime settings

Learn more in the [Vercel Deployments API documentation](https://vercel.com/docs/rest-api/reference/endpoints/deployments/create-a-new-deployment) or 參見 [Vercel API Reference](https://vercel.com/docs/rest-api/reference).
...
4. **Transfer completion**: Project and database transfer 使用r's selected team
...
Vercel enforces rate limits on API endpoints. 您可以 implement exponential backoff to handle rate limits:
...
以下 are some production considerations for your deployment service:

### Integration with existing platforms

您可以 integrate your deployment service with existing platforms to provide a smooth experience for your users:
...
Now that you understand instant app deployment with Vercel and Prisma Postgres, 您可以:
