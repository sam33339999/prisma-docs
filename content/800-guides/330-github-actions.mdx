---
title: '如何 provision 預覽 databases with GitHub Actions and Prisma Postgres'
metaTitle: '如何 provision 預覽 databases with GitHub Actions and Prisma Postgres'
...
本指南向您展示如何 automatically create and delete [Prisma Postgres](https://www.prisma.io/postgres) databases using GitHub Actions and [the Prisma Postgres management API](https://api.prisma.io/v1/swagger-editor). The setup provisions a new database for every pull request, seeds it with sample data, and the `github-actions` bot leaves a comment with the database name and the status.

![GitHub Actions comment](/img/guides/github-comment.png)

After the PR is closed, the database is automatically deleted. This allows you to test changes in isolation without affecting 主要 development database. 

## 前置需求

Make sure you have 以下:
...
在本節中, you'll set up Prisma in your project and verify that it works locally before integrating it into GitHub Actions. This involves installing Prisma's dependencies, connecting to a Prisma Postgres database, defining your data models, applying your schema, and seeding the database with sample data.

By the end of this section, your project will be fully prepared 使用 Prisma both locally and in a CI workflow.

### 2.1. Install dependencies

開始使用 with Prisma, install the required dependencies:
...
Create a `prisma.config.ts` file 配置 Prisma with seeding:
...
You'll need 安裝 the `dotenv` package:
...
This workflow uses `us-east-1` as the default region for Prisma Postgres. 您可以 change this to your preferred region by modifying the `region` parameter in the API calls, or even by adding a `region` input to the workflow.

:::

### 3.2. Add the base configuration

Paste 以下 into `.github/workflows/prisma-postgres-management.yml`. This sets up when the workflow runs and provides required environment variables.
...
Append 以下 under the `jobs:` key in your workflow file:
...
When a pull request is closed, 您可以 automatically remove the associated database by adding a cleanup job. The cleanup job:

* Finds the database by name
* Deletes it from the Prisma Postgres project
* Can also be triggered manually with `action: cleanup`

Append 以下 to your `jobs:` section, after the `provision-database` job:
...
If you don't have a repository in GitHub yet, [create one on GitHub](https://docs.github.com/en/repositories/creating-and-managing-repositories/creating-a-new-repository). Once the repository is ready, run 以下 commands:
   ```terminal
   git add .
...
To avoid conflicts with your development databases, you'll now create a dedicated project specifically for CI workflows. Use 以下 curl command 建立 a new Prisma Postgres project using the Prisma Postgres Management API:
...
Copy and store the `$PRISMA_PROJECT_ID` from 輸出. This is your Prisma project ID, which you will use in the next step.
...
您可以 test the setup in two ways:
...
4. Choose `provision` as the action and optionally provide a custom database name. 您可以 also choose `cleanup` 刪除 an *existing* database.
...
This setup improves confidence in changes and reduces the risk of shared database conflicts. 您可以 extend this by integrating test suites, or integrating it in your workflow.