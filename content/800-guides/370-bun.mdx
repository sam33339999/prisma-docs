---
title: '如何 use Prisma in Bun'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with Bun'
...
這會建立 a basic Bun project that includes a `package.json` file and an `index.ts` file.
...
We're going 使用 a direct connection string for connecting to Prisma Postgres. To get your [direct connection string](/postgres/database/direct-connections#how-to-connect-to-prisma-postgres-via-direct-tcp):
...
Create a `db.ts` file in your project root 配置 `PrismaClient`:
...
Replace the `index.ts` file contents with 以下 code to build a simple HTTP server that uses Prisma ORM to fetch and display users:
...
您應該 see `Listening on http://localhost:3000` in the console. When you visit `http://localhost:3000` in your browser, you'll see a JSON response with all the users in your database and the total count.
...
這會建立 an executable file named `index` (or `index.exe` on Windows) in your project directory.
...
您應該 參見 same `Listening on http://localhost:3000` message, and your application will work exactly the same as before. The executable includes all dependencies and can be deployed to any compatible system without requiring Bun or Node.js to be installed.
...
- **Distribution**: Share your application without requiring users 安裝 Bun
...
您可以 explore [this example](https://pris.ly/bun_ppg_example) to see a sample application built with Bun and Prisma.

Now that you have a Bun application connected to a Prisma Postgres database, 您可以 continue by:
