---
title: 'Partner 資料庫 Provisioning & 使用者 Claim Flow'
metaTitle: 'Partner 資料庫 Provisioning & 使用者 Claim Flow'
description: 'Learn how to use the Prisma Postgres Management API to provision and claim databases'
sidebar_label: '管理 API'
...
This guide walks you through 如何使用 the Prisma Postgres Management API, to power experiences like the [`npx create-db`](https://create-db.prisma.io/) command.

You'll 學習如何 provision a Prisma Postgres database on your workspace as a partner, and 如何 transfer it to another user's workspace so they can "claim" the database. We'll cover how the process is secured using OAuth2, and by the end, you'll understand the full flow and 如何 integrate it into your own product experience.

This guide references the actual implementation in the `npx create-db` CLI and Cloudflare Workers as real world examples. The repo for the `npx create-db` is [here](https://github.com/prisma/create-db), which can be used as a reference for 如何使用 the Management API in your own projects.

:::note 如何 this fit into your app?
The two Cloudflare Workers 在本指南中 are just reference examples. You would typically build this logic into your own backend or serverless functions.

Similarly, the `npx create-db` CLI is a simple demo. In your product, 您可以 trigger the same API calls from your own UI or onboarding flows 建立 a seamless experience for your users.
:::

## Core concepts

Before diving into implementation, let's clarify 主要 concepts involved in the Management API integration:

- **Management API**: A set of endpoints that allow you to programmatically provision and manage Prisma Postgres databases.
- **Projects vs Databases**: A project is a container that can hold multiple databases. 您可以 use this to organize databases you create e.g. by user. Projects can then be transferred 使用rs, including all databases they contain.
...
使用 the Prisma Postgres Management API, you first need 設定 as a partner:
...
2. **Obtain OAuth credentials**: 您可以 obtain your OAuth credentials in the [Prisma Console](https://console.prisma.io). 參見 [next section](#get-oauth-credentials) for details.

For a complete list of available endpoints and details on request/response formats, 參見 [Prisma Management API documentation](/postgres/introduction/management-api).
...
On the next screen, 您可以 access and save the client ID and client secret for your OAuth app.
...
1. **Gather required information**: Prepare the necessary details for provisioning, such as region, database name, and any other options your application 需要. This information may come from user input or be determined by your application logic.
...
3. **Send a database provisioning request**: Make a `POST` request to the Management API endpoint 建立 a new project with a default database. 例如：
    ```ts
...
By following this guide, you have learned 如何:
...
For further details, 參見 [create-db](https://github.com/prisma/create-db) repo for a reference implementation, or consult the [Prisma Management API documentation](/postgres/introduction/management-api).