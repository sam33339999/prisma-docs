---
title: '如何 use Prisma Postgres with Shopify'
metaTitle: '如何 use Prisma Postgres with Shopify'
...
:::note
If you do not have the Shopify CLI installed, 您可以 install it with `npm install -g @shopify/cli`.
:::
...
During setup, you'll be prompted to customize your app. Don't worry—just follow these recommended options 開始使用 quickly and ensure your app is set up for success:
...
Prisma comes pre-installed in your project, but let's take a moment 更新 it to the latest version. This ensures you have access to the newest features, improvements, and the best possible experience as you build your app.
...
您需要 update a few things in the `schema.prisma` file to get it working with Remix and Prisma Postgres.
...
Create a `prisma.config.ts` file 配置 Prisma:
...
Since Shopify apps typically have dotenv pre-installed, 您應該 already have access to it. If not, install it with:
...
啟用 your app to store notes for each product, let's add a new `ProductNote` model to your Prisma schema. 
...
Prisma Postgres allows you 建立 a new database on the fly, 您可以 create a new database at the same time you initialize your project by adding the `--db` flag:
...
4. **Apply your database schema:**
   - Run 以下 command 建立 your tables and get your database ready:
   ```terminal
...
Now, before moving on, let's update your `db.server.ts` file 使用 the newly generated Prisma client with the driver adapter:
...
:::warning
It is recommended 添加 `app/generated/prisma` to your `.gitignore` file.
:::

## 3. Create your Remix model

To keep your project organized, let's create a new `models/` folder. Inside this folder, add a file named `notes.server.js`. 這將 be the home for all your note-related logic and make your codebase easier to manage as your app grows.

The `notes.server.js` file will contain two functions:
- `getNotes` - 這將 get all the notes for a given product.
- `createNote` - 這將 create a new note for a given product.
...
啟用 users 添加 new notes to your database, let's create a function in `notes.server.js` that uses `prisma.productNote.create`:
...
If you run `npm run dev`, you won't be able to 參見 `Product Notes` route. To fix this, 您需要 add the `ProductNotesLayout` to the `app.jsx` file so it shows up in the sidebar:
...
Follow these steps 建立 the product notes route:

Create a new `routes/app/app.notes.$productGid.jsx` file which will take in the productGid as a parameter, and return the product notes associated with the product as well as a form 建立 a new 注意：
...
您應該 be seeing "No notes yet.". If so, you're on the right track.

### 5.2. Add the form

A few things need to be added to the route in order 建立 a new 注意：
...
Now, 您可以 build out the form that will call the `action` function:
...
您應該 now be able 添加 a note to a product and see it displayed.
...
Now that you have a working Shopify app connected to a Prisma Postgres database, 您可以:
