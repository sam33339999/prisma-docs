---
title: '如何 管理 Schema changes in a team'
metaTitle: '如何 管理 Schema changes in a team with Prisma Migrate and Prisma ORM'
description: 'Learn how to use Prisma Migrate effectively when collaborating on a project as a team'
sidebar_label: 'Schema 管理 in teams'
...
When working in a team, managing database schema changes can be challenging. 本指南向您展示如何 effectively collaborate on schema changes using Prisma Migrate, ensuring that all team members can safely contribute to and incorporate schema changes.
...
Migrations are **applied in the same order as they were created**. The creation date is part of the migration subfolder name - 例如, `20210316081837-updated-fields` was created on `2021-03-16-08:18:37`.

### 1.2. Source control 需求

您應該 commit 以下 files to source control:

- The contents of the `.prisma/migrations` folder, including the `migration_lock.toml` file
- The Prisma Schema (`schema.prisma`)

Source-controlling the `schema.prisma` file is not enough - 您必須 include your migration history because:
...
Now that you understand team schema management, 您可以:

- 學習關於 [customizing migrations](/orm/prisma-migrate/workflows/customizing-migrations)
