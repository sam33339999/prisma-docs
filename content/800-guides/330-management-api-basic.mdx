---
title: 'Get started with the Prisma 管理 API'
metaTitle: "Get started with the Prisma 管理 API"
description: "Learn how to get started with the Prisma Management API"
sidebar_label: 'Programmatically 管理 Prisma Postgres'
...
This guide walks you through setting up a basic TypeScript project that uses the [Prisma Postgres Management API](/postgres/introduction/management-api) 建立 a new [Prisma Console project](/platform/about#project) with a [Prisma Postgres](/postgres/introduction/overview) database, and print out all connection details. 
...
First, 您需要 create a service token to be able to access the Management API:
...
Open your terminal and run 以下 commands:
...
You now have an `index.ts` file that 您可以 execute with `npx tsx index.ts`. It's still empty, you'll start writing code in step 3.
...
You're going 使用 [`axios`](https://github.com/axios/axios/tree/main) as your HTTP client to interact with the Management API. Install it as follows:

```terminal
npm install axios
```

You're all set, let's write some code 建立 a project and provision a Prisma Postgres database!

## 3. Programmatically create a new project with a database

Paste 以下 code into `index.ts`:
...
您可以 run your script with 以下 command:
...
Your output of the command should look similar to 輸出 above.

## Conclusion

You have now set up a TypeScript project that interacts with the Prisma Management API, creates a new project and database, and prints out all connection strings. 您可以 extend this script to manage more resources or automate other tasks using the Management API.
