...
metaDescription: '學習 如何 integrate Prisma Postgres in a Deno Deploy project 使用 a simple Deno application.'
...
To ensure VS Code recognizes this as a Deno project and provides proper TypeScript validation, 您需要 initialize the workspace. Without this, VS Code will show errors when using Deno-specific APIs like `Deno.serve`.
...
Update the `main.ts` file 建立 a simple HTTP server that responds with "Hello, World!", establishing the foundation for your application before adding database functionality.
...
您可以 test the server locally by running:

```terminal
deno run dev
```

Visit `localhost:8000` in your browser to 參見 application running.

### 1.3 Push initial project to GitHub

To connect your project to Deno Deploy and get a database connection string, 您需要 have a successful deployment. Set up a GitHub repository and push your project to it.

## 2. Deploy the project to Deno Deploy

Deploy your repository to Deno Deploy. Any subsequent commits will trigger automatic redeployments. 您需要 deploy now, as the database string 需要 a successful deployment to generate.
...
## 3. Provision a Prisma Postgres 資料庫
...
### 4.2 Initialize Prisma and create 綱要
...
## 5. Update the application 使用 Prisma
...
Visit `localhost:8000` in your browser to 參見 application running. 您應該 see a JSON response containing the log entry:
...
The build command must generate the Prisma Client code to ensure it is 可用於 production.
...
Navigate back to Deno Deploy and 您應該 see a successful build. Once deployed, click the deployment URL at the top right of the dashboard.

### 6.3 Verify the 部署

When you visit your deployed application, 您應該 see a response that looks like this:
...
Now that you have a working Deno app connected to a Prisma Postgres database, 您可以:
