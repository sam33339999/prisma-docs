...
本指南向您展示如何 write guides for Prisma ORM documentation. It covers the required structure, formatting, and style conventions to ensure consistency across all guides. You'll 學習關於 frontmatter 需求, section organization, and writing style.
...
Every guide must include 以下 frontmatter at the top of the file:

```mdx
---
title: '如何 [do something] with Prisma ORM'
metaTitle: '如何 [do something] with Prisma ORM'
...
- `title`: Should be action-oriented and start with "如何"
- `metaTitle`: Usually matches the title, used for SEO
- `description`: A one-sentence summary starting with "學習如何", used for SEO
...
1. **Introduction**
   ...
   - Link to any example repositories or 相關資源
...
- Include complete, runnable 程式碼範例
...
To get the guide into the sidebar, 您需要 add 以下 to the `sidebars.ts` file:
...
````markdown
---
title: '如何 use Prisma ORM with __________'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with __________'
...
```markdown
markdown - *Which package manager would you like 使用?* `npm` 
```
