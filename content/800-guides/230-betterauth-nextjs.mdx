---
title: '如何 use Prisma ORM with Better Auth and Next.js'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with Better Auth and Next.js'
...
在本指南中, you'll wire Better Auth into a brand-new [Next.js](https://nextjs.org/) app and persist users in a [Prisma Postgres](https://prisma.io/postgres) database. 您可以 find a complete example of this guide on [GitHub](https://github.com/prisma/prisma-examples/tree/latest/orm/betterauth-nextjs).
...
- *Would you like 使用 TypeScript?* `Yes`  
- *Would you like 使用 ESLint?* `Yes`  
- *Would you like 使用 Tailwind CSS?* `Yes`  
- *Would you like your code inside a `src/` directory?* `Yes`  
- *Would you like 使用 App Router?* `Yes`  
- *Would you like 使用 Turbopack?* `Yes`
- *Would you like to customize the import alias (`@/*` 預設情況下)?* `No`
...
Run 以下 command 建立 the database tables and generate the Prisma Client:
...
In the `src` directory, create a `lib` folder and a `prisma.ts` file inside it. This file will be used 建立 and export your Prisma Client instance.
...
In this file, you'll configure Better Auth 使用 the Prisma adapter, which allows it to persist user and session data in your database. You will also enable email and password authentication.
...
Better Auth also supports other sign-in methods like social logins (Google, GitHub, etc.), which 您可以 explore in their [documentation](https://www.better-auth.com/docs/authentication/email-password).
...
:::info
If your application runs on a port other than `3000`, 您必須 add it to the `trustedOrigins` in your `auth.ts` configuration to avoid CORS errors during authentication requests.
...
With the new models in your schema, 您需要 update your database. Run a migration 建立 the corresponding tables:
...
Add 以下 code to the newly created `route.ts` file. This code uses a helper from Better Auth 建立 Next.js-compatible `GET` and `POST` request handlers.
...
Add 以下 code, which creates the React hooks and functions you'll use in your UI:
...
Now, let's build the user interface for authentication. In the `src/app` directory, create 以下 folder structure:
...
First, create the basic `SignUpPage` component in `src/app/sign-up/page.tsx`. This sets up 主要 container and a title for your page.
...
Finally, update the home page to provide simple navigation to the sign-in and sign-up pages. Replace the contents of `src/app/page.tsx` with 以下:
...
2. Navigate to `http://localhost:3000` in your browser. 您應該 參見 home page with "Sign Up" and "Sign In" buttons.

3. Click on **Sign Up**, create a new account, and 您應該 be redirected to the dashboard. 您可以 then sign out and sign back in.

4. To view the user data directly in your database, 您可以 use Prisma Studio.

```terminal
npx prisma studio
```

5. 這將 open a new tab in your browser where 您可以 參見 `User`, `Session`, and `Account` tables and their contents.
