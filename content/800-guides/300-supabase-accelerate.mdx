---
title: 'Set up Supabase with Accelerate 連線 Pool'
metaTitle: "Set up PostgreSQL on Supabase with Prisma Accelerate's 連線 Pool"
...
This guides teaches you 如何 add connection pooling to a PostgreSQL database hosted on [Supabase](https://supabase.com/) using [Prisma Accelerate](/accelerate). 

Prisma Accelerate is a robust and mature connection pooler enabling your database to function properly during traffic spikes and high load scenarios. 查看 this [video](https://www.youtube.com/watch?v=cnL75if6Aq0) demonstrating how it performs in a load test or [learn why connection pooling is important](https://www.prisma.io/blog/saving-black-friday-with-connection-pooling).
...
If you already have a project using Prisma ORM, 您可以 skip the first two steps and jump ahead to [Step 3. Install the Accelerate extension](#3-install-the-accelerate-extension).
...
Then, run 以下 command to initialize a new project:

```
npx prisma init
```

這將 create a new `prisma` directory with a `schema.prisma` file and add a `.env` file with the `DATABASE_URL` environment variable.
...
Next, run 以下 command to introspect your database and create your data model:
...
If you want 使用 Prisma Migrate in the future, you also need to [baseline your database](/orm/prisma-migrate/workflows/baselining).
...
設定 Accelerate in the Prisma Console, follow these steps:
...
Once you went through these steps, you'll be redirected to another page where 您需要 the click the **Generate API key** button.
...
If you want 使用 Prisma Migrate with Prisma Accelerate, 您需要 provide a direct database URL in your `prisma.config.ts` file. The Accelerate URL (starting with `prisma://`) is used for queries via PrismaClient, while the direct database URL is used for migrations.
...
With your Prisma schema in place, 您可以 go ahead and generate Prisma Client:
...
At this point, 您可以 now start sending queries which will be routed through the connection pool to your database.