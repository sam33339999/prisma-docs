---
title: '如何 use Prisma ORM with TanStack 啟動'
metaTitle: '如何 use Prisma ORM and Prisma Postgres with TanStack 啟動'
description: 'Learn how to use Prisma ORM in a TanStack Start app'
sidebar_label: 'TanStack 啟動'
...
For the purpose of this guide, we're using the same setup instructions that 您可以 find in the [TanStart Start docs](https://tanstack.com/start/latest/docs/framework/react/build-from-scratch).

:::

In the directory where you'd like 建立 your project, run 以下 commands:
...
這將 create a new folder called `tanstack-start-prisma`, navigate into it, and initialize a new Node.js project.

Open the directory in your IDE and create a `tsconfig.json` file with 以下 configuration:
...
Next, install TanStack Router and Vinxi, as TanStack Start currently 需要 them:
...
Update your `package.json` 使用 Vinxi's CLI. Add `"type": "module"` and modify the scripts 使用 Vinxi's CLI:
...
您可以 create them with these commands:
...
您應該 be seeing an error about `routeTree.gen.ts` not existing. This is expected. It will be generated when you run TanStack Start for the first time.
...
這將 generate the `routeTree.gen.ts` file and resolve any routing errors.
...
In `schema.prisma`, create a model for our posts and change the generator 使用 the [`prisma-client`](/orm/prisma-schema/overview/generators#prisma-client) provider:
...
Store the response from the loader in 主要 component using [`Route.useLoaderData()`](https://tanstack.com/router/latest/docs/framework/react/api/router/useLoaderDataHook):
...
You've successfully integrated Prisma ORM with TanStack Start, creating a seamless full-stack application. Here are a few suggestions for what 您可以 do next:
