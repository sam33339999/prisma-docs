---
title: '如何 embed Prisma Studio in a Next.js app'
metaTitle: '如何 embed Prisma Studio in a Next.js app with Prisma Postgres'
description: 'Learn how to embed Prisma Studio directly in your Next.js application for database management'
sidebar_label: '嵌入 Prisma Studio (with Next.js)'
...
Prisma Studio can be embedded directly into your Next.js application using the [`@prisma/studio-core`](https://www.npmjs.com/package/@prisma/studio-core) package. This guide walks you through the setup so 您可以 manage your database from within your app instead of running Prisma Studio separately.
...
✔️ Free 使用 in production
⚠️ Prisma branding must remain visible and unchanged
🔐 移除 branding or 學習關於 upcoming partner-only features, reach out at [partnerships@prisma.io](mailto:partnerships@prisma.io)
...
這會建立 the tables in your Prisma Postgres database and generates the Prisma Client.

### 2.6. Seed your database (optional)

Create a seed file 添加 some sample data. Create a `seed.ts` file in the `prisma` folder and add 以下 code:
...
Now that you have Prisma ORM and Prisma Postgres set up, 您可以 embed Prisma Studio in your Next.js app. 
...
If you encounter a dependency resolution error while installing `@prisma/studio-core`, 您可以 force the install with:
...
Next, set up a backend endpoint that Prisma Studio can communicate with. This endpoint receives SQL queries from the embedded Studio UI, forwards them to your Prisma Postgres database, and then returns 結果s (or errors) back to the frontend.
...
### 3.4. Create 主要 Studio page

Open the `app/page.tsx` file and replace the existing code to render the embedded Studio with 以下:
...
Open your browser and go to `http://localhost:3000`. 您應該 now see Prisma Studio running inside your application:
...
- Navigate through relationships (例如, view a user's posts) to confirm associations work.

Once these actions work as expected, your embedded Prisma Studio is set up and connected to your Prisma Postgres database.

## 後續步驟

At this point you have Prisma Studio running inside your Next.js application and connected to your Prisma Postgres database. 您可以 browse, edit, and manage your data without leaving your app. To make this setup production-ready, consider these improvements:

1. **Add authentication**: Currently, anyone who can open your app has access to Prisma Studio. [Add user authentication](/postgres/database/prisma-studio/embedding-studio#adding-user-authentication) and only allow specific roles (例如, admins) 使用 the embedded Studio. 您可以 do this by checking authentication tokens in your `/api/studio` endpoint before running queries.

2. **Use environment-specific configuration**: In development you may want a test database, while in production you'll need a separate live database. Update your `.env` file 使用 different `DATABASE_URL` values for each environment, and confirm that your `/api/studio` endpoint is reading the correct one.
...
For more patterns and examples, 參見 [Prisma Studio Core demo repository](https://github.com/prisma/studio-core-demo), which includes an alternative implementation using Hono.js and React. If you prefer a guided walkthrough, watch the YouTube video: [**Use Prisma Studio in Your Own Applications
