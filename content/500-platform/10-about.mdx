---
title: 'General'
metaTitle: 'General | Prisma 控制台'
metaDescription: '學習 關於 the 控制台 to integrate the Prisma 資料 平台 products.'
...
- [**User account**](#user-account): In order 使用 PDP products, 您需要 have a PDP user account. A _user_ will typically create one user account to manage all their workspaces, projects and environments. The _user_ can also be invited to join other workspaces to collaborate on the projects in that workspace.
...
A user account is the prerequisite for any interactions with PDP products. 您可以 use it to manage your workspaces (and their projects). A user account can be invited to collaborate on workspaces created by other users as well.

If 您需要 delete your user account, go [here](/platform/support#deleting-your-pdp-account).

### Workspace

您可以 create several workspaces. A workspace is an isolated space to host projects. A workspace can have multiple user accounts associated with it so that multiple users can collaborate on the the projects in the workspace.

In each workspace, 您可以:
...
您可以 have a single workspace that hosts several database. Within each database, 您可以 view
...
您可以 access Optimize within your [Prisma Data Platform account](https://console.prisma.io/optimize) workspace.
...
1. Navigate to the workspace where you want 使用 Optimize.
...
7. Copy the API key and store it securely. 這將 be used in your project's [`.env` file](/optimize/getting-started#22-add-the-optimize-api-key-to-your-env-file) via the `"OPTIMIZE_API_KEY"`. Finally, click the **"I've stored it securely"** button.

You now have your Optimize API key.

### Project

In each workspace, 您可以 create several projects. A project typically represents an application (a product or service). You typically have one [Prisma schema](/orm/prisma-schema/) per project.

In each project, 您可以:

- view and manage all environments in that project.

The number of project 您可以 create in a workspace depends on the [subscription plan](https://www.prisma.io/pricing?utm_source=docs&utm_medium=platform-docs) configured in that workspace.
...
The number of environments 您可以 create in a project depends on the [subscription plan](https://www.prisma.io/pricing?utm_source=docs&utm_medium=platform-docs) configured in your workspace.

## 資料庫連線 management

The **Database** tab in the left panel of a project environment lets you configure and manage connections to your remote database. Within this tab, the **Connections** section displays a table with 以下 columns:
...
The [subscription plan](https://www.prisma.io/pricing?utm_source=docs&utm_medium=platform-docs) you select in your workspace determines how many databases 您可以 create in that workspace:
...
您可以 configure all billing details in the **Billing** section of your workspace.
...
If you downgrade a subscription plan, you may need 刪除 some of your projects and/or their environments in order to adhere to the [limits](#environment) of the newly selected plan.

例如, if your workspace is on a **Business** plan and currently has 14 (out of 15) projects, you will need 刪除 at least 4 projects to adhere to the project limit of the **Pro** plan. Additionally, 您需要 make sure that the remaining projects don't have more than 6 environments per project to adhere to the environment limit of the **Pro** plan.

You also need to disable features that are exclusive to **Pro** or **Business** plans, such as Static IPs. Once these adjustments are made, including disabling Static IPs, 您可以 proceed to downgrade your subscription plan.

## Programmatic access via the 平台 CLI
...
閱讀更多 about the [Prisma CLI](/platform/platform-cli).
